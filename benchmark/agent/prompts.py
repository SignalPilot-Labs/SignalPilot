...
4. For each priority model in dependency order — complete ALL sub-steps before moving to the next model:
   ...
   b. Write the .sql file
      ...
      - COALESCE: ONLY use COALESCE(col, 0) for explicit count columns (count_*, total_count, num_*) from LEFT JOINs. For amount/monetary/SUM columns (amount, spend, revenue, etc.), do NOT use COALESCE — let NULLs remain. NULL means "no data for this row" which is semantically different from 0. The evaluator distinguishes NULL from 0.
