...
**No cross-references between skills.** Do NOT write "see dbt-write §3" or
"per Section 14." Skills load sequentially into the agent's context — by the
time dbt-write loads, dbt-workflow is already there. Cross-references add
confusion, not clarity. State each rule once, in the skill where it's most
relevant. Do NOT restate the same rule in multiple files.

**Skills must not overlap.** One skill owns one topic.
...
1. **Cross-reference audit** — every step number, file name, tool name referenced must exist
...
4. **Scope overlap** — topic covered in multiple places must say the same thing
5. **Default conflict** — two rules giving different defaults for the same situation → merge or prioritize

**Single source of truth.** Every fact lives in one place. Other places point to it, not restate it.
