...
| **Chat worker** | Separate gateway process that leases durable standalone-chat runs from PostgreSQL and invokes restricted scratch notebook sessions. |
...
Standalone data chat uses the gateway database as its only persistence and queue
authority. Visiting `/chats` creates no state. The first question atomically stores
an author-private conversation, user message, and queued run.

The separately deployed `signalpilot-chat-worker` claims runs with
`FOR UPDATE SKIP LOCKED`, renews a database lease, and executes analysis in an
ephemeral scratch notebook identity. Each execution receives a signed session token
that fixes its organization, author, project, branch, production connection, and
read-only capability set. The governed query layer still parses and blocks database
writes; the agent prompt is not the enforcement boundary.

Public status, progress, tool summaries, text deltas, and artifact notifications are
persisted as ordered events and replayed over resumable SSE. Browser disconnects do
not cancel work. Final assistant messages are inserted idempotently so an expired
lease can be reclaimed without duplicating an answer.

Standalone conversations never include notebook, Slack, or Notion traces. Artifact
snapshots are immutable and author-private: formula-safe CSV tables, static chart
PNGs with source CSV, and sanitized scriptless HTML reports.
