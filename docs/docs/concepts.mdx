...
The FastAPI backend that exposes the 54 MCP tools over `streamable-http`. It handles auth, rate limiting, SQL governance, query execution, and audit logging. All governance logic lives here — the AI agent never touches the database directly.
...
The [SignalPilot Claude Code plugin](https://github.com/SignalPilot-Labs/signalpilot-plugin) adds 23 skills and two verifier agents on top of the 54 MCP tools. Skills are markdown files that load into Claude Code's context automatically based on relevance — they guide Claude's behavior without you writing prompt instructions.
...
A named, encrypted database credential stored in the gateway. Each connection has a type (e.g. `snowflake`), a name used by tools like `list_database_connections`, and credentials encrypted with Fernet at rest.
