---
sidebar_position: 5
sidebar_label: Connect a database
...
import { Warehouse } from '@site/src/components/illustrations';

<PageHero
  eyebrow="Your data · Connect a database"
  title="Connect a database"
  lede="SignalPilot connects to 11 databases and warehouses. Credentials are encrypted at rest and never shown to the AI. It only ever sees results."
...
For self-hosted deployments, you can pre-configure connections via environment variables instead of the API. See [Configuration](/docs/self-host/configuration) for the variable names.
...
Self-hosted deployments where the warehouse sits on a private network can allow RFC1918 targets with `SP_ALLOW_PRIVATE_CONNECTIONS=true` (loopback and link-local stay blocked).
