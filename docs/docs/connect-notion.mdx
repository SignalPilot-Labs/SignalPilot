...
Add a comment on that page and mention the `SignalPilot` agent, then write the analysis request you want SignalPilot to run:
...
Select the `SignalPilot` agent from Notion's picker. SignalPilot ignores unrelated comments and only handles events routed to the SignalPilot connection.
...
Confirm the page where you commented has SignalPilot added as a connection, and confirm your comment mentions the `SignalPilot` agent.
