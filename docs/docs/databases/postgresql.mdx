...
Supported in both cloud and local (self-hosted) deployments. In cloud mode, connections to private address ranges are refused; self-hosted deployments can allow them with `SP_ALLOW_PRIVATE_CONNECTIONS=true`.
