...
import { Evals } from '@site/src/components/illustrations';

<PageHero
  eyebrow="Evals · Overview"
  title="Evals"
  lede="An eval is a graded question you already know the answer to. SignalPilot runs each one as a real agent against your real warehouse, in a sandbox, and scores what comes back."
  illustration={<Evals />}
/>
...
2. **Ship the project.** The gateway clones the dbt project repository linked
   in the eval configuration (at the configured branch, or the default branch),
   strips `.git`, and hands each sandbox a short-lived presigned tarball. The
   sandbox never gets a git credential. A run with no project repository
   configured stops before it starts.
3. **Fan out.** Tasks run concurrently, up to the configured parallelism.
   Each one is a short-lived, isolated sandbox (Docker locally, a hosted
   sandbox on Cloud) running the Claude CLI against the SignalPilot MCP server.
...
One subtlety: a container that hits its timeout is only `ERROR` if it produced
**no** answer. If a truncated transcript still contains a number, that number is
graded like any other — so a task can time out and still score `CORRECT`. Check
task durations against `SP_EVAL_TIMEOUT_SECONDS` when a result looks surprising.
