...
2. **A warehouse connection** the graded agent will use. Add it under
   **Connections**. Give it read-only credentials unless you plan to run write
   tasks.
3. **A paid plan and admin access.** Evals are available to every organization
   on a paid plan. Eval configuration and run triggering also require the
   `admin` scope.
...
| **Project repository** | **Required.** The dbt project every run evaluates: a `https://github.com/…` URL (public, or covered by your GitHub installation), or a path under `SP_EVAL_PROJECTS_DIR` when self-hosting. Optionally pin a branch; empty means the repository default branch. Local paths are checked when you save. |
...
Click **Run all evals**. Tasks fan out concurrently, and the page shows each one
