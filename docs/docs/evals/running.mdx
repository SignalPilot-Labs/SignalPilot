...
sandbox backing it. The sandbox panel streams container logs and surfaces sandbox
events, including `oom_killed` when the task ran out of memory. That is usually the fastest way to explain a task that
...
denominator comes from the configured project repository checkout (its
`target/manifest.json` if present, otherwise the `models/` tree). Both `covers`
and `builds` count as declared.
...
All routes are under `/api`. Except where noted, each requires the organization
to be on a paid plan (`402 plan_required` otherwise). Routes with the `admin`
scope also require a deployment wired to run evals (`503
not_available_in_deployment` otherwise).

| Route | Scope | Purpose |
|---|---|---|
| `GET /evals/availability` | `read` | `{"billable", "capable", "enabled"}`: whether the org is on a paid plan, whether the deployment can run evals, and both. **Not** plan-gated — that is how a free org can read its answer. |
...
| `GET /evals/sandboxes` · `…/{name}/events` · `…/{name}/logs/stream` | `read` | Live sandboxes, sandbox events, log stream. |
| `POST /evals/upload/{initiate,complete,abort}` | `write` | Eval-set zip intake. Not plan-gated. |
...
image to any `read`-scoped caller of a paid organization.
