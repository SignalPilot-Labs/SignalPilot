...
- **Teardown runs if — and only if — a setup script ran**, pass or fail. A task
  that declares `teardown` without `setup` is accepted by the manifest and its
  teardown never executes. The branch is destroyed immediately afterwards, so
  teardown is for releasing warehouse-side resources and for leaving a trace in
  the log, not for cleaning up data.
