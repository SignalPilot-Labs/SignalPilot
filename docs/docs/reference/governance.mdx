...
<MiniCopyBlock
  label="blocked ddl"
  plain
  code="CREATE    DROP    ALTER    TRUNCATE    RENAME"
/>
...
<MiniCopyBlock
  label="blocked dml"
  plain
  code="INSERT    UPDATE    DELETE    MERGE    UPSERT    REPLACE"
/>
...
118+ functions are blocked across 10 SQL dialects (an unrecognised dialect is checked against the union of all of them). These are functions that can read files, execute OS commands, make network calls, or access sensitive system information.
...
The full denylist is maintained in `signalpilot/gateway/gateway/engine/denylists.py` in the gateway source.

## Per-session budget cap

Budget caps are registered per session through the gateway's budget API (`/api/budget`); the `check_budget` MCP tool reports remaining spend. A session without a registered budget has no spending limit.
...
- **Fernet/MultiFernet** — connection credentials encrypted at rest using `SP_ENCRYPTION_KEY` and optional rotation keys in `SP_ENCRYPTION_KEY_OLD`
- **One-way API-key storage** — high-entropy API keys are stored as SHA-256 hashes and shown only at creation
