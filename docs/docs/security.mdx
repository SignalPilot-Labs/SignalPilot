...
- Eval configuration, execution, evidence, and live-sandbox routes require an organization on a paid plan; execution additionally requires the `admin` API scope and a deployment wired to run evals. There is no platform-staff list and no organization allowlist.
