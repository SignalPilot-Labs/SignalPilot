...
- **Gateway** — a single container (FastAPI + MCP server) that owns connections,
  ...
  TLS reverse proxy.
- **Chat worker** — the same gateway image with the `signalpilot-chat-worker`
  command. It owns durable standalone-chat execution leases and must share the
  gateway database, notebook JWT secret, and orchestrator configuration.
...
When enabling standalone data chat, set `SP_FEATURE_STANDALONE_CHAT=true` on both
processes and start the durable worker from the same digest-pinned gateway image:

<MiniCopyBlock
  label="run standalone chat worker"
  code={`docker run -d --name signalpilot-chat-worker \\\n+  --env-file /secure/signalpilot-gateway.env \\\n+  -e SP_FEATURE_STANDALONE_CHAT=true \\\n+  -e CHAT_WORKER_CONCURRENCY=4 \\\n+  your-registry/gateway:VERSION signalpilot-chat-worker`}
/>

Do not run this queue as a request-local FastAPI background task. Multiple worker
replicas are supported because PostgreSQL leases and row locks coordinate claims.
