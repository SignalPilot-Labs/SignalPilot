...
Writes a `.py` notebook file into the workspace and executes it in an isolated sandbox. Returns cell outputs and a URL to view the notebook in the browser.
...
The first call creates the sandbox and workspace. Later calls reuse the running sandbox for the same org/user, so sandbox-local state persists across calls within a session. Each execution runs `sp export session` on the file and commits the result to the workspace git repo.
...
Pre-installed packages in the sandbox: pandas, polars, numpy, duckdb, sqlglot, altair, plotly, matplotlib, seaborn, scikit-learn, scipy, pyarrow, dbt-core, dbt-duckdb, anthropic, openai, mcp.

## Isolation

Each session runs in its own isolated microVM, one per org/user. Isolation is enforced at several layers:

- **One microVM per session.** Nothing is shared with other sessions or with the host.
...
- **Fixed resources.** CPU and memory are fixed per sandbox (`SP_NOTEBOOK_VCPUS`, `SP_NOTEBOOK_MEMORY_MB`), and `SP_NOTEBOOK_MAX_RUNNING_PER_ORG` (default 20) caps how many sandboxes one organization can run at once.
...
- **Snapshot and resume.** A sandbox stays warm across `run_notebook` calls for fast iteration. After `SP_NOTEBOOK_IDLE_SNAPSHOT_SECONDS` (15 minutes) of inactivity it is snapshotted and destroyed. The next call resumes from the snapshot, which is kept for `SP_NOTEBOOK_SNAPSHOT_EXPIRATION_SECONDS` (7 days). Durable state is the committed workspace git repo, not the sandbox filesystem.
