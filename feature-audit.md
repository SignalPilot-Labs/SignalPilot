...
## Round 25: Auth Bypass on Non-StoreD Endpoints & Credential Export Scope Escalation

### COMPLETED

- [x] **Auth Bypass on 20 Endpoints (MEDIUM-HIGH)** — `require_scopes()` treats `auth is None` as "grant all scopes" (correct for JWT users verified by `resolve_user_id`). But 20 endpoints had `RequireScope()` guards without a `StoreD` or `UserID` dependency, meaning the JWT was never verified. An attacker could send `Authorization: Bearer fake_token` and bypass auth entirely. Fixed by adding `_: UserID` parameter to all 20 endpoints across 5 files: `cache.py` (5), `budget.py` (4), `sandboxes.py` (2), `connections.py` (8 + 1 capabilities duplicate), `metrics.py` (1 capabilities duplicate). `UserID` forces `resolve_user_id` to run, verifying the JWT in cloud mode.
