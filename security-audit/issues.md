...
|---|------|--------|----------|--------------|--------|---------|
| 7 | `mcp-auth-passthrough-when-no-keys-exist` | MCP accepts ALL unauthenticated requests when zero API keys exist | Critical | Yes | Open | [link](mcp-auth-passthrough-when-no-keys-exist-proposal.md) |
| 1 | `clerk-jwt-audience-not-verified` | Clerk JWT `aud` claim not verified | High | Yes | Open | [link](clerk-jwt-audience-not-verified-proposal.md) |
| 9 | `sandboxes-store-not-org-scoped` | In-memory sandbox store shared across all tenants | High | Yes | Open (deprioritized — feature disabled) | [link](sandboxes-store-not-org-scoped-proposal.md) |
| 8 | `mcp-auth-key-lookup-leaks-cross-tenant-keys` | MCP API key validated against all orgs, no tenant scope | High | Yes | Open | [link](mcp-auth-key-lookup-leaks-cross-tenant-keys-proposal.md) |
| 10 | `scope-guard-bypass-for-jwt-requests` | All scopes granted to JWT requests regardless of endpoint dependencies | High | Yes | Open | [link](scope-guard-bypass-for-jwt-requests-proposal.md) |
| 31 | `csp-allows-unsafe-inline-and-unsafe-eval` | CSP `script-src` includes `unsafe-inline` and `unsafe-eval` in production | High | Yes | Open | [link](csp-allows-unsafe-inline-and-unsafe-eval-proposal.md) |
...
| 11 | `org-id-fallback-to-local-in-cloud` | `Store` defaults `org_id="local"` when org_id is unset in cloud mode | Medium | Yes | Open | [link](org-id-fallback-to-local-in-cloud-proposal.md) |
...
| 18 | `api-key-storage-allows-user-id-null-and-org-id-null` | API keys with `org_id=None`/`"local"` accepted in cloud mode | Medium | Yes | Open | [link](api-key-storage-allows-user-id-null-and-org-id-null-proposal.md) |
| 19 | `sql-validation-bypass-via-cte-or-set-op-with-non-select-leaf` | SELECT-only validator may permit dangerous dialect functions | Medium | Yes | Open | [link](sql-validation-bypass-via-cte-or-set-op-with-non-select-leaf-proposal.md) |
| 21 | `inject-limit-fallback-string-concat-on-parse-failure` | `inject_limit` falls back to string concatenation on parse failure | Medium | Yes | Open | [link](inject-limit-fallback-string-concat-on-parse-failure-proposal.md) |
...
| 27 | `sandbox-execute-no-tenant-binding` | Sandbox manager `/execute` has no authentication | Medium | Partial | Open (deprioritized — feature disabled) | [link](sandbox-execute-no-tenant-binding-proposal.md) |
| 29 | `sandbox-output-html-rendered-with-dangerously-set-inner-html` | Sandbox HTML output rendered via `dangerouslySetInnerHTML` | Medium | Yes | Open (deprioritized — feature disabled) | [link](sandbox-output-html-rendered-with-dangerously-set-inner-html-proposal.md) |
| 33 | `localStorage-stores-api-key-vulnerable-to-xss` | API key persisted in `localStorage` — exfiltratable by XSS | Medium | Yes | Open | [link](localstorage-stores-api-key-vulnerable-to-xss-proposal.md) |
| 38 | `dockerfile-gateway-runs-as-root` | `Dockerfile.gateway` runs uvicorn as UID 0 | Medium | Yes | Open | [link](dockerfile-gateway-runs-as-root-proposal.md) |
| 40 | `sp-encryption-key-derivation-allows-passphrase-with-deterministic-salt` | Passphrase salt derived from passphrase itself defeats salting | Medium | Yes | Open | [link](sp-encryption-key-derivation-allows-passphrase-with-deterministic-salt-proposal.md) |
| 42 | `cors-allow-credentials-true-with-permissive-origin-list` | `allow_credentials=True` with origins from env var including localhost | Medium | Yes | Open | [link](cors-allow-credentials-true-with-permissive-origin-list-proposal.md) |
...
| 60 | `mcp-xff-uses-leftmost-spoofable-by-attacker` | MCP audit log trusts attacker-controlled leftmost X-Forwarded-For | Medium | Yes | Open | [link](mcp-xff-uses-leftmost-spoofable-by-attacker-proposal.md) |
...
| 46 | `client-ip-trust-x-forwarded-for-without-trusted-proxy-list` | X-Forwarded-For honored without trusted proxy validation | Low | Yes | Open | [link](client-ip-trust-x-forwarded-for-without-trusted-proxy-list-proposal.md) |
...
| 57 | `byok-aws-config-passed-as-json-env-var` | AWS credentials in JSON env var instead of IAM role | Low | Yes | Open | [link](byok-aws-config-passed-as-json-env-var-proposal.md) |
...
| 44 | `request-correlation-header-not-validated` | `X-Request-ID` validation already in place; residual log-volume concern | Info (was Low) | Yes | **Stale — mostly fixed** | [link](request-correlation-header-not-validated-proposal.md) |
...
|----------|-------|-------|
| Critical | 1 | #7 |
| High | 6 | #1, #8, #9, #10, #31, #59 (new) |
| Medium | 21 | (#56 fixed-but-counted; net effective open Medium = 20) |
| Low | 23 | includes 4 new (#63, #64, #65, #66) |
| Info | 9 | #44 demoted from Low; 2 new (#67, #68) |
| **Total** | **68** | 56 valid open + 1 stale + 1 fixed + 10 new |
