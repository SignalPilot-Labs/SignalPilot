...
# Authenticate Claude CLI using the setup token
# CLAUDE_SETUP_TOKEN is passed as env var, never written to disk
if [ -n "$CLAUDE_SETUP_TOKEN" ]; then
    echo "[entrypoint] Authenticating Claude CLI..."
    echo "$CLAUDE_SETUP_TOKEN" | claude setup --token - 2>/dev/null || \
    claude setup --token "$CLAUDE_SETUP_TOKEN" 2>/dev/null || \
