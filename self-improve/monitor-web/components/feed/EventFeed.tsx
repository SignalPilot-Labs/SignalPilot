  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        key={
          activity.kind +
          (activity.kind === "waiting_tool" ? activity.toolName : "")
        }
      ...
      >
        ...
        {activity.kind === "writing" && (
          <span className="text-[10px] text-[#88ccff]/80 flex items-center gap-1.5">
            ...
            Generating response...
            <span className="text-[#88ccff]/40 truncate max-w-[200px]">
              {activity.text}
            </span>
          </span>
        )}

        {(activity.kind === "calling_tool" ||
          activity.kind === "waiting_tool") && (
          <span className="text-[10px] text-[#ffaa44]/80 flex items-center gap-1.5">
            <span className="opacity-60 shrink-0">
              {getToolIcon(getToolCategory(activity.toolName), "#ffaa44")}
            </span>
            <span className="font-medium">{activity.toolName}</span>
            {activity.detail && (
              <span className="text-[#ffaa44]/40 truncate max-w-[200px]">
                {activity.detail}
              </span>
            )}
          </span>
    ...
    <div className="flex-1 flex flex-col min-h-0 relative">
      ...
      <div className="flex items-center gap-4 px-4 py-2 border-b border-[#1a1a1a] bg-[#0a0a0a]/80 frosted-glass">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-[#e8e8e8] font-semibold tabular-nums">
            {events.length}
          </span>
        </div>
        <div className="flex items-center gap-1.5">
