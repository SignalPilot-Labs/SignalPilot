...
export function SlotCard({
  ...
  return (
    <div className="border border-[#1a1a1a] rounded-lg bg-[#0a0a0a] p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          {isActive && (
            ...
            >
              <span className={clsx("w-1.5 h-1.5 rounded-full transition-colors", healthColor)} />
      ...
      {/* Prompt preview */}
      {slot.prompt && (
        <p className="text-[10px] text-[#999] mb-2 line-clamp-2">{slot.prompt}</p>
      )}
            ...
            <div className="mt-1.5 space-y-0.5 pl-3 border-l border-[#1a1a1a]">
              {slot.volume_name && (
                <p className="text-[9px] text-[#555] font-mono">
                  Volume: <span className="text-[#666]">{slot.volume_name}</span>
                </p>
              )}
              {slot.container_id && (
                <p className="text-[9px] text-[#555] font-mono">
                  Container: <span className="text-[#666]">{slot.container_id.slice(0, 12)}</span>
                </p>
              )}
            </div>
          )}
            ...
            <div className="space-y-1.5">
              <textarea
                ...
                onKeyDown={e => {
                  ...
                  if (e.key === "Escape") { setShowInject(false); setInjectText(""); }
                }}
              ...
              <div className="flex items-center justify-between">
                <span className="text-[9px] text-[#555]">Ctrl+Enter to send · Esc to cancel</span>
                <Button
                  variant="success"
