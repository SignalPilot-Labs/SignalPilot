...
function extractFileChanges(events: FeedEvent[]): FileChange[] {
  ...
  for (const ev of events) {
    ...
    switch (cat) {
      case "read": {
        const fileObj = (output as Record<string, unknown>)?.file as
          | Record<string, unknown>
          | undefined;
        const fp =
          (input.file_path as string) || (fileObj?.filePath as string) || "";
        if (fp)
          changes.push({
            path: norm(fp),
            action: "read",
      ...
      case "write": {
        const fp =
          (input.file_path as string) || (output.filePath as string) || "";
        if (fp) {
          ...
          let added = 0;
          if (patch)
            for (const h of patch) added += (h.newLines as number) || 0;
          changes.push({
            path: norm(fp),
            action: "write",
    ...
    <div>
      ...
      >
        ...
        {(totalAdded > 0 || totalRemoved > 0) && (
          <span className="flex items-center gap-1 shrink-0">
            {totalAdded > 0 && (
              <span className="text-[9px] text-[#00ff88]/70 tabular-nums">
                +{totalAdded}
              </span>
            )}
            {totalRemoved > 0 && (
              <span className="text-[9px] text-[#ff4444]/70 tabular-nums">
                -{totalRemoved}
              </span>
            )}
          </span>
      ...
      <AnimatePresence>
        {open && isDir && (
          ...
          >
            {sorted.map((child) => (
              <NodeItem key={child.fullPath} node={child} depth={depth + 1} />
            ))}
          </motion.div>
        )}
      </AnimatePresence>
  ...
  return (
    <div className="space-y-0.5">
      {sorted.map((f, i) => (
        <div
          key={i}
          className="flex items-center gap-1.5 px-2 py-[3px] text-[10px] hover:bg-white/[0.02] rounded transition-colors"
        >
          <span
            className={clsx(
...
export function WorkTree({
  ...
  const liveTree = useMemo(
    () => buildTreeFromChanges(liveChanges),
    [liveChanges],
  );

  // Git diff tree
  const diffTree = useMemo(
    () => (diffData?.files ? buildTreeFromDiff(diffData.files) : null),
    [diffData],
  );
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[#1a1a1a]">
        {!collapsed && (
          <>
              ...
              <span
                className={clsx(
                  ...
                  diffData.source === "live"
                    ? "text-[#00ff88]/50 bg-[#00ff88]/8"
                    : diffData.source === "stored"
                      ? "text-[#88ccff]/50 bg-[#88ccff]/8"
            ...
            <span className="text-[10px] text-[#777] tabular-nums ml-auto">
              {totalFiles} files
            </span>
          </>
        )}
        ...
        <>
          ...
          {(totalAdded > 0 || totalRemoved > 0) && (
            <div className="flex items-center gap-3 px-3 py-1.5 border-b border-[#1a1a1a]/60 text-[10px] text-[#888]">
              ...
              {totalAdded > 0 && (
                <span className="text-[#00ff88]/70">+{totalAdded}</span>
              )}
              {totalRemoved > 0 && (
                <span className="text-[#ff4444]/70">-{totalRemoved}</span>
              )}
            </div>
          )}
          ...
          <div className="flex border-b border-[#1a1a1a]/60">
            {hasGitDiff && (
              <button
                ...
                className={clsx(
                  "flex-1 py-1.5 text-[10px] font-medium text-center transition-colors",
                  activeTab === "tree"
                    ? "text-[#e8e8e8] border-b border-[#00ff88]"
