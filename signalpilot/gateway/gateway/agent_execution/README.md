...
Apply gateway Alembic migrations through `0027`. Build the agent image from the
repository root:
...
and an opt-in real Docker watchdog test. PostgreSQL 17 passed the `0026` to
`0027` upgrade, downgrade, and re-upgrade. Simultaneous PostgreSQL admissions
