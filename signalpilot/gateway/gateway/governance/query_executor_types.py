...
Also the typed row encoding shared by every path. A fresh execution and a
replay of the same ``sql_hash`` must hand callers the same Python types.
Rows are therefore encoded exactly once, with ``encode_rows``, and both the
durable copy and the in-memory result carry that encoding: numbers stay
numbers (``Decimal`` becomes ``int`` or ``float``), temporal values become
ISO 8601 strings, and bytes become base64. ``columns[].logical_type``
records the driver's original type so a reader can re-hydrate dates and
timestamps when it needs them.
"""
