...
- Content-Security-Policy: frame-ancestors 'self' only (not the default CSP).
  marimo's own HTML/JS sets its own internal CSP. We do not layer one on top
...
- Cache-Control: NOT forced to no-store — let marimo's own caching headers pass
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        # Notebook proxy paths get relaxed framing and caching headers so marimo works.
