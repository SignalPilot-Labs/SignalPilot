...
- Strips upstream Set-Cookie so marimo's session cookie does not leak to the gateway origin.
            ...
            try:
                while True:
                    try:
                        ...
                        try:
                            await ws.close(code=upstream_ws.close_code or 1000)
                        except Exception:
                            pass
                        break
                    except websockets.exceptions.ConnectionClosedError as exc:
                        close_code = exc.rcvd.code if exc.rcvd else 1011
                        logger.debug("Upstream WS closed with error: code=%s", close_code)
                        try:
