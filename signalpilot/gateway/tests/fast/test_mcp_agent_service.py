...
@pytest.mark.asyncio
async def test_runtime_environment_isolates_claims_reaping_and_continuation(service, monkeypatch):
    monkeypatch.setenv("SP_RUNTIME_ENV", "production")
    queued = await service.start("org", "owner", request())
    stale = await service.start("org", "owner", request())
    async with service.factory() as db:
        row = await db.get(MCPAgentThread, stale.thread_id)
        row.status, row.lease_expires_at = "running", time.time() - 1
        await db.commit()
    monkeypatch.setenv("SP_RUNTIME_ENV", "staging")
    assert not await service.run_once()
    with pytest.raises(ValueError, match="not found"):
        await service.resume("org", "owner", queued.thread_id, "continue")
    with pytest.raises(ValueError, match="not found"):
        await service.cancel("org", "owner", stale.thread_id)
    async with service.factory() as db:
        assert (await db.get(MCPAgentThread, queued.thread_id)).status == "queued"
        assert (await db.get(MCPAgentThread, stale.thread_id)).status == "running"
    monkeypatch.setenv("SP_RUNTIME_ENV", "production")
    assert await service.run_once()
    assert (await service.get("org", "owner", queued.thread_id)).status == "completed"
    assert (await service.get("org", "owner", stale.thread_id)).status == "failed"
