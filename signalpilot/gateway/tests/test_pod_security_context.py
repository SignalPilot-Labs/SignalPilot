...
- Required env vars: PYTHONDONTWRITEBYTECODE, HOME, MARIMO_LOG_DIR.
...
class TestPodSecurityContext:
    ...
    def test_pod_env_has_marimo_log_dir_in_tmp(self):
        """Pod env contains MARIMO_LOG_DIR pointing to a path under /tmp."""
        manifest = _make_manifest()
        env = manifest["spec"]["containers"][0]["env"]
        env_dict = {e["name"]: e["value"] for e in env}
        assert "MARIMO_LOG_DIR" in env_dict
        assert env_dict["MARIMO_LOG_DIR"].startswith("/tmp")
