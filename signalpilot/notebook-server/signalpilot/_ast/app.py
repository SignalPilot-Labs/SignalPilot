...
from signalpilot._ast.cell import Cell, CellConfig, CellImpl, ImportWorkspace
from signalpilot._ast.cell_id import external_prefix
from signalpilot._ast.cell_manager import CellManager
from signalpilot._ast.errors import (
    CycleError,
    MultipleDefinitionError,
    SetupRootError,
    UnparsableError,
)
from signalpilot._ast.parse import ast_parse
from signalpilot._ast.variables import BUILTINS
from signalpilot._convert.converters import SpConvert
from signalpilot._messaging.mimetypes import KnownMimeType
from signalpilot._messaging.notebook.changes import SetConfig, Transaction
...
from signalpilot._runtime.app.kernel_runner import AppKernelRunner
from signalpilot._runtime.app.script_runner import AppScriptRunner
...
if TYPE_CHECKING:
    ...
    from signalpilot._messaging.notification import HumanReadableStatus
    from signalpilot._plugins.core.web_component import JSONType
    from signalpilot._runtime.context.types import ExecutionContext
...
class _SetupContext:
    ...
    See design discussion in MEP-0008 (github:signalpilot-team/meps/pull/8).
    """
...
class App:
    ...
    A sp notebook is a dataflow graph, with each node computing a Python
    function.
    """
    ...
    def function(
        ...
        **kwargs: Any,
        ...
        """A decorator to wrap a callable function into a sp cell.
    ...
    def class_definition(
        ...
        **kwargs: Any,
        ...
        """A decorator to wrap a class into a sp cell.
    ...
    def setup(self) -> _SetupContext:
        ...
        This block should only be utilized at the start of a sp notebook.
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        """
        Run the sp app and return its outputs and definitions.

        Use this method to run sp apps programmatically and retrieve their
        ...
        Args:
            defs (dict[str, Any]):
                ...
                arguments. sp will use these values instead of executing
        ...
        Environment Variables:
            SP_SCRIPT_EDIT: If set, opens the notebook in edit mode instead
                of running it. Requires the app to have a filename.
        """
        del kwargs
        # Enabled specifically for debugging purposes.
        # see docs.signalpilot.ai.io/guides/debugging
        if os.environ.get("SP_SCRIPT_EDIT"):
            # unset the env var to avoid recursion
            os.environ.pop("SP_SCRIPT_EDIT")
            from signalpilot._cli.cli import edit

            if self._filename is None:
                raise RuntimeError(
                    "SP_SCRIPT_EDIT is set, but filename cannot be determined."
                )
        ...
        try:
            ...
            if self._filename is not None:
                ...
                signalpilot_error = type(e)(str(e))
                raise signalpilot_error from None
            else:
                raise
    ...
    async def embed(
        ...
        notebook that in turn embeds another notebook, and sp will do the
        right thing.
        ...
        """
        from signalpilot._plugins.stateless.flex import vstack
...
class InternalApp:
    ...
    def to_py(self) -> str:
        return SpConvert.from_ir(self.to_ir()).to_py()
