...
from signalpilot._ast.sql_visitor import SQLRef, SQLVisitor
from signalpilot._ast.visitor import ImportData, Language, Name, VariableData
from signalpilot._messaging.msgspec_encoder import asdict
from signalpilot._runtime.exceptions import SpRuntimeException
from signalpilot._types.ids import CellId_t
from signalpilot._utils.deep_merge import deep_merge
...
# sp-error: cell was prevented from executing
...
class Cell:
    ...
    Cells are the fundamental unit of execution in sp. They represent
    ...
    def my_cell(mo, x, y):
        z = x + y
        sp.md(f"The value of z is {z}")  # This will output markdown
        return (z,)
    ```
    ...
    Cells can be named via the sp editor in the browser, or by
    ...
    def run(
        ...
        Examples:
            sp cells can be given names either through the editor cell menu
        ...
        Args:
            **refs (Any):
                ...
                arguments. sp will automatically compute values for any refs
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> Any:
        ...
        is_pytest = (
            ...
            or "SP_PYTEST_WASM" in os.environ
        )
        ...
        if self._pytest_reserved - set(arg_names):
            raise TypeError(
                ...
                "resave the notebook in the sp editor."
            )
