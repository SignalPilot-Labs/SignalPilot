...
from signalpilot._ast.cell import CellConfig, CellImpl
...
from signalpilot._ast.toplevel import TopLevelExtraction, TopLevelStatus
from signalpilot._ast.variables import BUILTINS
from signalpilot._ast.visitor import Name, VariableData
from signalpilot._convert.converters import SpConvert
from signalpilot._schemas.serialization import NotebookSerializationV1
    ...
    for ref in sorted_refs:
        if ref not in allowed_refs:
            if not ref.isidentifier():
                ...
                # eg. `def _(sp, my_schema.my_table):`
...
def to_top_functiondef(
    ...
    if cell.code:
        assert toplevel_var.kind in ("function", "class"), (
            "Unexpected cell kind, please report an issue to docs.signalpilot.ai/docs/"
        )
...
def generate_app_constructor(config: _AppConfig | None) -> str:
    ...
    return format_tuple_elements("app = sp.App(...)", kwargs)
...
def generate_filecontents_from_ir(ir: NotebookSerializationV1) -> str:
    ...
    silent = (
        SpPath(ir.filename).is_markdown()
        if ir.filename and ir.filename != "<sp>"
        else False
    )
...
def recover(filepath: Path) -> str:
    ...
    return SpConvert.from_notebook_v1(json.loads(contents)).to_py()
...
def get_header_comments(filename: str | Path) -> str | None:
    ...
    invalid, which is determined by:
        1. If the file is does not contain the sp
            import statement
        2. If the section before the sp import
    ...
    if "import signalpilot" not in contents:
        return None
    header, _ = re.split(
        r"^import signalpilot", contents, maxsplit=1, flags=re.MULTILINE
    )
