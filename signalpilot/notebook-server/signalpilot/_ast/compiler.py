...
from signalpilot._ast import parse
...
from signalpilot._ast.names import SETUP_CELL_NAME, TOPLEVEL_CELL_PREFIX
from signalpilot._ast.pytest import has_fixture_decorator
from signalpilot._ast.transformers import ContainedExtractWithBlock
from signalpilot._ast.variables import is_local
from signalpilot._ast.visitor import ImportData, Name, ScopedVisitor
from signalpilot._schemas.serialization import CellDef, ClassCell, FunctionCell
from signalpilot._types.ids import CellId_t
from signalpilot._utils.tmpdir import get_tmpdir
...
def cell_id_from_filename(filename: str) -> CellId_t | None:
    ...
    matches = re.findall(r"__sp__cell_(.*?)_", filename)
    if matches:
...
def get_filename(cell_id: CellId_t, suffix: str = "") -> str:
    ...
    basename = f"__sp__cell_{cell_id}_"
...
def _extract_markdown(tree: ast.Module) -> str | None:
    ...
    try:
        (body,) = tree.body
        # Only match bare expressions like `sp.md("...")`, not assignments
        # like `title = sp.md("...")` — both ast.Expr and ast.Assign have a
        ...
        if not value.args:  # Handle sp.md() with no arguments
            return None
    ...
    # Dedent behavior is a little different that in sp js, so handle
    # accordingly.
...
def _build_source_position_map(
    ...
    the same file.
    """
    from signalpilot._ast.load import _maybe_contents
...
def get_source_position(
    ...
    if module := inspect.getmodule(f):
        ...
        is_script = spec is None or spec.name != "sp_app"
...
def cell_factory(
    ...
    signature, sp will autofix them on save.
    """
