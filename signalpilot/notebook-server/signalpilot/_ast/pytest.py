...
from signalpilot._ast.fast_stack import fast_stack
...
SP_TEST_STUB_NAME = "SpTestBlock"

block_incrementer = itertools.count()


class SpTest:
    """Base class for SignalPilot tests."""
...
def build_test_class(
    ...
    class SpTestBlock_idx(SpTest):
        @staticmethod
        def test_one_of_many_tests_in_a_given_cell():
            ...
        class TestSubclass(SpTest):
            def test_subclass_method_works(self):
                assert True
    ...
    for node in body:
        ...
        if not isinstance(
            ...
            raise RuntimeError(
                ...
                " Please report to signalpilot-team/signalpilot/issues."
            )
    ...
    def hook(var: str) -> Callable[..., Any] | type[SpTest]:
        ...
        if test is None:
            ...
            def fails(*args: Any, **kwargs: Any) -> NoReturn:
                ...
                raise ValueError(
                    ...
                    "signalpilot-team/signalpilot/issues."
                )
...
def process_for_pytest(func: Fn, cell: Cell) -> None:
    ...
    name = f"{SP_TEST_STUB_NAME}_{next(block_incrementer)}"
    ...
    # ensure sp/_ not in frame path, using this file as a reference.
