...
from signalpilot._cli.config.utils import highlight_toml_headers
from signalpilot._cli.help_formatter import ColoredCommand, ColoredGroup
from signalpilot._cli.print import echo, green
...
@click.group(
    cls=ColoredGroup, help="""Various commands for the sp config."""
)
def config() -> None:
    pass


@click.command(cls=ColoredCommand, help="""Show the sp config.""")
def show() -> None:
    """
    Print out sp config information.
    Example usage:
...
@click.command(cls=ColoredCommand, help="""Describe the sp config.""")
def describe() -> None:
    """Print documentation for all config options."""
    import inspect
    from textwrap import indent

    import signalpilot._config.config as signalpilot_config
    from signalpilot._cli.print import echo, green, muted, yellow
    ...
    def format_type_docs(cls: type, indent_level: int = 0) -> str:
        ...
        for key, type_hint in annotations.items():
            ...
            if hasattr(type_hint, "__forward_arg__"):
                ...
                # Import from signalpilot._config.config
                ...
                if type_hint in signalpilot_config.__dict__ and "TypedDict" in str(
                    type(signalpilot_config.__dict__[type_hint])
                ):
                    config = signalpilot_config.__dict__[type_hint]
    ...
    echo(format_type_docs(signalpilot_config.SpConfig))
