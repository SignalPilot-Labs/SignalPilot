...
from signalpilot._data.models import DataType
from signalpilot._messaging.errors import Error as SpError
from signalpilot._messaging.notification import NotificationMessage
from signalpilot._runtime.commands import CommandMessage
...
def _generate_server_api_schema() -> dict[str, Any]:
    ...
    import signalpilot._data._external_storage.models as storage
    import signalpilot._data.models as data
    import signalpilot._messaging.notification as notifications
    import signalpilot._secrets.models as secrets_models
    from signalpilot._ast.cell import CellConfig, RuntimeStateType
    from signalpilot._config import config
    from signalpilot._messaging import errors
    ...
    from signalpilot._metadata import opengraph
    from signalpilot._runtime import commands
    from signalpilot._runtime.packages.package_manager import PackageDescription
    from signalpilot._server.api.endpoints import dbt as _dbt_endpoints
    from signalpilot._server.api.router import build_routes
    from signalpilot._server.models import (
        completion,
        export,
        files,
        home,
    ...
    from signalpilot._snippets import snippets
    ...
    MODELS = [
        ...
        errors.SpInterruptionError,
        errors.SpInternalError,
        errors.SpAncestorStoppedError,
        errors.SpAncestorPreventedError,
        errors.SpStrictExecutionError,
        errors.SpExceptionRaisedError,
        errors.SpSyntaxError,
        errors.UnknownError,
        errors.Error,
        # Outputs
        CellOutput,
        ...
        # DBT request schemas
        _dbt_endpoints.DbtCommandRequest,
        _dbt_endpoints.DbtArtifactRequest,
    ]
...
@click.group(
    cls=ColoredGroup,
    help="""Various commands for the sp development.""",
    hidden=True,
)
...
@click.command(cls=ColoredCommand, help="""Print the sp OpenAPI schema""")
def openapi() -> None:
    """
    Example usage:

        sp development openapi
    """
    import yaml
...
@click.group(
    cls=ColoredGroup,
    help="Various commands for the sp processes",
    hidden=True,
)
def ps() -> None:
    pass
...
def get_signalpilot_processes() -> list[psutil.Process]:
    import psutil

    def is_sp_process(proc: psutil.Process) -> bool:
        if proc.name() == "sp":
            return True
    ...
    for proc in psutil.process_iter():
        if is_sp_process(proc):
            result.append(proc)

    return result


@ps.command(help="List the sp processes", name="list")
def list_processes() -> None:
    """
    Example usage:

        sp development ps list
    """
    from signalpilot._cli.print import orange

    # pretty print processes
    result = get_signalpilot_processes()
    for proc in result:
        cmds = proc.cmdline()
        cmd = " ".join(cmds[1:])
...
@ps.command(help="Kill the sp processes")
def killall() -> None:
    """
    Example usage:

        sp development ps killall
    """

    for proc in get_signalpilot_processes():
        # Ignore self
    ...
    click.echo("Killed all sp processes")
...
def inline_packages(name: Path) -> None:
    ...
    Example usage:
        ...
        sp development inline-packages
    ...
    from signalpilot._runtime.packages.module_name_to_pypi_name import (
...
def print_routes() -> None:
    ...
    from signalpilot._server.main import create_starlette_app
...
@click.command(cls=ColoredCommand, help="Preview a sp file as static HTML")
@click.argument(
    "file_path",
    required=True,
    type=click.Path(
...
def preview(file_path: Path, port: int, host: str, headless: bool) -> None:
    """
    Preview a sp file as static HTML.

    Creates a static HTML export of the sp file and serves it
    on a simple HTTP server for preview purposes.

    Example usage:
        sp development preview my_notebook.py
        sp development preview my_notebook.py --port 8000
    """
    import threading
    import webbrowser
    ...
    try:
        ...
        from signalpilot._server.export import run_app_until_completion
        ...
        static_root = sp_package_path() / "_static"
        ...
        raise SpCLIRuntimeError(f"Error creating preview: {e}") from e
