...
from signalpilot._server.workspace import flatten_files
...
def collect_notebooks(paths: Iterable[Path]) -> list[SpPath]:
    notebooks: dict[str, SpPath] = {}
    ...
    for path in paths:
        if path.is_dir():
            ...
            for file_info in flatten_files(file_infos):
                if not file_info.is_sp_file or file_info.is_directory:
                    continue
                absolute_path = str(Path(path) / file_info.path)
                notebooks[absolute_path] = SpPath(absolute_path)
        else:
            notebooks[str(path)] = SpPath(str(path))
...
class SandboxVenvPool:
    ...
    def close(self) -> None:
        from signalpilot._cli.sandbox import cleanup_sandbox_dir
