...
async def _export_session_snapshot(
    ...
    *,
    ...
    if venv_python is None:
        ...
        file_manager = load_notebook(sp_path.absolute_name)
        ...
        session_snapshot = serialize_session_snapshot(
            session_view,
            notebook_path=sp_path.absolute_name,
...
from signalpilot._cli.parse_args import parse_args
from signalpilot._server.export import run_app_until_completion
from signalpilot._server.export._session_cache import serialize_session_snapshot
from signalpilot._session.notebook import load_notebook
from signalpilot._utils.sp_path import SpPath
...
def _maybe_skip_fresh_snapshot(
    notebook: SpPath, *, force_overwrite: bool
) -> bool:
    output = get_session_cache_file(notebook.path)
...
@click.option(
    "--verbose/--no-verbose",
    default=False,
    help="Print cell errors and console output to stderr during execution.",
)
...
def session(
    ...
    from signalpilot._cli.sandbox import SandboxMode, run_in_sandbox
