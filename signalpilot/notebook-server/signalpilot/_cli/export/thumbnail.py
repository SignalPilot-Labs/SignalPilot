...
from signalpilot._cli.export._common import (
    SandboxVenvPool,
    collect_notebooks,
    is_multi_target,
...
from signalpilot._cli.print import echo, green, red, yellow
from signalpilot._dependencies.dependencies import DependencyManager
from signalpilot._server.export import run_app_then_export_as_html
from signalpilot._server.export._html_asset_server import HtmlAssetServer
from signalpilot._server.utils import asyncio_run
from signalpilot._utils.sp_path import SpPath
from signalpilot._utils.paths import sp_package_path, maybe_make_dirs
...
_sandbox_bootstrapped_env = "SP_THUMBNAIL_SANDBOX_BOOTSTRAPPED"
_sandbox_mode_env = "SP_THUMBNAIL_SANDBOX_MODE"
...
async def _render_html(
    ...
    *,
    ...
    if not execute:
        from signalpilot._server.export import export_as_html_without_execution

        result = await export_as_html_without_execution(
            sp_path, include_code=True, asset_url=asset_url
        )
        return result.text
...
async def _generate_thumbnails(
    *,
    ...
    from signalpilot._cli.sandbox import SandboxMode
    from signalpilot._metadata.opengraph import default_opengraph_image_abs

    failures: list[tuple[SpPath, Exception]] = []
    ...
    static_dir = sp_package_path() / "_static"
    ...
    try:
        with HtmlAssetServer(
            directory=static_dir, route="/__signalpilot_thumbnail__.html"
        ) as server:
...
@click.option(
    ...
    help=(
        ...
        "`<notebook_dir>/__sp__/assets/<notebook_stem>/opengraph.png`."
    ),
)
@click.option(
...
def thumbnail(
    ...
    if not notebooks:
        raise click.ClickException("No sp notebooks found.")
