...
from signalpilot._cli.print import bold, echo, green, muted
...
from signalpilot._utils.inline_script_metadata import (
    PyProjectReader,
    has_signalpilot_in_script_metadata,
    is_sp_dependency,
)
from signalpilot._utils.uv import find_uv_bin
from signalpilot._utils.versions import is_editable
from signalpilot._version import __version__


class SandboxMode(Enum):
    """Sandbox mode for sp notebooks.
...
def maybe_prompt_run_in_sandbox(name: str | None) -> bool:
    ...
    if DependencyManager.which("uv"):
        ...
        echo(
            bold(
                "This notebook has inlined package dependencies. \n"
                + "Consider installing uv so that sp can create a "
...
def _normalize_sandbox_dependencies(
    ...
    signalpilot_version: str,
    additional_features: list[DepFeatures],
) -> list[str]:
    """Normalize sp dependencies to have only one version.

    If multiple sp dependencies exist, prefer the one with brackets.
    ...
    def include_features(dep: str, features: list[DepFeatures]) -> str:
        ...
        if "[" in dep:
            return dep.replace("sp[", f"sp[{','.join(features)},")

        return dep.replace("sp", f"sp[{','.join(features)}]")

    # Find all sp dependencies
    signalpilot_deps = [d for d in dependencies if is_sp_dependency(d)]
    if not signalpilot_deps:
        if is_editable("sp"):
            LOGGER.info("Using editable of sp for sandbox")
            return dependencies + [f"-e {get_signalpilot_dir()}"]

        return dependencies + [
            include_features(f"sp=={signalpilot_version}", additional_features)
        ]

    # Prefer the one with brackets if it exists
    bracketed = next((d for d in signalpilot_deps if "[" in d), None)
    chosen = bracketed if bracketed else signalpilot_deps[0]

    # Remove all sp deps
    filtered = [d for d in dependencies if not is_sp_dependency(d)]

    if is_editable("sp"):
        LOGGER.info("Using editable of sp for sandbox")
        return filtered + [f"-e {get_signalpilot_dir()}"]

    # Add version if not already versioned
    if not _is_versioned(chosen):
        chosen = f"{chosen}=={signalpilot_version}"
...
def construct_uv_flags(
    ...
    # If there are no dependencies, which can happen for sp new or
    # on sp edit a_new_file.py, uv may use a cached venv, even though
    # we are passing --isolated; `--refresh` ensures that the venv is
    # actually ephemeral.
    uv_needs_refresh = not dependencies

    # Normalize sp dependencies
...
def construct_uv_command(
    ...
    # Final command assembly: combine the uv prefix with the original sp
    # command.
    return uv_cmd + cmd
...
def _ensure_python_version_in_script_metadata(name: str) -> None:
    ...
    from signalpilot._utils.scripts import read_pyproject_from_script
...
def _ensure_signalpilot_in_script_metadata(name: str | None) -> None:
    """Ensure sp is in the script metadata.

    If the file has no PEP 723 script metadata or sp is not listed
    as a dependency, add sp using uv.
    """
    # Only applicable to `.py` files.
    if name is None or not name.endswith(".py"):
        return

    # If the file doesn't exist or is empty, don't create it here - let sp
    # create the notebook normally with proper structure
    if not os.path.exists(name) or os.path.getsize(name) == 0:
        return

    # Check if script metadata exists and whether sp is present
    # Returns: True (has sp), False (no sp), None (no metadata)
    has_signalpilot = has_signalpilot_in_script_metadata(name)
    if has_signalpilot is True:
        # sp is already present
        return

    # Add sp to script metadata using uv
    # This will create the script metadata block if it doesn't exist
    try:
        result = subprocess.run(
            [find_uv_bin(), "add", "--script", name, "sp"],
            check=True,
            capture_output=True,
            text=True,
        ...
        LOGGER.info(f"Added sp to script metadata: {result.stdout}")
    except subprocess.CalledProcessError as e:
        LOGGER.warning(f"Failed to add sp to script metadata: {e.stderr}")
    except subprocess.TimeoutExpired:
        LOGGER.warning("Timed out adding sp to script metadata")
    except Exception as e:
        LOGGER.warning(f"Failed to add sp to script metadata: {e}")
...
def run_in_sandbox(
    ...
    *,
    ...
    """Run sp in a sandboxed uv environment.

    This wraps the sp command with `uv run` to create an isolated
    virtual environment with the notebook's dependencies.

    Used for "single" sandbox mode (sp edit --sandbox notebook.py).
    ...
    # Ensure sp and python version are in the script metadata before running
    _ensure_signalpilot_in_script_metadata(name)
    ...
    # Let the inner sp server poll for our PID so it can shut down if we
    # get SIGKILLed (signal handlers below can't catch uncatchable signals).
    env["SP_ANCESTOR_PID"] = str(os.getpid())
    if extra_env:
...
def get_sandbox_requirements(
    ...
    normalizes sp dependency, and adds any additional deps
...
def build_sandbox_venv(
    ...
    sandbox_dir = tempfile.mkdtemp(prefix="sp-sandbox-")
