...
from signalpilot._cli.install_hints import get_upgrade_commands
from signalpilot._cli.print import echo, green, orange
from signalpilot._config.cli_state import (
    SpCLIState,
    get_cli_state,
    write_cli_state,
)
...
from signalpilot._version import __version__ as current_version
...
def print_latest_version(current_version: str, state: SpCLIState) -> None:
    message = f"Update available {current_version} → {state.latest_version}"
    echo(orange(message))
    upgrade_commands = get_upgrade_commands("sp")
    if upgrade_commands:
...
def check_for_updates(
    on_update: Callable[[str, SpCLIState], None],
) -> None:
    return
...
def _update_with_latest_version(state: SpCLIState) -> SpCLIState:
    """
    ...
    if is_test:
        api_url = "https://pypi.org/pypi/signalpilot/json"
    else:
        api_url = "https://docs.signalpilot.ai/docs/"
