...
def check_app_correctness(filename: str, noninteractive: bool = True) -> None:
    ...
    except (SyntaxError, SpFileError):
    ...
    if status == "invalid" and filename.endswith(".py"):
        ...
        raise click.ClickException(
            f"Python script not recognized as a sp notebook.\n\n"
            f"  {green('Tip:')} Try converting with"
            "\n\n"
            f"    sp convert {filename} -o {stem}_nb.py\n\n"
            f"  then open with sp edit {stem}_nb.py"
        ) from None
    ...
    if status == "invalid" and interactive:
        ...
        click.confirm(
            (
                "The file is not detected as a sp notebook, opening it may "
