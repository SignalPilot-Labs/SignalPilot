"""Export sp notebooks to Jupyter ipynb format."""
...
from signalpilot._ast.cell import Cell, CellConfig
from signalpilot._ast.errors import CycleError, MultipleDefinitionError
from signalpilot._ast.names import is_internal_cell_name
from signalpilot._convert.common.format import get_markdown_from_cell
...
from signalpilot._messaging.mimetypes import METADATA_KEY
...
# and break reproducibility. The signalpilot_version in metadata is sufficient.
...
def _extract_markdown_prefix(code: str) -> str:
    """Extract the string prefix from a sp.md() call (e.g. '', 'r', 'f', 'fr')."""
...
def convert_from_ir_to_ipynb(
    ...
    *,
    ...
    DependencyManager.nbformat.require("to convert sp notebooks to ipynb")
    import nbformat  # type: ignore[import-not-found]

    from signalpilot import __version__

    notebook = nbformat.v4.new_notebook()  # type: ignore[no-untyped-call]
    notebook["cells"] = []

    # Add sp-specific notebook metadata
    signalpilot_metadata: dict[str, Any] = {
        "signalpilot_version": __version__,
    }
    app_config_diff = app.config.asdict_difference()
    if app_config_diff:
        signalpilot_metadata["app_config"] = app_config_diff
    # Include header if present (PEP 723 metadata, docstrings, etc.)
    if app._app._header:
        signalpilot_metadata["header"] = app._app._header
    notebook["metadata"]["sp"] = signalpilot_metadata
    ...
    for cell_data in cell_data_list:
        ...
        if session_view is not None:
            ...
            outputs = _convert_signalpilot_output_to_ipynb(
...
def _create_ipynb_cell(
    ...
    _add_signalpilot_metadata(node, name, config)
    return node


def _add_signalpilot_metadata(
    node: NotebookNode,
    name: str,
    config: CellConfig,
    md_prefix: str | None = None,
) -> None:
    """Add sp-specific metadata to a notebook cell."""
    signalpilot_metadata: dict[str, Any] = {}
    if config.is_different_from_default():
        signalpilot_metadata["config"] = config.asdict_without_defaults()
    if not is_internal_cell_name(name):
        signalpilot_metadata["name"] = name
    if md_prefix is not None:
        # Always store prefix for markdown cells so importer knows the original
        # prefix and can distinguish sp-created cells from foreign ipynb cells
        signalpilot_metadata["md_prefix"] = md_prefix
    if signalpilot_metadata:
        node["metadata"]["sp"] = signalpilot_metadata
...
def _is_signalpilot_component(html_content: Any) -> bool:
    """Check if the content is a sp component."""
    if isinstance(html_content, list):
        html_content = "".join(html_content)
    if not isinstance(html_content, str):
        return False
    return "<sp-" in html_content
...
def _strip_html_from_traceback(html_traceback: str) -> list[str]:
    ...
    # Strip temp file paths like /tmp/signalpilot_12345/__sp__cell_Hbol_.py
    # Replace with empty string to get cleaner tracebacks
    text = re.sub(r'[^"]*__sp__cell_[^"]*\.py', "", text)
...
def _extract_traceback_from_console(
    ...
    for output in console_outputs:
        if (
            ...
            and output.mimetype == "application/vnd.sp+traceback"
        ):
...
def _get_error_info(
    error: SpError | dict[str, Any],
) -> tuple[str, str]:
    """Extract ename and evalue from a sp error."""
    from signalpilot._messaging.msgspec_encoder import asdict

    if isinstance(error, dict):
        return error.get("type", "UnknownError"), error.get("msg", "")
    elif isinstance(error, SpExceptionRaisedError):
...
def _convert_output_to_ipynb(
    ...
    if output.mimetype == "application/vnd.sp+error":
        # Errors are handled separately via SP_ERROR channel
        return None
    elif output.mimetype == "application/vnd.sp+mimebundle":
        ...
        for mime, content in mimebundle.items():
            if mime == METADATA_KEY and isinstance(content, dict):
                metadata = content
            elif mime == "text/html" and _is_signalpilot_component(content):
                # Skip sp components because they cannot be rendered
        ...
        data[output.mimetype] = _convert_signalpilot_tex_to_latex(output.data)
    else:
...
def _clean_ansi_for_export(text: Any) -> str:
    ...
    used by logging libraries like rich, colorama, and sp's own logger.
...
def _convert_signalpilot_output_to_ipynb(
    cell_output: CellOutput | None, console_outputs: list[CellOutput]
) -> list[NotebookNode]:
    """Convert sp output format to IPython notebook format."""
    import nbformat
    ...
    for console_out in console_outputs:
        if console_out.channel == CellChannel.STDOUT:
            ...
            if console_out.mimetype == "application/vnd.sp+traceback":
                continue
            ipynb_outputs.append(
                cast(
                    nbformat.NotebookNode,
    ...
    if cell_output.channel == CellChannel.SP_ERROR:
        traceback_lines = _extract_traceback_from_console(console_outputs)
        errors = cast(list[SpError | dict[str, Any]], cell_output.data)
        for error in errors:
...
def _convert_signalpilot_tex_to_latex(html_string: str) -> str:
    """Convert sp-tex elements back to standard LaTeX delimiters.
    Keep in sync with TexPlugin.tsx

    Converts:
    - <sp-tex ...>||(content||)</sp-tex> → $content$ (inline)
    - <sp-tex ...>||[content||]</sp-tex> → $$content$$ (block)
    - <sp-tex ...>||(||(content||)||)</sp-tex> → $$content$$ (nested display)
    """
