...
from signalpilot._ast.compiler import compile_cell
from signalpilot._ast.names import DEFAULT_CELL_NAME
from signalpilot._ast.transformers import NameTransformer, RemoveImportTransformer
from signalpilot._ast.variables import is_local
from signalpilot._ast.visitor import Block, NamedNode, ScopedVisitor
from signalpilot._convert.common.format import (
    DEFAULT_MARKDOWN_PREFIX,
    markdown_to_signalpilot,
)
from signalpilot._convert.ipynb.from_ir import NBCONVERT_REMOVE_INPUT_TAG
...
def _strip_paragraph_tags(source: str) -> str:
    ...
    # Uses RE_NESTED_FENCE_START from pymdownx.superfences (already a sp
...
def transform_add_signalpilot_import(sources: list[CodeCell]) -> list[CodeCell]:
    """
    Add an import statement for sp if any cell uses
    the `sp.md` or `sp.sql` functions.
    """

    def contains_mo(cell: str) -> bool:
        return cell.startswith("sp.md(") or "sp.sql(" in cell

    def has_signalpilot_import(cell: str) -> bool:
        # Quick check
        if "import sp" not in cell:
            return False

        def is_in_import_line(line: str) -> bool:
            if line.startswith("import sp"):
                return True
            if line.startswith(("import ", "from ")):
                return "import sp" in line
            return False
    ...
    already_has_signalpilot_import = any(
        has_signalpilot_import(cell.source) for cell in sources
    )
    if already_has_signalpilot_import:
        return sources

    if any(contains_mo(cell.source) for cell in sources):
        return [CodeCell("import sp")] + sources
...
def transform_magic_commands(sources: list[str]) -> list[str]:
    """
    Transform Jupyter magic commands to their sp equivalents
    or comment them out.
    ...
    def magic_sql(source: str, command: str) -> str:
        """
        Transform SQL magic into sp SQL functions.
    ...
    def magic_remove(source: str, command: str) -> str:
        ...
        if not double:
            return "\n".join(
                [
                    "# magic command not supported in sp; please file an issue to add support",
                    f"# {command + ' ' + source}",
                ]
            )
    ...
    def magic_already_supported(source: str, command: str) -> str:
        ...
        double = command.startswith("%%")
        if not double:
            return f"# {(command + ' ' + source)!r} command supported automatically in sp"

        result = [
            f"# {command!r} command supported automatically in sp",
        ]
        if source:
            result.append(source)
        return "\n".join(result)
    ...
    def magic_mkdir(source: str, command: str) -> str:
        """
        Transform mkdir magic into sp mkdir functions.
    ...
    def magic_cd(source: str, command: str) -> str:
        """
        Transform cd magic into sp cd functions.
    ...
    def magic_html(source: str, command: str) -> str:
        """
        Transform html magic into sp html functions.

        %html <h1>Heading</h1>

        to

        sp.Html('<h1>Heading</h1>')
        """
        del command
        return f"sp.Html({source!r})"

    def magic_ls(source: str, command: str) -> str:
        """
        Transform ls magic into sp ls functions.
    ...
    def magic_bash(source: str, command: str) -> str:
        """
        Transform bash magic into sp bash functions.

        %bash echo "Hello, world!"

        to

        sp.bash('echo "Hello, world!"')
        """
        del command
    ...
    def magic_env(source: str, command: str) -> str:
        """
        Transform env magic into sp env functions
    ...
    magics: dict[str, Callable[[str, str], str]] = {
        ...
        "env": magic_env,
        # Already supported in sp, can just comment out the magic
...
def _extract_pip_install(
    ...
    replacement = (
        f"{_pass_if_indented(indent_level)}# packages added via sp's "
...
def _handle_exclamation_command(
    ...
    for i, token in enumerate(command_tokens):
        if token.startswith("pip"):
            # Pip installs always use sp's package management (never subprocess)
...
def transform_duplicate_definitions(sources: list[str]) -> list[str]:
    ...
    for cell_idx, source in enumerate(sources):
        ...
        try:
            ...
            # Cell contains constructs like `import *` that sp
...
def bind_cell_metadata(
    ...
    - If sp-specific metadata is present, it is used to restore cell config.
    """
    cells: list[CodeCell] = []
    for source, meta, hide_code in zip(
        ...
        # sp's ipynb exporter (and recognized by nbconvert / JupyterLab /
        ...
        # Extract sp-specific cell config if present
        signalpilot_meta = meta.get("sp", {})
        signalpilot_config = signalpilot_meta.get("config", {})
        name = signalpilot_meta.get("name", DEFAULT_CELL_NAME)

        # Determine hide_code with priority: sp config > tags > hide_flags default
        if "hide_code" in signalpilot_config:
            hide_code = signalpilot_config["hide_code"]
            ...
            # Cell was created by sp; sp would have stored hide_code=True
        ...
        cells.append(
            CodeCell(
                ...
                config=CellConfig(
                    hide_code=hide_code,
                    column=signalpilot_config.get("column"),
                    disabled=signalpilot_config.get("disabled", False),
                ),
            )
        )
    return cells
...
def build_metadata(
    ...
    merge dependencies.
    """
    from signalpilot._utils.inline_script_metadata import PyProjectReader
    from signalpilot._utils.scripts import write_pyproject_to_script
...
def _transform_sources(
    ...
    """
    from signalpilot._convert.common.comment_preserver import CommentPreserver
    ...
    _REPORT_URL = "https://docs.signalpilot.ai/docs/"
    ...
    cells = transform_add_signalpilot_import(cells)
...
def convert_from_ipynb_to_notebook_ir(
    ...
    # Extract notebook-level sp metadata if present
    notebook_metadata = notebook.get("metadata", {})
    signalpilot_nb_meta = notebook_metadata.get("sp", {})
    app_config = signalpilot_nb_meta.get("app_config", {})
    stored_header = signalpilot_nb_meta.get("header")
    ...
    for cell in notebook["cells"]:
        ...
        if is_markdown:
            ...
            md_prefix = cell_meta.get("sp", {}).get(
                "md_prefix", DEFAULT_MARKDOWN_PREFIX
            )
            source = markdown_to_signalpilot(source, prefix=md_prefix)
