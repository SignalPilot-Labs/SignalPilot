...
# Markdown is a dependency of sp, as such we utilize it as much as possible
# to parse markdown.
...
from signalpilot._convert.common.format import markdown_to_signalpilot, sql_to_signalpilot
...
def _is_code_tag(text: str) -> bool:
    ...
    if DependencyManager.new_superfences.has_required_version(quiet=True):
        supported_format = bool(re.search(r".*\{.*sp.*\}", head))
...
def formatted_code_block(
    ...
    """Wraps code in a fenced code block with sp attributes."""
...
def get_source_from_tag(tag: Element) -> str:
    ...
    if tag.tag == SP_MD:
        ...
        if not (source and source.strip()):
            return ""
        source = markdown_to_signalpilot(source)
    elif tag.attrib.get("language") == "sql":
        source = sql_to_signalpilot(
            source,
        ...
        assert tag.tag == SP_CODE, f"Unknown tag: {tag.tag}"
    return source
...
def _tree_to_ir(root: Element) -> SafeWrap[NotebookSerializationV1]:
    ...
    from signalpilot._utils.scripts import wrap_script_metadata
    ...
    for child in root:
        ...
        cell_config.append(
            get_cell_config_from_tag(child, hide_code=child.tag == SP_MD)
        )
...
class SpMdParser(IdentityParser):
    """Parses Markdown to sp notebook string."""
    ...
    def __init__(
        self,
        *args: Any,
        output_format: ConvertKeys = "sp-ir",
        **kwargs: Any,
    ) -> None:
        super().__init__(
...
class SanitizeParser(IdentityParser):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.preprocessors.register(preprocessor, "sp-sanitizer", 1)
...
class FrontMatterPreprocessor(Preprocessor):
    ...
    def __init__(self, md: SpMdParser):
        super().__init__(md)
        self.md: SpMdParser = md
...
class ExpandAndClassifyProcessor(BlockProcessor):
    ...
    def run(self, parent: Element, blocks: list[str]) -> None:
        ...
        assert isinstance(self.parser.md, SpMdParser)
        ...
        def add_paragraph() -> None:
            ...
            if text[-1].strip() == "":
                ...
                if not text:
                    return
            paragraph = SubElement(parent, SP_MD)
            paragraph.text = "\n".join(text).strip()
            text.clear()
        ...
        for line in "\n\n".join(blocks).split("\n"):
            ...
            # Check for the placeholder, and ensure it is a sp code block,
            ...
            code_block = SubElement(parent, SP_CODE)
...
def convert_from_md_to_signalpilot_ir(
    ...
    notebook = SpMdParser(output_format="sp-ir").convert(text)
