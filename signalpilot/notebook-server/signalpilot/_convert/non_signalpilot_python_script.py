...
from signalpilot._convert.ipynb.to_ir import convert_from_ipynb_to_notebook_ir
...
def convert_pypercent_script_to_notebook_ir(
    ...
    """Convert a pypercent Python script into sp notebook IR.

    Converts pypercent to jupyter to sp notebook IR using jupytext.
    """
    try:
...
def convert_python_block_to_notebook_ir(
    ...
    """Convert a Python script block to sp notebook IR.
...
def convert_script_block_to_notebook_ir(
    ...
    """Converts unknown script block to sp notebook IR.
...
def convert_non_signalpilot_python_script_to_notebook_ir(
    source: str,
) -> NotebookSerialization:
    """Convert a Python script that isn't in the notebook format into sp notebook IR.

    This should only be called after verifying the file is not already
    a valid sp notebook. It converts by:
...
def convert_non_signalpilot_script_to_notebook_ir(
    source: str,
) -> NotebookSerialization:
    """Convert a non-sp script to sp notebook IR.

    This is a convenience function that turns any string into a
    sp notebook.
    """
    try:
        return convert_non_signalpilot_python_script_to_notebook_ir(source)
    except ImportError:
        pass
    try:
        import ast
...
def _transform_main_blocks(ir: NotebookSerialization) -> None:
    """Transform if __name__ == "__main__": blocks in cells to sp-compatible functions."""
