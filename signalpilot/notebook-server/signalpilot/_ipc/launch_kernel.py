...
def main() -> None:
    """Launch a sp kernel using ZeroMQ for IPC.
    ...
    to sp's internal kernel.

    Typically, this entry point is invoked via the command line with:

        python -m sp._ipc.launch_kernel

    IMPORTANT: The module path "signalpilot._ipc.launch_kernel" is a public API
    used by external consumers (e.g., sp-lsp). Changing this path is a
