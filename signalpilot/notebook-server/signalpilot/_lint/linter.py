...
from signalpilot._ast.parse import SpFileError
from signalpilot._cli.print import red
from signalpilot._convert.converters import SpConvert
from signalpilot._lint.diagnostic import Diagnostic, Severity
from signalpilot._lint.formatters import LintResultJSON
from signalpilot._lint.rule_engine import EarlyStoppingConfig, RuleEngine
from signalpilot._loggers import capture_output
...
if TYPE_CHECKING:
    ...
    from signalpilot._config.config import LintConfig
...
class Linter:
    """High-level interface for linting and fixing sp files.
    ...
    def _rule_engine_for_file(self, file_path: str) -> RuleEngine:
        ...
        If the file contains PEP 723 ``[tool.sp.lint]`` metadata and no
        ...
        from signalpilot._config.manager import ScriptConfigManager
    ...
    async def _process_single_file(self, file: Path) -> FileStatus:
        ...
        try:
            ...
            if self.ignore_scripts:
                ...
                file_status.skipped = True
                file_status.message = (
                    f"Skipped: {file_path} (not a sp notebook)"
                )
                return file_status
            else:
                self.errored = True
                file_status.failed = True
                file_status.message = (
                    f"Not recognizable as a sp notebook: {file_path}"
                )
                file_status.details = [f"SpFileError: {e!s}"]
                return file_status
    ...
    def _generate_file_contents_from_notebook(
        ...
        converter = SpConvert.from_ir(notebook)
    ...
    async def fix(self, file_status: FileStatus) -> bool:
        ...
        if self.unsafe_fixes:
            ...
            from signalpilot._lint.rules.base import UnsafeFixRule
    ...
    def get_json_result(self) -> LintResultJSON:
        ...
        from signalpilot._lint.formatters import (
            FileErrorJSON,
            IssueJSON,
            JSONFormatter,
        )
