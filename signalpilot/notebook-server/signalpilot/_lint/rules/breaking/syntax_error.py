...
IMPORT_STAR_ERROR_MESSAGE = (
    "Importing symbols with `import *` is not allowed in sp."
)
IMPORT_STAR_HINT = (
    "Star imports are incompatible with sp's reactive execution. Use "
...
class SyntaxErrorRule(LintRule):
    ...
    Attempts to compile each cell using sp's internal compiler and catches any
    ...
    SyntaxErrors prevent sp from creating the dependency graph and running the
...
def _get_known_hints(message: str) -> str | None:
    if message == "'return' outside function":
        return (
            "sp cells are not normal Python functions; treat cell bodies "
