...
class BranchExpressionRule(LintRule):
    ...
    sp, only the last expression at the top level of a cell is displayed.
    ...
    - Users expect to see the result (like sp.md(), string literals, etc.)
    ...
    **Problematic:**
    ```python
    if condition:
        sp.md("Result A")  # Won't be displayed
    else:
        sp.md("Result B")  # Won't be displayed
    ```
    ...
    **Solution:**
    ```python
    # Assign to a variable that sp will display
    result = sp.md("Result A") if condition else sp.md("Result B")
    result
    ```
    ...
    **Alternative Solution (if no output intended):**
    ...
    - [Reactivity](https://docs.signalpilot.ai/docs/)
    """
    ...
    def _has_output_expression(self, stmts: list[ast.stmt]) -> bool:
        ...
        - SignalPilot output calls: sp.md(), sp.ui.button()

        Returns False for:
        - All function calls (assumed side effects): func(), obj.method()
        - Except sp output calls which ARE flagged
        """
        if not stmts:
            return False
        ...
        # For calls, only flag sp output calls
        if isinstance(expr, ast.Call):
            func = expr.func

            # SignalPilot output calls should be flagged (sp.md, sp.ui.*, etc.)
            return self._is_signalpilot_output_call(func)

        # All other expressions (constants, names, attributes, operations) are flagged
        return True

    def _is_signalpilot_output_call(self, func: ast.expr) -> bool:
        """Check if a call is a sp output function (sp.md, sp.ui.*, etc.).

        Excludes control flow and side-effect calls like sp.stop() and sp.output.*.
        """
        if isinstance(func, ast.Attribute):
            # Direct sp.* calls (sp.md, sp.Html, etc.)
            if isinstance(func.value, ast.Name) and func.value.id == "mo":
                # Exclude sp.stop()
                return func.attr != "stop"

            # sp.ui.* or other nested sp.* calls
            if isinstance(func.value, ast.Attribute) and (
                isinstance(func.value.value, ast.Name)
                and func.value.value.id == "mo"
            ):
                # Exclude sp.output.* calls (append, replace, clear)
    ...
    async def _report_diagnostic(
        ...
        message = (
            ...
            "sp can only display the last expression if it is unnested. "
