...
from signalpilot._ast.toplevel import HINT_ORDER_DEPENDENT, TopLevelExtraction
...
if TYPE_CHECKING:
    ...
    from signalpilot._ast.toplevel import TopLevelStatus
...
class ReusableDefinitionOrderRule(UnsafeFixRule):
    ...
    safely serialized as reusable due to the ordering of sp cells.


    ## What it does

    sp serializes reusable definitions in notebook order. Like all python
    scripts, a reusable function cannot refer to a variable that has _not yet
    been defined_. While ordering in sp normally doesn't matter, for reuse
    ...
    **Problematic:**
    ...
    def uses_offset(x: int = offset()) -> int:
        # This will run in sp, but will cause an error if run as a script!
    ...
    **Problematic:**
    ...
    ```bash
    sp check --fix --unsafe-fixes my_notebook.py
    ```
    ...
    - [setup](https://docs.signalpilot.ai/docs/)
    - [Reusing functions](https://docs.signalpilot.ai/docs/)
    """
