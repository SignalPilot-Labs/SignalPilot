...
from signalpilot._utils.paths import SP_DIR_NAME, notebook_output_dir
...
class OpenGraphMetadata(msgspec.Struct, rename="camel"):
    ...
    - a relative path (typically under `__sp__/`), or
...
def _normalize_opengraph_image(value: str | None) -> str | None:
    """Normalize an opengraph image value to a safe path (typically under `__sp__/`) or HTTPS URL."""
    if value is None:
        return None
    ...
    for node in module_ast.body:
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            ...
            if is_setup_expr(context_expr):
                ...
                # are available to the generator, without invoking sp's setup cell registration.
