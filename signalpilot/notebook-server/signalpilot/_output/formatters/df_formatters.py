...
from signalpilot import Html, _loggers
from signalpilot._messaging.mimetypes import KnownMimeType
from signalpilot._output.formatters.formatter_factory import (
    FormatterFactory,
    Unregister,
)
...
from signalpilot._plugins.ui._impl import tabs
from signalpilot._plugins.ui._impl.table import get_default_table_page_size, table
from signalpilot._runtime._wasm._duckdb import patch_duckdb_for_wasm
from signalpilot._runtime._wasm._polars import patch_polars_for_wasm
...
def include_opinionated() -> bool:
    ...
    if runtime_context_installed():
        ctx = get_context()
        return ctx.signalpilot_config["display"]["dataframes"] == "rich"
    return True
...
class PolarsFormatter(FormatterFactory):
    ...
    def register(self) -> Unregister | None:
        ...
        def _show_signalpilot_dataframe(
            df: pl.DataFrame,
        ...
        def _show_signalpilot_lazyframe(
            df: pl.LazyFrame,
        ) -> tuple[KnownMimeType, str]:
            return tabs.tabs(
                {
...
class DataFusionFormatter(FormatterFactory):
    ...
    def register(self) -> None:
        ...
        def _show_signalpilot_datafusion_dataframe(
...
class IbisFormatter(FormatterFactory):
    ...
    and displayed using an appropriate sp formatter.
    ...
    def register(self) -> None:
        ...
        def _format_ibis_expression(
            ...
            Interactive mode: Shows data as sp table
        ...
        def _show_signalpilot_ibis_table(
            table_expr: ir.Table,
        ) -> tuple[KnownMimeType, str]:
        ...
        def _show_signalpilot_ibis_column(
        ...
        def _show_scalar(scalar: ir.Scalar) -> tuple[KnownMimeType, str]:
            ...
            Complex scalars (arrays, maps, structs) render using sp's json_output.
