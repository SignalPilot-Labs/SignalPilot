...
from signalpilot._output.formatters.iframe import maybe_wrap_in_iframe
...
def _maybe_as_anywidget_html(
    ...
    into the same ``<sp-anywidget>`` HTML that ``sp.ui.anywidget()``
    ...
    from signalpilot._plugins.core.web_component import build_ui_plugin
    ...
    # Wrap in <sp-ui-element> so the plugin gets proper lifecycle
    # management (remount on re-run via random-id change).
    html = (
        f"<sp-ui-element object-id='{model_id}' "
...
def maybe_get_repr_formatter(
    ...
    reprs: list[tuple[str, KnownMimeType]] = [
        ...
        ("_repr_mimebundle_", "application/vnd.sp+mimebundle"),
