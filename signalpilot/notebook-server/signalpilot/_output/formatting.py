...
from signalpilot._output.formatters.repr_formatters import maybe_get_repr_formatter
...
from signalpilot._plugins.core.media import io_to_data_url
from signalpilot._plugins.stateless.json_output import json_output
from signalpilot._plugins.stateless.mime import mime_renderer
from signalpilot._plugins.stateless.plain_text import plain_text
from signalpilot._utils.methods import is_callable_method
...
def get_formatter(
    ...
    try:
        ...
        if FORMATTERS.is_empty():
            ...
            # Install formatters when sp is being used without
...
def plain(value: Any) -> Plain:
    ...
    This is the best way to opt out of sp's default dataframe rendering.
...
def iframe(html: str, *, width: str = "100%", height: str = "400px") -> Html:
    ...
    Scripts by default are not executed using `sp.as_html` or `sp.Html`,
    so if you have a script tag (written as `<script></script>`),
    you can use `sp.iframe` for scripts to be executed.
