...
if TYPE_CHECKING:
    ...
    from signalpilot._plugins.ui._impl.batch import batch as batch_plugin
...
class Html(MIME):
    ...
    def center(self) -> Html:
        ...
        Example:
            ```python3
            sp.md("# Hello, world").center()
            ```

        Returns:
            An `Html` object.
        """
        from signalpilot._plugins.stateless import flex
    ...
    def callout(
        ...
        Examples:
            ```python3
            sp.md("Hooray, you did it!").callout(kind="success")
            ```

            ```python3
            sp.md("It's dangerous to go alone!").callout(kind="warn")
            ```
        """

        from signalpilot._plugins.stateless.callout import callout as _callout
    ...
    def style(
        ...
        Example:
            ```python
            sp.md("...").style({"max-height": "300px", "overflow": "auto"})
            sp.md("...").style(max_height="300px", overflow="auto")
            ```
        ...
        """
        from signalpilot._plugins.stateless import style as _style
...
class _BlockWrapped(Html):
    ...
    inner wrapper uses ``display: contents`` (as ``sp.md`` does).

    The inner Html is re-rendered on every ``text`` access so mutable
    children (e.g. ``sp.status.spinner``) keep updating live.
    """
...
def patch_html_for_non_interactive_output() -> Iterator[None]:
    ...
    """
    # HACK: we must set SP_NO_JS since the rendering may happen in another
    # thread
    # This won't work when we are running a sp server and are auto-exporting
    # with this enabled.
    old_no_js = os.getenv(SP_NO_JS_KEY, "false")
    try:
        os.environ[SP_NO_JS_KEY] = "true"
        yield
    finally:
        os.environ[SP_NO_JS_KEY] = old_no_js
...
def is_non_interactive() -> bool:
    ...
    """
    return os.getenv(SP_NO_JS_KEY, "false").lower() == "true"
