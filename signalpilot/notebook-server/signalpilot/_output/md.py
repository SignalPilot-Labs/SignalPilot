...
from signalpilot._output.hypertext import Html, is_non_interactive
from signalpilot._output.md_extensions.breakless_lists import (
    BreaklessListsExtension,
)
from signalpilot._output.md_extensions.display_math import DisplayMathExtension
from signalpilot._output.md_extensions.external_links import ExternalLinksExtension
from signalpilot._output.md_extensions.flexible_indent import (
    FlexibleIndentExtension,
)
from signalpilot._output.md_extensions.iconify import IconifyExtension
...
def _get_extension_configs() -> dict[str, dict[str, Any]]:
    ...
    if is_pyodide():
        from signalpilot._runtime.runtime import notebook_dir
...
class _md(Html):
    ...
    def __format__(self, spec: str) -> str:
        ...
        blocks from being flattened when nested sp.md() calls are interpolated
        into other sp.md() calls.
...
def md(text: str) -> Html:
    ...
    **Interpolation.**
    ...
    For other objects, like plots, use sp's `as_html` method to embed
    them in markdown:
    ...
    plt.plot([1, 2])
    axis = plt.gca()
    md(f"Here's a plot: {sp.as_html(axis)}")
    ```
...
def latex(*, filename: str | Path) -> None:
    ...
    sp.latex(filename="https://example.com/macros.tex")
    ```
    ...
    from signalpilot._runtime import output
