...
from signalpilot._output.utils import create_style
...
class _FlexContainerHtml(Html):
    ...
    (e.g. sp.status.spinner) are re-rendered on every flush rather than
    ...
    def _build_text(self) -> str:
        def _item_style(idx: int) -> str | None:
            ...
            # flex: 1 and justify work. Leaf content (e.g. sp.stat) fills the
...
def vstack(
    ...
    *,
    ...
    Examples:
        ...
        ```python
        # Build a column of items
        sp.vstack([sp.md("..."), sp.ui.text_area()])
        ```
        ...
        sp.vstack(
            [
                sp.hstack([sp.md("..."), sp.ui.text_area()]),
                sp.hstack([sp.ui.checkbox(), sp.ui.text(), sp.ui.date()]),
            ]
        )
        ```
...
def hstack(
    ...
    *,
    ...
    Examples:
        ...
        ```python
        sp.hstack([sp.md("..."), sp.ui.text_area()], widths="equal")
        ```
        ...
        ```python
        sp.hstack(
            [sp.plain_text("..."), sp.ui.text_area(full_width=True)],
            widths=[0, 1],
        )
        ```
        ...
        sp.hstack(
            [
                sp.vstack([sp.md("..."), sp.ui.text_area()]),
                sp.vstack([sp.ui.checkbox(), sp.ui.text(), sp.ui.date()]),
            ]
        )
        ```
...
# Unused, but may be nice to keep around in case we want to add `sp.grid`
def _spaced(
    items: Sequence[object],
