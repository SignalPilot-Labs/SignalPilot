...
from signalpilot._output.utils import create_style, normalize_dimension
...
def _normalize_image(
    ...
    if (
        ...
        DependencyManager.pillow.require(
            "to render images from arrays in `sp.image`"
        )
        from PIL import Image as _Image

        if not hasattr(src, "__array_interface__"):
            DependencyManager.numpy.require(
                "to render images from generic arrays in `sp.image`"
            )
            import numpy
...
def image(
    ...
    Examples:
        ...
        sp.image(src="path/to/image.png")
        ```
        ...
        sp.hstack(
            [
                sp.image(dark, vmin=0, vmax=255),
                sp.image(light, vmin=0, vmax=255),
            ]
        )
        ```
