...
def new_figure_manager_given_figure(
    ...
    # so the warning is a false positive (especially in sp-run mode
...
def interactive(figure: Figure | SubFigure | Axes) -> Html:
    ...
    from signalpilot._plugins.ui._impl.from_mpl_interactive import mpl_interactive

    return mpl_interactive(figure)


# Custom CSS to make the mpl toolbar fit the sp UI
css_content = """
...
def patch_javascript(javascript: str) -> str:
    ...
    javascript = javascript.replace(
        " canvas.focus();",
        "// canvas.focus(); // don't steal focus when in sp",
    )
    javascript = javascript.replace(
        " canvas_div.focus();",
        "// canvas_div.focus(); // don't steal focus when in sp",
    )
    return javascript
