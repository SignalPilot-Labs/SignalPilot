...
import signalpilot._runtime.output._output as output
...
from signalpilot._utils.debounce import debounce
...
class Spinner(_Progress):
    ...
    def update(
        ...
        Examples:
            ```python
            with sp.status.spinner("Hang tight!") as _spinner:
                ...
...
class spinner:
    ...
    Use `sp.status.spinner()` as a context manager to show a loading spinner.
    You can optionally pass a title and subtitle.

    Examples:
        ```python
        with sp.status.spinner(subtitle="Loading data ...") as _spinner:
        ...
        ```python
        sp.status.spinner(title="Loading ...") if condition else sp.md("Done!")
        ```
...
class progress_bar(Generic[S]):
    ...
    Examples:
        ...
        ```python
        with sp.status.progress_bar(total=10) as bar:
            for i in range(10):
                ...
                bar.update()
        ```
...
def toast(
    ...
    Use `sp.status.toast()` to display a brief notification.
