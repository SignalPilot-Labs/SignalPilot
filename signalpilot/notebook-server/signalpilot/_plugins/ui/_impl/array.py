...
class array(UIElement[dict[str, JSONType], Sequence[object]]):
    ...
    (`array[index]`) and embedded in other sp outputs. You can also
    ...
    Examples:
        ...
        ```python
        array = sp.ui.array([sp.ui.slider(1, 10), sp.ui.text(), sp.ui.date()])
        ```
        ...
        ```python
        sp.md(f"This is a slider: array[0]")
        ```

        Some number of UI elements, determined at runtime:

        ```python
        sp.ui.array([sp.ui.slider(1, 10) for _ in range random.randint(4, 8)])
        ```
    ...
    def hstack(self, **kwargs: Any) -> Html:
        ...
        Args:
            **kwargs: Additional arguments passed to `sp.hstack`.
    ...
    def vstack(self, **kwargs: Any) -> Html:
        ...
        Args:
            **kwargs: Additional arguments passed to `sp.vstack`.
