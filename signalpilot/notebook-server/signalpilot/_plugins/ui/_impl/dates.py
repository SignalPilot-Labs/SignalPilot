...
class date(UIElement[str, dt.date]):
    ...
    Examples:
        ...
        date = sp.ui.date(value="2022-01-01")
        ```
        ...
        ```python
        date = sp.ui.date.from_series(df["column_name"])
        ```
...
class datetime(UIElement[str | None, dt.datetime | None]):
    ...
    Examples:
        ```python
        datetime_picker = sp.ui.datetime(
        ...
        ```python
        datetime_picker = sp.ui.datetime.from_series(df["datetime_column"])
        ```
...
class date_range(UIElement[tuple[str, str], tuple[dt.date, dt.date]]):
    ...
    Examples:
        ...
        ```python
        date_range = sp.ui.date_range.from_series(df["date_column"])
        ```
