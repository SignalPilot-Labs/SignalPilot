...
from signalpilot._data.series import (
    DataFrameSeries,
...
from signalpilot._plugins.ui._core.ui_element import S as JSONTypeBound, UIElement
from signalpilot._plugins.validators import (
    validate_between_range,
    validate_range,
    warn_js_safe_number,
)
from signalpilot._runtime.functions import Function
...
class number(UIElement[Numeric | None, Numeric | None]):
    ...
    Example:
        ```python
        number = sp.ui.number(start=1, stop=10, step=2)
        ```

        Or for integer-only values:

        ```python
        number = sp.ui.number(step=1)
        ```

        Or from a dataframe series:

        ```python
        number = sp.ui.number.from_series(df["column_name"])
        ```
...
class slider(UIElement[Numeric, Numeric]):
    ...
    Example:
        ```python
        slider = sp.ui.slider(start=1, stop=10, step=2)
        ```

        Or from a dataframe series:

        ```python
        slider = sp.ui.slider.from_series(df["column_name"])
        ```
        ...
        slider = sp.ui.slider(steps=steps)
        # log steps
        log_slider = sp.ui.slider(steps=np.logspace(0, 3, 4))
        # power steps
        power_slider = sp.ui.slider(steps=np.power([1, 2, 3], 2))
        ```
...
class range_slider(UIElement[list[Numeric], Sequence[Numeric]]):
    ...
    Example:
        ...
        ```python
        range_slider = sp.ui.range_slider.from_series(df["column_name"])
        ```
        ...
        steps = np.array([1, 2, 3, 4, 5])
        # linear steps
        range_slider = sp.ui.range_slider(steps=steps)
        # log steps
        log_range_slider = sp.ui.range_slider(steps=np.logspace(0, 3, 4))
        # power steps
        power_range_slider = sp.ui.range_slider(steps=np.power([1, 2, 3], 2))
        ```
    ...
    _name: Final[str] = "sp-range-slider"
    _mapping: dict[int, Numeric] | None = None
...
class radio(UIElement[str | None, Any]):
    ...
    Examples:
        ...
        Or from a dataframe series:
        ```python
        radiogroup = sp.ui.radio.from_series(df["column_name"])
        ```
...
class text(UIElement[str, str]):
    ...
    Examples:
        ```python
        text = sp.ui.text(value="Hello, World!")
        ```
...
class code_editor(UIElement[str, str]):
    ...
    _name: Final[str] = "sp-code-editor"
...
class dropdown(UIElement[list[str], Any]):
    ...
    Examples:
        ...
        Or from a dataframe series:
        ```python
        dropdown = sp.ui.dropdown.from_series(df["column_name"])
        ```
        ...
        *,
        ...
        if self._RESERVED_OPTION in options:
            raise ValueError(
                ...
                "is reserved by sp; please use another name."
            )
...
class multiselect(UIElement[list[str], list[object]]):
    ...
    Examples:
        ...
        Or from a dataframe series:
        ```python
        multiselect = sp.ui.multiselect.from_series(df["column_name"])
        ```
    ...
    _name: Final[str] = "sp-multiselect"
        ...
        *,
        ...
        if len(options) > multiselect._MAX_OPTIONS:
            raise ValueError(
                ...
                "using `sp.ui.text()` to let the user type an option name, "
                "and `sp.ui.table()` to present the options matching the "
                "user's query.",
            )
...
class button(UIElement[Any, Any]):
    ...
    def __init__(
        ...
        *,
        ...
        # This should be kept in sync with sp.ui.run_button()
        super().__init__(
...
class file(UIElement[list[tuple[str, str]], Sequence[FileUploadResults]]):
    ...
    Examples:
        ...
        ```python
        f = sp.ui.file(filetypes=[".png", ".jpg"], multiple=True)
        f
        ```
...
class form(UIElement[JSONTypeBound | None, T | None]):
    ...
    Examples:
        ...
        ''')
            .batch(
                name=sp.ui.text(label="name"),
                date=sp.ui.date(label="date"),
            )
        ...
        ```python
        # Instantiate a form directly
        form = sp.ui.form(element=sp.ui.slider(1, 100))
        ```
