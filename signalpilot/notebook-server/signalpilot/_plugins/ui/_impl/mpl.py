...
class EmptySelection:
    ...
    Returned by `sp.ui.matplotlib.value` when nothing is selected.
...
class matplotlib(UIElement[dict[str, JSONType], MatplotlibSelection]):
    ...
    Use `sp.ui.matplotlib` to make matplotlib plots interactive: draw a box
    ...
    name: Final[str] = "sp-matplotlib"

    def __init__(self, axes: Axes, *, debounce: bool = False) -> None:
        DependencyManager.matplotlib.require("for `sp.ui.matplotlib`")
        ...
        if x_scale not in _SUPPORTED_SCALES:
            raise ValueError(
                ...
                f"sp.ui.matplotlib supports: {', '.join(_SUPPORTED_SCALES)}."
            )
