...
from signalpilot._messaging.msgspec_encoder import encode_json_str
from signalpilot._pyodide.restartable_task import RestartableTask
from signalpilot._pyodide.streams import (
    PyodideStderr,
    PyodideStdin,
    PyodideStdout,
    PyodideStream,
)
from signalpilot._runtime import commands, handlers, patches
...
from signalpilot._snippets.snippets import read_snippets
...
class PyodideBridge:
    ...
    def save_user_config(self, request: str) -> None:
        ...
        config = merge_default_config(cast(PartialSignalPilotConfig, parsed.config))
...
def _launch_pyodide_kernel(
    ...
    debugger = SpPdb(stdout=stdout, stdin=stdin) if is_edit_mode else None
