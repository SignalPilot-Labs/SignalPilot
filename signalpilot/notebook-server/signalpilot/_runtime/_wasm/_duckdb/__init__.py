...
* **Direct read methods** — ``duckdb.read_csv/read_parquet/read_json`` are
  ...
  sp and returned as DuckDB relations.
* **SQL remote scans** — raw DuckDB APIs and sp's ``sp.sql`` path call the
...
from signalpilot._runtime._wasm._duckdb.io import (
    RemoteFileSource,
    remote_file_source_from_reader_args,
)
from signalpilot._runtime._wasm._duckdb.sources import (
    remote_file_source_from_table,
)
from signalpilot._runtime._wasm._patches import (
    Unpatch,
    WasmPatchSet,
    WrapperFactory,
)
...
_EVAL_BINDING_NAME_BASES = _EvalBindingNames(
    original="__signalpilot_wasm_duckdb_original",
    args="__signalpilot_wasm_duckdb_args",
    kwargs="__signalpilot_wasm_duckdb_kwargs",
)
...
class _RemoteTableNames:
    ...
    def name_for(self, source: RemoteFileSource) -> str:
        ...
        while True:
            candidate = f"__signalpilot_wasm_duckdb_remote_{idx}"
...
def patch_duckdb_query_for_wasm(
    ...
    *,
    ...
    becomes ``SELECT * FROM __signalpilot_wasm_duckdb_remote_0`` when suffix ``0``
