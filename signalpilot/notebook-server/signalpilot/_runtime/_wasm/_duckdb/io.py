...
from signalpilot._runtime._wasm import _fetch
from signalpilot._runtime._wasm._duckdb.dataframe import (
...
def remote_file_from_url(url: str) -> RemoteFile | None:
    """Return a fetchable remote file only for URL schemes sp supports."""
...
def reader_for_function(
    ...
    """Resolve DuckDB table-function names to sp's fallback readers."""
    return next(
        (
            reader
            for reader in _READERS
...
def _apply_common_table_option(
    ...
    """Handle options sp applies after per-file reads are decoded."""
