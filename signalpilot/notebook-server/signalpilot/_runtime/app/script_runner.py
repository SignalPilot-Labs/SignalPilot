...
from signalpilot._runtime.app.common import RunOutput
...
class AppScriptRunner:
            ...
            while self.cells_to_run:
                ...
                with get_context().with_cell_id(cid):
                    try:
                        ...
                        if isinstance(unwrapped_exception, SpStopError):
                            self._cancel(cid)
                        else:
                            raise
                    finally:
    ...
    def run(self) -> RunOutput:
        from signalpilot._runtime.context.script_context import (
        ...
        for cell in app.cell_manager.cells():
            if cell is None:
                raise RuntimeError(
                    "Unparsable cell encountered. This is a bug in sp, "
        ...
        try:
            ...
            # script
            from signalpilot._output.formatters.formatters import (
                register_formatters,
            )
            from signalpilot._output.formatting import FORMATTERS
            ...
            if DependencyManager.matplotlib.has():
                from signalpilot._output.mpl import close_figures
