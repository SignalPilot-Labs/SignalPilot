...
from signalpilot._data.models import DataTableSource
from signalpilot._messaging.notebook.document import NotebookCell
from signalpilot._types.encodable import Encodable
from signalpilot._types.ids import CellId_t, RequestId, UIElementId, WidgetModelId
...
def _meta_to_dict(meta: Any) -> dict[str, Encodable]:
    ...
    Trusted shape per the public contract (``sp.app_meta().request``); we
...
class HTTPRequest(Mapping[str, Any]):
    ...
    Attributes:
        ...
        headers: Request headers (sp-specific headers excluded).
    ...
    def from_request(request: HTTPConnection) -> HTTPRequest:
        ...
        # Convert headers to dict, remove all sp-specific headers
        headers: dict[str, str] = {}
        for k, v in request.headers.items():
            if not k.startswith(("sp", "x-sp")):
                headers[k] = v
...
class UpdateUserConfigCommand(Command):
    ...
    Updates global sp configuration (runtime settings, display options, editor preferences).
