...
from signalpilot._messaging.completion_option import CompletionOption
from signalpilot._messaging.notification import CompletionResultNotification
...
from signalpilot._output.md import _md
from signalpilot._runtime import dataflow
from signalpilot._runtime.commands import CodeCompletionCommand
from signalpilot._session.queue import QueueType
from signalpilot._types.ids import RequestId
from signalpilot._utils.docs import SpConverter
from signalpilot._utils.format_signature import format_signature
from signalpilot._utils.rst_to_html import convert_rst_to_html
...
def _convert_docstring_to_markdown(raw_docstring: str) -> str:
    ...
    # which uses our custom SpConverter
    ...
    # Then try our custom SpConverter
    ...
    # the same TeX pipeline used elsewhere in sp output.
...
def _key_options_dispatcher(obj: Any) -> list[str]:
    ...
    Returns a list of strings to later create sp `CompletionOption`
    """
...
def _resolve_chained_key_path(obj_name: str, document: str) -> list[list[str]]:
    ...
    # because sp `CompletionRequest` sends source code up to the cursor position
...
def complete(
    ...
    Args:
        ...
        graph: Dataflow graph backing the sp program
    ...
    try:
        ...
        if (
            ...
            # for example, if the document is "sp.ui.slider(start=1,
            signatures = script.get_signatures()
            if signatures:
                _write_completion_result(
                    stream=stream,
