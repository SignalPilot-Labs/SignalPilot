...
from signalpilot._runtime.packages.utils import split_packages
from signalpilot._utils.platform import is_pyodide
from signalpilot._utils.subprocess import safe_popen
from signalpilot._utils.uv import find_uv_bin
from signalpilot._utils.uv_tree import DependencyTreeNode, parse_uv_tree
from signalpilot._utils.versions import (
    extract_extras,
...
class UvPackageManager(PypiPackageManager):
    ...
    def install_command(
        ...
        if self.is_in_uv_project:
            ...
            target = os.environ.get("SP_UV_TARGET", None)
            if target:
    ...
    def update_notebook_script_metadata(
        ...
        *,
        ...
        def _maybe_add_version(package: str) -> str:
            # Skip sp and sp[<mod>], but not sp-<something-else>
            if package == "sp" or package.startswith("sp["):
                return package
    ...
    def _process_md_changes(
        ...
        header = yaml.signalpilot_compat_dump(
            frontmatter,
            sort_keys=False,
        )
    ...
    def is_in_uv_project(self) -> bool:
        """Determine if we are currently running sp from a uv project
        ...
        we are in a temporary virtual environment (e.g. `uvx sp edit` or `uv --with=sp run sp edit`)
        ...
        # This tells us that sp was run by uv
