...
from signalpilot._runtime import sp_browser, sp_pdb
...
def patch_pdb(debugger: sp_pdb.SpPdb) -> None:
    import pdb

    # Patch Pdb so manually instantiated debuggers create our debugger
    pdb.Pdb = sp_pdb.SpPdb  # type: ignore[misc, assignment]
    pdb.set_trace = functools.partial(sp_pdb.set_trace, debugger=debugger)
...
def patch_webbrowser() -> None:
    ...
    # (nor a webbrowser.Error, so careful)
        ...
        webbrowser.open = sp_browser.browser_open_fallback
    except webbrowser.Error:
        SpBrowser = sp_browser.build_browser_fallback()
        webbrowser.register(
            "sp-output",
            None,
            SpBrowser(),
...
"""
    ...
    from signalpilot._output.formatters.formatters import patch_finder
...
def create_main_module(
    ...
    _module = types.ModuleType(
        "__main__",
        doc=doc if doc is not None else "Created for the sp kernel.",
    )
...
def patch_jedi_parameter_completion() -> None:
    ...
    def extract_signalpilot_style_arguments(docstring: str) -> dict[str, str]:
        lines = docstring.splitlines()
        try:
            start = lines.index("# Arguments")
        except ValueError:
            return {}
        parsing_signalpilot_docstring = False
        param_descriptions: dict[str, str] = {}
        for line in lines[start + 1 :]:
            if line == "| Parameter | Type | Description |":
                parsing_signalpilot_docstring = True
                continue
            if line == "|-----------|------|-------------|":
                continue
            if parsing_signalpilot_docstring:
    ...
    def py__doc__(self: ParamNameWrapper) -> str:
        ...
        if "# Arguments" in docstring:
            param_descriptions = extract_signalpilot_style_arguments(docstring)
        else:
