...
UNCLONABLE_TYPES = [
    "signalpilot._runtime.state.State",
    "signalpilot._runtime.state.SetFunctor",
    "signalpilot._runtime.watch._file.FileState",
    "signalpilot._runtime.watch._directory.DirectoryState",
]
...
UNCLONABLE_MODULES = {
    ...
    "_io",
    "signalpilot._ast",
    "signalpilot._plugins.ui",
    "numpy.lib.npyio",
}


def is_external(value: Any) -> bool:
    return "__sp__cell_" not in inspect.getfile(value)
...
def is_pure_function(
    ...
    # sp notebook as an external module, and we should still check for
    # 'purity'.
    is_sp_lib = False
    if graph is not None:
        ...
        is_sp_lib = ref in extraction.variables
    if is_lib and not is_sp_lib:
        return True
