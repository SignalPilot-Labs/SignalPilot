...
from signalpilot._ast.pytest import SP_TEST_STUB_NAME
from signalpilot._cli.print import bold, green
from signalpilot._dependencies.dependencies import DependencyManager
from signalpilot._runtime.capture import capture_stdout
from signalpilot._runtime.context import safe_get_context
from signalpilot._runtime.runtime import notebook_location

SP_TEST_BLOCK_REGEX = re.compile(rf"{SP_TEST_STUB_NAME}_\d+[(?::)\.]+")
...
def _to_signalpilot_uri(uri: str) -> str:
    """Convert a file path URI to a sp URI if it matches the cell pattern."""
    # Should be like
    # /tmp/signalpilot_1234567/__sp__cell_1234_.py
    if "__sp__cell_" not in uri:
        return uri
    cell_id = uri.split("_")[6]

    notebook = os.path.relpath(_get_name(), notebook_location())
    return f"sp://{notebook}#cell_id={cell_id}"
...
class ReplaceStubPlugin:
    ...
    def pytest_runtest_logreport(
        ...
        """
        report.nodeid = SP_TEST_BLOCK_REGEX.sub("", report.nodeid)
        if report.location is not None:
            fspath, lineno, domain = report.location
            report.location = (
                fspath,
                lineno,
                SP_TEST_BLOCK_REGEX.sub("", domain),
            )
        ...
        # Not all TerminalRepr seem to have a reprtraceback
            ...
            for entry in report.longrepr.reprtraceback.reprentries:
                if entry.reprfileloc is not None:
                    entry.reprfileloc.path = _to_signalpilot_uri(
