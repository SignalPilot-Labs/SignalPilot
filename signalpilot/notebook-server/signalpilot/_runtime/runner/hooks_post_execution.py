...
from signalpilot._ast.cell import CellImpl
from signalpilot._ast.toplevel import TopLevelExtraction
from signalpilot._data._external_storage.get_storage import (
    get_storage_backends_from_variables,
    storage_backend_to_storage_namespace,
)
from signalpilot._data.get_datasets import (
...
from signalpilot._runtime.runner import cell_runner
from signalpilot._runtime.runner.hook_context import PostExecutionHookContext
from signalpilot._runtime.runner.hooks import PostExecutionHook
from signalpilot._runtime.side_effect import SideEffect
from signalpilot._sql.engines.duckdb import (
...
from signalpilot._utils.flatten import contains_instance
...
def _set_run_result_status(
    ...
    if isinstance(run_result.exception, SpInterruptionError):
...
def _broadcast_outputs(
    ...
    if (
        run_result.success()
        or isinstance(run_result.exception, SpStopError)
    ...
    elif isinstance(run_result.exception, SpStrictExecutionError):
    ...
    elif isinstance(run_result.exception, SpInterrupt):
    ...
    elif isinstance(run_result.exception, SpExceptionRaisedError):
        CellNotificationUtils.broadcast_error(
            data=[run_result.exception],
            clear_console=False,
            cell_id=cell.cell_id,
        )
    elif isinstance(run_result.exception, SpSQLError):
...
def attempt_pytest(
    ...
    if cell._test:
        try:
            import signalpilot._runtime.pytest as signalpilot_pytest

            if ctx.execution_context is not None:
                with ctx.execution_context(cell.cell_id):
                    result = signalpilot_pytest.run_pytest(cell.defs, ctx.glbls)
                    if result.output:
...
def _reset_matplotlib_context(
    ...
    if get_global_context().mpl_installed:
        ...
        exec("__sp__._output.mpl.close_figures()", ctx.glbls)
...
_gateway_connections_broadcasted = False


@kernel_tracer.start_as_current_span("broadcast_gateway_connections")
def broadcast_gateway_connections(
    cell: CellImpl,
    ctx: PostExecutionHookContext,
    run_result: cell_runner.RunResult,
) -> None:
    ...
    try:
        ...
        LOGGER.debug("Failed to broadcast gateway connections", exc_info=True)
