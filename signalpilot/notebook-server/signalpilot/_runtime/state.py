...
def state(
    ...
    paths. **In almost all cases, you should prefer using sp's built-in
    [reactive execution](https://docs.signalpilot.ai/docs/) and
    [interactivity](https://docs.signalpilot.ai/docs/).**
    ...
    prefer using sp's built-in [reactive execution for interactive
    elements](https://docs.signalpilot.ai/docs/).
    ...
    Examples:
      Create state:
      ```python
      get_count, set_count = sp.state(0)
      ```
      Read the value:
      ```python
      get_count()
    ...
    **Synchronizing multiple UI elements:**
      ...
      slider = sp.ui.slider(0, 100, value=get_state(), on_change=set_state)
      ```
      ```python
      # Updating the state through the number will recreate the slider (above)
      number = sp.ui.number(0, 100, value=get_state(), on_change=set_state)
      ```
      ```python
    ...
    Warning:
      Do not store `sp.ui` elements in state; doing so can cause
