...
from signalpilot._runtime.watch._path import (
...
def directory(path: Path | str) -> DirectoryState:
    ...
    Note:
        ...
        the directory structure. Utilize `sp.watch.file()` to watch for changes
    ...
    if isinstance(path, str):
        path = Path(path)
    if not path.is_dir():
        raise ValueError("Path must be a directory, use sp.file()")
