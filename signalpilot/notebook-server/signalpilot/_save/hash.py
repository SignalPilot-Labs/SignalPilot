...
from signalpilot._ast.transformers import DeprivateVisitor, get_hashable_ast
from signalpilot._ast.variables import (
    get_cell_from_local,
    if_local_then_mangle,
    unmangle_local,
)
from signalpilot._ast.visitor import ImportData, Name, ScopedVisitor
...
from signalpilot._runtime.dataflow import induced_subgraph
from signalpilot._runtime.dataflow.topology import GraphTopology
from signalpilot._runtime.primitives import (
    CLONE_PRIMITIVES,
    FN_CACHE_TYPE,
...
from signalpilot._runtime.side_effect import CellHash, SideEffect
from signalpilot._runtime.state import SetFunctor, State
from signalpilot._runtime.watch._path import PathState
from signalpilot._save.cache import Cache, CacheType, HashMemoCleanup
from signalpilot._save.encode import (
...
from signalpilot._save.stubs import maybe_get_custom_stub
...
if TYPE_CHECKING:
    ...
    from signalpilot._runtime.context.types import RuntimeContext
    from signalpilot._runtime.dataflow import DirectedGraph
    from signalpilot._save.loaders import Loader
...
class BlockHasher:
    def __init__(
        ...
        *,
        ...
        utilizing builtin functions like `sp.watch.file`, but the onus is put on the user currently
        ...
        if refs:
            ...
            assert not children.get(self.cell_id, None), (
                ...
                "You may have a cycle which sp should have previously "
                "detected."
            )
    ...
    def serialize_and_dequeue_content_refs(
        ...
        for local_ref in sorted(refs):
            ...
            # And we assume all sp cases are caught
            elif not inspect.isclass(
                value
            ) and not value.__module__.startswith("sp"):
                continue
            ...
            if cell_id in ctx.cell_lifecycle_registry.registry:
                ...
                if cell_hash:
                    assert len(cell_hash) == 1, (
                        ...
                        "Please report this issue to signalpilot-team/signalpilot"
                    )
