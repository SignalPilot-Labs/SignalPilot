...
from signalpilot._save.loaders.loader import BasePersistenceLoader
from signalpilot._save.stores import FileStore, Store
from signalpilot._save.stubs import (
    FunctionStub,
    ModuleStub,
)
from signalpilot._save.stubs.lazy_stub import (
    _LAZY_STUB_CACHE,
    BLOB_DESERIALIZERS,
    BLOB_SERIALIZERS,
    LAZY_STUB_LOOKUP,
...
from signalpilot._save.stubs.stubs import mro_lookup
...
class LazyLoader(BasePersistenceLoader):
    ...
    def restore_cache(self, _key: HashKey, blob: bytes) -> Cache:
        ...
        return Cache(
            ...
            defs=defs,
            meta={
                "version": cache_data.meta.version or SP_CACHE_VERSION,
                "return": return_item,
    ...
    def save_cache(self, cache: Cache) -> bool:
        ...
        manifest = msgspec.json.encode(
            CacheSchema(
                ...
                defs=defs_dict,
                meta=Meta(
                    version=cache.meta.get("version", SP_CACHE_VERSION),
