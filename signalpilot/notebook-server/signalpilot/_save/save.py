...
from signalpilot._ast.cell_id import is_external_cell_id
from signalpilot._ast.load import find_cell
from signalpilot._ast.transformers import (
    ARG_PREFIX,
    CacheExtractWithBlock,
    get_hashable_ast,
)
from signalpilot._ast.variables import is_mangled_local, unmangle_local
from signalpilot._messaging.tracebacks import write_traceback
from signalpilot._runtime.context import get_context, safe_get_context
from signalpilot._runtime.side_effect import SideEffect
from signalpilot._runtime.state import State
from signalpilot._save.cache import (
    UNEXPECTED_FAILURE_BOILERPLATE,
    Cache,
    CacheContext,
    CacheException,
)
from signalpilot._save.hash import (
    DEFAULT_HASH,
    BlockHasher,
    cache_attempt_from_hash,
    content_cache_attempt_from_base,
)
from signalpilot._save.loaders import (
    PERSISTENT_LOADERS,
    Loader,
    LoaderKey,
    LoaderPartial,
...
from signalpilot._save.stores.file import FileStore
from signalpilot._save.toplevel import get_cell_id_from_scope, graph_from_scope
from signalpilot._types.ids import CellId_t
from signalpilot._utils.with_skip import SkipContext
...
if TYPE_CHECKING:
    ...
    from signalpilot._save.stores import Store
...
class _cache_call_async(_cache_call[P, R]):
    ...
    # that concurrent sessions (each with their own loop, as in sp run)
    ...
    async def __call__(  # type: ignore[override]
        ...
        # (each with their own loop, as in sp run) never share tasks.
        existing_task = None
...
def cache(  # type: ignore[misc]
    ...
    **kwargs: Any,
    ...
    Decorating a function with `@sp.cache` will cache its value based on
    ...
    `sp.cache` is similar to `functools.cache`, but with three key benefits:

    1. `sp.cache` persists its cache even if the cell defining the cached
    ...
    2. `sp.cache` keys on closed-over values in addition to function arguments,
        preventing accumulation of hidden state associated with
        `functools.cache`.
    3. `sp.cache` does not require its arguments to be
        hashable (only pickleable), meaning it can work with lists, sets, NumPy
        arrays, PyTorch tensors, and more.

    `sp.cache` obtains these benefits at the cost of slightly higher overhead
    than `functools.cache`, so it is best used for expensive functions.

    Like `functools.cache`, `sp.cache` is thread-safe.

    The cache has an unlimited maximum size. To limit the cache size, use
    `@sp.lru_cache`. `sp.cache` is slightly faster than `sp.lru_cache`, but in
    most applications the difference is negligible.

    Note, `sp.cache` can also be used as a drop in replacement for context block
    caching like `sp.persistent_cache`.
    ...
    The `sp.cache` context manager lets you delimit a block of code in which
    ...
    runs, for that functionality, refer to `sp.persistent_cache`.

    Examples:
    ```python
    with sp.cache("my_cache") as cache:
...
def lru_cache(  # type: ignore[misc]
    ...
    **kwargs: Any,
    ...
    `sp.lru_cache` is a version of `sp.cache` with a bounded cache size. As an
    ...
    see the documentation of `sp.cache`.
    ...
    if {"loader"} & set(kwargs.keys()):
        raise ValueError(
            ...
            "for lru_cache, use sp.cache instead."
        )
...
def persistent_cache(  # type: ignore[misc]
    ...
    **kwargs: Any,
    ...
    The `sp.persistent_cache` context manager lets you delimit a block of code
    in which variables will be cached to disk when they are first computed. On
    subsequent runs of the cell, if sp determines that this block of code
    ...
    `sp.persistent_cache` to make notebooks start *instantly*, with variables
    ...
    Note that `sp.state` and `UIElement` changes will also trigger cache
    ...
    **Warning.** Since context abuses sys frame trace, this may conflict with
    ...
    for `@sp.cache` or `@sp.lru_cache`. This is much slower than cache, but
    can be useful for saving function values between kernel restarts. For more
    details, refer to `sp.cache`.
    ...
    **Usage.**
    ...
    @sp.persistent_cache(save_path="my/path/to/cache")
    ...
    Args:
        ...
        save_path: the folder in which to save the cache, defaults to
            `__sp__/cache` in the directory of the notebook file
    ...
    if {"loader"} & set(kwargs.keys()):
        raise ValueError(
            "loader is not a valid argument "
            "for persistent_cache, use sp.cache instead."
        )
