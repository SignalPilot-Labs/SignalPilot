...
class CellRuntimeMetadata:
    # String form of the runtime state (see sp._ast.cell.RuntimeStateType);
...
class GetCellRuntimeDataData:
    ...
    cell_id: str
    code: str | None = None
    errors: list[SpErrorDetail] | None = None
...
class CellOutputData:
    ...
    console_outputs: SpCellConsoleOutputs = field(
        default_factory=SpCellConsoleOutputs
    )
...
class GetLightweightCellMap(
        ...
        for cell_data in cell_manager.cell_data():
            ...
            if cell_notif is not None:
                ...
                has_errors = (
                    ...
                    and cell_notif.output.channel == CellChannel.SP_ERROR
                )
                has_output = (
                    not has_errors
    ...
    def _is_markdown_cell(self, code: str) -> bool:
        return code.lstrip().startswith("sp.md(")
