...
from signalpilot._server.config import StarletteServerState
...
class AppStateBase:
    ...
    @property
    def config_manager(self) -> SpConfigManager:
...
class AppState(AppStateBase):
    ...
    def get_current_session_id(self) -> SessionId | None:
        ...
        session_id = self.request.headers.get("Sp-Session-Id")
    ...
    def require_current_session_id(self) -> SessionId:
        ...
        if session_id is None:
            raise ValueError("Missing Sp-Session-Id header")
        return session_id
    ...
    # Config manager for the sp file that we are running.
    # This could have custom config in the script metadata.
    @property
    def app_config_manager(self) -> SpConfigManager:
    ...
    def config_manager_at_file(self, path: str) -> SpConfigManager:
