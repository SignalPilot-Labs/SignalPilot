...
# Root-level health checks (mounted without prefix in router.py)
root_health_router = APIRouter()
root_health_router.add_route("/health", health_check, methods=["GET"])
root_health_router.add_route("/healthz", health_check, methods=["GET"])
    ...
    return JSONResponse(
        {
            ...
            # sp kernel (for the given session)
            "kernel": {
