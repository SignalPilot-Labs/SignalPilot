...
from signalpilot._config.cli_state import SpCLIState
...
from signalpilot._server.api.endpoints.ws.ws_formatter import (
    serialize_notification_for_websocket,
)
from signalpilot._server.api.endpoints.ws.ws_kernel_ready import (
    build_kernel_ready,
    is_rtc_available,
)
...
from signalpilot._session.managers.ipc import KernelStartupError
from signalpilot._session.model import (
    ConnectionState,
    SessionMode,
)
from signalpilot._types.ids import CellId_t, ConsumerId
...
async def websocket_endpoint(
    ...
    """Main WebSocket endpoint for sp sessions.
...
async def ws_sync(
    ...
    if not (LORO_ALLOWED and DependencyManager.loro.has()):
        ...
        await websocket.close(
            WebSocketCodes.NORMAL_CLOSE, "SP_LORO_NOT_INSTALLED"
        )
        return
    ...
    if session is None:
        ...
        await websocket.close(WebSocketCodes.FORBIDDEN, "SP_NOT_ALLOWED")
        return
...
class WebSocketHandler(SessionConsumer):
    ...
    def _connect_kiosk(self, session: Session) -> None:
        ...
        sp kiosk/frontend to visualize the output.
        """
    ...
    async def start(self) -> None:
        ...
        try:
            # Run in a thread — connect() calls create_session() which spawns
            # a kernel subprocess. Process.start() blocks for ~3s and would
            # freeze the entire async event loop, queuing all other HTTP
            # requests behind it.
    ...
    def _check_status_update(self) -> None:
        ...
        def on_update(current_version: str, state: SpCLIState) -> None:
            # Let's only toast once per sp server
            ...
            release_url = "https://docs.signalpilot.ai/docs/"
