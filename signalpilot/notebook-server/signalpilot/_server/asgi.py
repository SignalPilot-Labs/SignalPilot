...
_DEFAULT_ALLOW_ORIGINS: tuple[str, ...] = ("localhost", "127.0.0.1")


def _parse_allow_origins_env(value: str | None) -> tuple[str, ...]:
    if not value:
        return _DEFAULT_ALLOW_ORIGINS
    origins = tuple(part.strip() for part in value.split(",") if part.strip())
    return origins if origins else _DEFAULT_ALLOW_ORIGINS


def _parse_allow_credentials_env(value: str | None) -> bool:
    ...
    if normalized == "false":
        return False
    raise ValueError(
        f"SP_ALLOW_CREDENTIALS must be 'true' or 'false' (got {value!r})."
    )
    ...
    def with_dynamic_directory(
        ...
        *,
        ...
        Args:
            ...
            middleware (Optional[list[MiddlewareFactory]]): Middleware to apply to sub app. `sp_app_file`
...
class DynamicDirectoryMiddleware:
    ...
    async def __call__(
        ...
        signalpilot_file, remaining_path = match_result
        LOGGER.debug(
            f"Found matching file: {signalpilot_file} with remaining path: {remaining_path}"
        )

        # Add app_path to scope
        # This is considered public API, so downstream middleware can access it
        new_scope = dict(scope)
        new_scope["sp_app_file"] = str(signalpilot_file)
        ...
        cache_key = str(signalpilot_file)
        if cache_key not in self._app_cache:
            ...
            try:
                relative_notebook = signalpilot_file.relative_to(
                    self.directory
                ).as_posix()
...
def create_asgi_app(
    *,
    ...
    from signalpilot._server.api import lifespans
    from signalpilot._server.lsp import NoopLspServer
    from signalpilot._server.main import create_starlette_app
    from signalpilot._server.registry import LIFESPAN_REGISTRY
    from signalpilot._server.session_manager import SessionManager
    from signalpilot._server.tokens import AuthToken
    from signalpilot._server.utils import initialize_asyncio
    from signalpilot._server.workspace import SingleFileWorkspace
    ...
    class Builder(ASGIAppBuilder):
        ...
        def _create_app_for_file(base_url: str, file_path: str) -> ASGIApp:
            ...
            session_manager = SessionManager(
                workspace=SingleFileWorkspace.from_path(SpPath(file_path)),
            ...
            app = create_starlette_app(
                ...
                allow_origins=_parse_allow_origins_env(
                    os.environ.get("SP_ALLOW_ORIGINS")
                ),
                allow_credentials=_parse_allow_credentials_env(
                    os.environ.get("SP_ALLOW_CREDENTIALS")
                ),
