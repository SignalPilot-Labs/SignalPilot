...
from signalpilot._config.config import DisplayConfig, RuntimeConfig
from signalpilot._config.manager import (
    get_default_config_manager,
)
from signalpilot._convert.common.filename import get_download_filename
...
from signalpilot._messaging.errors import Error, is_unexpected_error
from signalpilot._messaging.notification import (
    CellNotification,
    CompletedRunNotification,
)
from signalpilot._messaging.serde import deserialize_kernel_message
from signalpilot._messaging.types import KernelMessage
from signalpilot._output.hypertext import patch_html_for_non_interactive_output
from signalpilot._runtime.commands import AppMetadata, SerializedCLIArgs
from signalpilot._runtime.patches import extract_docstring_from_header
from signalpilot._schemas.serialization import NotebookSerialization
from signalpilot._server.export._status import emit_pdf_export_status
...
from signalpilot._session.notebook import AppFileManager, load_notebook
...
if TYPE_CHECKING:
    ...
    from signalpilot._server.export._status import PDFExportStatusCallback
    from signalpilot._session.state.session_view import SessionView
    from signalpilot._session.types import Session
...
def _as_ir(path: SpPath) -> NotebookSerialization:
    if path.is_python():
        py_contents = path.read_text(encoding="utf-8")
        converter = SpConvert.from_py(py_contents)
        return _with_filename(converter.ir, path.short_name)
    elif path.is_markdown():
        md_contents = path.read_text(encoding="utf-8")
        converter = SpConvert.from_md(md_contents)
        return _with_filename(converter.ir, path.short_name)
    raise ValueError(f"Unsupported file type: {path.path.suffix}")


def export_as_script(path: SpPath) -> ExportResult:
    from signalpilot._convert.script import convert_from_ir_to_script
...
def export_as_md(path: SpPath) -> ExportResult:
    ir = _as_ir(path)
    return ExportResult(
        contents=SpConvert.from_ir(ir).to_markdown(),
...
def export_as_ipynb(
    path: SpPath, sort_mode: Literal["top-down", "topological"]
) -> ExportResult:
...
def notebook_uses_slides_layout(filepath: SpPath) -> bool:
    ...
    *,
    ...
    if include_outputs:
        ...
        if (
            ...
            from signalpilot._server.export._pdf_raster import (
...
async def run_app_then_export_as_reactive_html(
    ...
    generator = SpIslandGenerator.from_file(
    ...
    class RunUntilCompletionSessionConsumer(SessionConsumer):
        ...
        def notify(self, notification: KernelMessage) -> None:
            ...
            if isinstance(data, CellNotification):
                ...
                if output and output.channel == CellChannel.SP_ERROR:
