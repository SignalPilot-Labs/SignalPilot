...
class LiveNotebookServer(AbstractContextManager["LiveNotebookServer"]):
    """Run a temporary headless sp server for live output capture."""
    ...
    def _build_command(self) -> list[str]:
        """Build the sp CLI command used to launch the live notebook."""
        ...
        command = [
            ...
            "--no-skew-protection",
            "--no-check",
            # We don't need a sandbox because we are going to use the same sp process.
            "--no-sandbox",
            "--host",
            "127.0.0.1",
            "--port",
