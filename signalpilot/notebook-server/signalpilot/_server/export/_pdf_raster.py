...
from signalpilot._config.config import DisplayConfig
...
from signalpilot._server.export._live_notebook_server import LiveNotebookServer
from signalpilot._server.export._raster_mime import (
    TEXT_HTML,
    TEXT_MARKDOWN,
    TEXT_PLAIN,
    VEGA_MIME_TYPES,
...
MIMEBUNDLE_TYPE = "application/vnd.sp+mimebundle"
SP_COMPONENT_MARKERS: tuple[str, ...] = (
    "<sp-",
    "&lt;sp-",
)
...
def _should_rasterize_output(mimetype: str, data: Any) -> bool:
    ...
    if mimetype in {TEXT_HTML, TEXT_PLAIN, TEXT_MARKDOWN}:
        return _contains_marker(data, SP_COMPONENT_MARKERS)
...
def _promote_text_output_for_capture(output: Any) -> None:
    if not _contains_marker(output.data, SP_COMPONENT_MARKERS):
        return
...
def _promote_mimebundle_output_for_capture(output: Any) -> None:
    ...
    if html_data is not None and _contains_marker(
        html_data, SP_COMPONENT_MARKERS
    ):
...
async def _collect_static_captures(
    *,
    ...
    with HtmlAssetServer(
        ...
        route="/__signalpilot_pdf_raster__.html",
    ) as server:
...
async def _capture_pngs_from_live_page(
    *,
    ...
    """Capture outputs by running notebook in a live sp server process."""
