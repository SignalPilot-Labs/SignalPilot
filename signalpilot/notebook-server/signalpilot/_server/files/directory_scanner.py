...
from signalpilot._server.files.os_file_system import natural_sort_file
...
def is_sp_app(full_path: str) -> bool:
    """
    Detect whether a file is a sp app.

    Rules:
    - Markdown (`.md`/`.qmd`) files are sp apps if the first 512 bytes
      contain `sp-version:`.
    - Python (`.py`) files are sp apps if the header (first 512 bytes)
      contains both `sp.App` and `import sp`.
    ...
    def contains_sp_app(content: bytes) -> bool:
        return b"sp.App" in content and b"import sp" in content

    try:
        path = SpPath(full_path)

        # Raw editable files (.sql, .yml, etc.) are always valid
        if path.is_raw_editable():
            return True
        ...
        if path.is_markdown():
            return b"sp-version:" in header

        if path.is_python():
            if contains_sp_app(header):
                return True

            if b"# /// script" in header:
                full_content = path.read_bytes()
                if contains_sp_app(full_content):
                    return True
...
class DirectoryScanner:
    """Scans directories for sp files with filtering and limits.
    ...
    def allowed_extensions(self) -> tuple[str, ...]:
        ...
        if self.include_markdown:
            return (".py", ".md", ".qmd") + raw_extensions
        return (".py",) + raw_extensions
