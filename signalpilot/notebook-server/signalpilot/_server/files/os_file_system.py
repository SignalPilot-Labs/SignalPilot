...
class OSFileSystem(FileSystem):
            ...
            with os.scandir(path) as it:
                for entry in it:
                    ...
                    info = FileInfo(
                        ...
                        is_sp_file=not is_directory
                        and self._is_signalpilot_file(entry.path),
    ...
    def _get_file_info(self, path: str) -> FileInfo:
        ...
        return FileInfo(
            ...
            is_directory=is_directory,
            is_sp_file=not is_directory and self._is_signalpilot_file(path),
    ...
    def _is_signalpilot_file(self, path: str) -> bool:
        file_path = Path(path)
        if file_path.suffix not in (".py", ".md", ".qmd"):
            return False

        from signalpilot._server.files.directory_scanner import is_sp_app
