...
from signalpilot._messaging.notification import AlertNotification
from signalpilot._server.models.lsp import (
    LspHealthResponse,
    LspRestartResponse,
    LspServerHealth,
    LspServerId,
...
from signalpilot._tracer import server_tracer
...
class LspServer(ABC):
    ...
    @abstractmethod
    async def restart(
        self, server_ids: list[LspServerId] | None = None
    ) -> LspRestartResponse:
        pass
...
class BaseLspServer(LspServer):
    ...
    async def _start_internal(self) -> AlertNotification | None:
    ...
    async def restart_server(self) -> AlertNotification | None:
...
class CopilotLspServer(BaseLspServer):
    ...
    def missing_binary_alert(self) -> AlertNotification:
        return AlertNotification(
            ...
            description="<span><a class='hyperlink' href='https://docs.signalpilot.ai/docs/'>Install Node.js</a> to use copilot.</span>",
            variant="danger",
        )
...
class PyLspServer(BaseLspServer):
    ...
    - If pylsp fails to start, check ~/.cache/signalpilot/logs/pylsp.log for errors
    ...
    - When pylsp is disabled, sp falls back to Jedi for completions
    """
...
class BasedpyrightServer(BaseLspServer):
    ...
    def get_command(self) -> list[str]:
        lsp_bin = sp_package_path() / "_lsp" / "index.cjs"
...
class CompositeLspServer(LspServer):
    ...
    def _is_enabled(self, config: SpConfig, server_name: str) -> bool:
    ...
    async def start(self) -> AlertNotification | None:
        ...
        tasks: list[asyncio.Task[AlertNotification | None]] = []
...
def any_lsp_server_running(config: SpConfig) -> bool:
