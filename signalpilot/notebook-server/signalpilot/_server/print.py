...
from signalpilot._cli.print import bold, green, light_blue, muted, yellow
...
def print_shutdown() -> None:
    print_()
    print_tabbed(
        "\033[32mThanks for using sp!\033[0m {}".format(_utf8("🌊🍃"))
    )
    print_()
...
def print_experimental_features(config: SpConfig) -> None:
...
def print_mcp_server(mcp_url: str, server_token: str | None) -> None:
    ...
    # Add to Claude code
    print_tabbed(
        f"{_utf8('➜')}  {green('Add to Claude Code')}: claude mcp add --transport http sp {mcp_url}"
    )
    if server_token is not None:
        print_tabbed(
            f"{_utf8('➜')}  {green('Add header')}: Sp-Server-Token: {muted(server_token)}"
        )
    print_()
