...
class LoroDocManager:
    def __init__(self) -> None:
        self.loro_docs: dict[SpFileKey, LoroDoc] = {}
        self.loro_docs_lock = asyncio.Lock()
        self.loro_docs_clients: dict[
            SpFileKey, set[asyncio.Queue[bytes]]
        ] = {}
        self.loro_docs_cleaners: dict[
            SpFileKey, asyncio.Task[None] | None
        ] = {}

    async def _clean_loro_doc(
        self, file_key: SpFileKey, timeout: float = 60
    ) -> None:
    ...
    async def get_or_create_doc(self, file_key: SpFileKey) -> LoroDoc:
    ...
    def add_client_to_doc(
        self, file_key: SpFileKey, update_queue: asyncio.Queue[bytes]
    ) -> None:
    ...
    async def _do_remove_doc(self, file_key: SpFileKey) -> None:
    ...
    async def remove_doc(self, file_key: SpFileKey) -> None:
