...
``~/.local/state/signalpilot/servers/<host>_<port>.json`` at startup and
removes it on shutdown.  External tools (e.g. ``list-sessions.sh``)
read these files to discover running instances — no sp import or
...
from signalpilot._utils.xdg import sp_state_dir
...
def _servers_dir() -> Path:
    return sp_state_dir() / SERVERS_DIR_NAME
...
class ServerRegistryEntry:
    """Minimal descriptor for a running sp server.
