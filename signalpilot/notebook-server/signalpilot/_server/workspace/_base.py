...
def file_not_found(key: SpFileKey) -> HTTPException:
...
def normalize_allowlist_entry(path: str) -> str:
    ...
    Resolves the path to absolute form (validating it as a sp source file)
    and applies our standard normalization. Allowlist entries and lookup keys
    must run through this same pipeline to match.
    """
    return str(normalize_path(Path(SpPath(path).absolute_name)))
