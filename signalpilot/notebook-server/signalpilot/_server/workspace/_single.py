...
class SingleFileWorkspace(NotebookWorkspace):
    ...
    Used by ``sp edit nb.py`` and ``sp run nb.py``. In edit mode the
    allowlist can grow at runtime (via ``register_allowed_path``) to support
    notebooks created during the session.
    """

    def __init__(self, file: SpFile) -> None:
        self._file = file
        self._allowed_paths = {normalize_allowlist_entry(file.path)}

    @staticmethod
    def from_path(file: SpPath) -> SingleFileWorkspace:
        """Build a workspace from a validated sp path."""
