...
in sp's sandbox mode. It handles:
- Finding Python interpreters in virtual environments
- Checking sp installation status in venvs
- PYTHONPATH injection for kernel subprocesses
- Installing sp into configured venvs
"""
...
def get_kernel_pythonpath() -> str:
    ...
    Returns paths needed to import sp and its dependencies (pyzmq, msgspec)
    from the parent process's environment. Used when launching a kernel
    in a configured venv that has sp available via path injection.
    ...
    # Also add sp's parent directory.
    # NB. If running in edit mode this may be local directory.
    for module_name in ["sp", "zmq", "msgspec"]:
        try:
...
def has_signalpilot_installed(venv_python: str) -> bool:
    """Check if sp and its IPC deps are installed in the venv.

    Args:
        venv_python: Path to the venv's Python interpreter.

    Returns:
        True if sp, msgspec, and zmq can all be imported.
    """
    result = subprocess.run(
        [
            venv_python,
            "-c",
            "import sp, msgspec, zmq; print(sp.__version__)",
        ],
        capture_output=True,
        text=True,
    )
...
def install_signalpilot_into_venv(venv_python: str) -> None:
    """Install sp and IPC dependencies into a venv.

    Installs sp and IPC dependencies (pyzmq) into the specified venv.
    ...
    packages = [f"sp=={__version__}"] + get_ipc_kernel_deps()

    echo("Installing sp into configured venv...", err=True)
    ...
    if result.returncode != 0:
        LOGGER.warning(
            f"Failed to install sp into configured venv: {result.stderr}"
        )
