...
class QueueType(Protocol[T]):
    """
    Minimal queue protocol used by SignalPilot.

    Defines the common subset of methods shared by `multiprocessing.Queue` and
    `queue.Queue` that SignalPilot depends on. Keeping this protocol minimal makes it
    straightforward to stub in `sp-lsp` and to substitute alternative queue
