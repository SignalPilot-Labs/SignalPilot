...
from signalpilot._schemas.notebook import (
    NotebookCell,
    NotebookCellConfig,
    NotebookMetadata,
    NotebookV1,
)
from signalpilot._schemas.session import (
    VERSION,
    Cell,
    ConsoleType,
    DataOutput,
...
from signalpilot._utils.background_task import AsyncBackgroundTask
from signalpilot._utils.code import hash_code
from signalpilot._utils.lists import as_list
...
# (see `sp/_runtime/virtual_file/virtual_file.py`). Anchored to the
...
def _normalize_error(error: SpError | dict[str, Any]) -> ErrorOutput:
    ...
    *,
    ...
    for cell_id in cell_ids:
        ...
        if cell_notif.output:
            if cell_notif.output.channel == CellChannel.SP_ERROR:
                for error in cast(
                    list[SpError | dict[str, Any]],
            ...
            if output["type"] == "error":
                ...
                if len(output["data"]) == 0:
                    ...
                    cell_outputs.append(
                        CellOutput(
                            channel=CellChannel.OUTPUT,
                            mimetype="application/vnd.sp+mimebundle",
                            data=output["data"],
                        )
                    )
            else:
...
def serialize_notebook(
    ...
    return NotebookV1(
        version="1",
        metadata=NotebookMetadata(signalpilot_version=__version__),
        cells=cells,
    )
...
def get_session_cache_file(path: Path) -> Path:
    ...
    `foo/bar/__sp__/session/baz.py.json`.
    """
    from signalpilot._utils.paths import notebook_output_dir
...
class SessionCacheManager:
    ...
    def is_cache_hit(
        ...
        if (len(key.codes) != len(notebook_session["cells"])) or any(
            ...
            return False
        if key.signalpilot_version != metadata.get("signalpilot_version"):
            return False
