...
from signalpilot._data.models import DataSourceConnection, DataTable
from signalpilot._messaging.cell_output import CellChannel, CellOutput
from signalpilot._messaging.mimetypes import KnownMimeType, MimeBundleTuple
...
from signalpilot._sql.connection_utils import (
    update_schema_list_in_connection,
    update_table_in_connection,
    update_table_list_in_connection,
)
from signalpilot._sql.engines.duckdb import INTERNAL_DUCKDB_ENGINE
from signalpilot._types.ids import CellId_t, UIElementId, WidgetModelId
from signalpilot._utils.lists import as_list

LOGGER = _loggers.sp_logger()

ExportType = Literal["html", "md", "ipynb", "session"]
MIMEBUNDLE_TYPE: KnownMimeType = "application/vnd.sp+mimebundle"
...
class SessionView:
    ...
    * various other state needed for replay
    ...
    def __init__(self) -> None:
        ...
        # The most recent data source connections notification.
...
def merge_cell_notification(
    ...
    if current.output is None:
        ...
        previous.output is not None
        and current.output.channel == CellChannel.OUTPUT
        and current.output.data == ""
        and (previous.output.data != "" or previous.output.channel != CellChannel.OUTPUT)
    ):
        current.output = previous.output
