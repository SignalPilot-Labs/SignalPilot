...
def _(alt, flights, sp):
    ...
    with alt.data_transformers.enable("vegafusion"):
        sp.output.replace(delay_hist)
    return (delay_hist,)


@app.cell
def _(alt, delay_hist, sp):
    alt.data_transformers.enable("vegafusion")
    sp.as_html(delay_hist)
    return
...
def _(alt, delay_hist, sp):
    ...
    try:
        ...
        sp.output.replace(delay_hist)
        sp.output.replace("No error found")
...
def _(sp):
    sp.md(
        ...
        The examples below still hit `SP_OUTPUT_MAX_BYTES` limitations, and require increasing this before running this notebook:

        ```
        export SP_OUTPUT_MAX_BYTES=100_000_000
        ```
        """
    )
    return
...
@app.cell
def _(sp):
    sp.md(r"""## Interactive Cross-filter""")
    return
...
@app.cell
def _(sp):
    sp.md(r"""## Interactive Chart with Cross-Highlight""")
    return
