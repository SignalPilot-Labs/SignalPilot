...
def _():
    ...
    #   SP_BENCH_ARRAY_SIZE=125000000 python cache_lazy_benchmark.py
    ARRAY_SIZE = int(os.environ.get("SP_BENCH_ARRAY_SIZE", "100000"))
    return ARRAY_SIZE, np, time


@app.cell
def _(ARRAY_SIZE, sp, np, time):
    _start = time.perf_counter()
    with sp.persistent_cache(name="bench_pickle", method="pickle"):
...
def _(ARRAY_SIZE, sp, np, time):
    _start = time.perf_counter()
    with sp.persistent_cache(name="bench_lazy", method="lazy"):
