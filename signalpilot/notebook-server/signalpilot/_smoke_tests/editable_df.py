...
@app.cell
def _(sp):
    sp.md(r"""## Editing different inputs (dicts, lists, polars, pandas)""")
    return


@app.cell(hide_code=True)
def _(column_oriented, sp, pandas_df, polars_df, row_oriented):
    df = sp.ui.dropdown(
        {
            "polars": polars_df,
            "pandas": pandas_df,
...
@app.cell
def _(df, sp):
    edited = sp.ui.data_editor(df.value)
    edited
    return (edited,)
...
def _(edited, sp):
    sp.vstack(
        [
            sp.ui.table(edited.value, selection=None),
...
@app.cell
def _(sp):
    sp.md(r"""## Editing different data types""")
    return
...
@app.cell
def _(sp, pl, varying_data):
    edited_df = sp.ui.data_editor(pl.DataFrame(varying_data))
    edited_df
    return (edited_df,)
