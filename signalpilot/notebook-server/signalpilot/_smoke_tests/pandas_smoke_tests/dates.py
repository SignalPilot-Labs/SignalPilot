...
def _(sp, pd):
    ...
    sp.vstack([sp.plain(date_df), date_df])
    return


@app.cell
def _(datetime, sp, pd):
    time = pd.DataFrame(
        {
...
def _(sp, pd):
    ...
    sp.vstack([sp.plain(date_ranges), date_ranges])
    return
...
@app.cell
def _(sp, pd):
    # Bug with inf re-render https://docs.signalpilot.ai/docs/
    df = pd.DataFrame(
        {
            "ID_SBSC": [6013949] * 10,
            "ID_DATE": [20260401] * 10,
