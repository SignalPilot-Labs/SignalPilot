...
def _(sp):
    ...
    with `subplots=True` don't render properly in sp.
    ...
    **Actual behavior**: Only textual representation appears
    ...
    ndarray of matplotlib Axes objects, which sp doesn't currently format.
    """
    )
    return
...
@app.cell(hide_code=True)
def _(sp):
    sp.md("""## Test Case 1: Box plot WITHOUT subplots (works correctly)""")
    return
...
@app.cell(hide_code=True)
def _(sp):
    sp.md("""## Test Case 3: Multiple subplots with custom layout""")
    return
