...
python sp/_smoke_tests/process_isolation/serve.py

# Option B: sp run (multi-file, auto-enables process isolation)
sp run sp/_smoke_tests/process_isolation/app1.py \
           sp/_smoke_tests/process_isolation/app2.py

# Option C: sp run --sandbox (multi-file, sandbox + process isolation)
sp run --sandbox sp/_smoke_tests/process_isolation/app1.py \
                     sp/_smoke_tests/process_isolation/app2.py
```
...
```bash
sp run sp/_smoke_tests/process_isolation/virtual_files.py \
           sp/_smoke_tests/process_isolation/app1.py
```

Multi-file `sp run` auto-enables process isolation. You should see a
