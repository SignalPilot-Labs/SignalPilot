...
app = sp.App(auto_download=["ipynb"])
...
def _(sp):
    ...
    3. Save again (Ctrl+S)
    4. ✅ Expect: auto-export to `.sp/server_package_alerts.ipynb` succeeds (no more banner)

    ### Scenario 4 — sandbox mode
    ```
    sp edit --sandbox sp/_smoke_tests/server_package_alerts.py
    ```
