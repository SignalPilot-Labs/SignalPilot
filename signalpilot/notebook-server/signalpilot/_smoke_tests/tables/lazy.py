...
@app.cell
def _(execution_timer, ibis_table, sp):
    # This takes a while (runs a count(*) with no limit)
    with execution_timer("sp.ui.dataframe"):
        _df_viewer = sp.ui.dataframe(ibis_table)
    _df_viewer
    return
...
def _(execution_timer, ibis_table, sp):
    # This takes a while (runs a count(*) with no limit)
    with execution_timer("sp.ui.dataframe(limit=10)"):
        _df_viewer = sp.ui.dataframe(ibis_table, limit=10)
    _df_viewer
    return
