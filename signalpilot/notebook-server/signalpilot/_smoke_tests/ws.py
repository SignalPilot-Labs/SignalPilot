...
def _(sp, os):
    ...
    api_key_input = sp.ui.text(value=env_api_key or "", label="YOUR_API_KEY")
    api_secret_input = sp.ui.text(
...
def _(sp):
    def output_stats(df):
        if len(df) == 0:
            return sp.callout("Waiting for data...")
...
def _(alt, sp):
    def output_chart(df):
        ...
        return sp.ui.altair_chart(_chart)
    return (output_chart,)
...
def _(sp):
    def output_table(df):
        return sp.ui.table(df[::-1], selection=None)
    return (output_table,)
            ...
            sp.output.replace(
                sp.vstack(
                    [
                        sp.md(
                            f"""
                        Since sp does not let other cells runt until the current cell has completed,
