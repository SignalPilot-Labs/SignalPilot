...
async def read_snippets(config: SpConfig) -> Snippets:
    snippets: list[Snippet] = []
...
def is_markdown(code: str) -> bool:
    return code.strip().startswith("sp.md")
...
def read_snippet_filenames(
    ...
    if include_default_snippets:
        paths.append(sp_package_path() / "_snippets" / "data")
    if custom_paths:
