...
from signalpilot._sql.engines.types import InferenceConfig, SQLConnection
from signalpilot._sql.utils import CHEAP_DISCOVERY_DATABASES, convert_to_output
...
class IbisToSignalPilotConversionError(Exception):
    """Raise for unhandled type during Ibis to SignalPilot conversion"""
...
class IbisEngine(SQLConnection["SQLBackend"]):
    ...
    def get_table_details(
        ...
        for col_name, ibis_dtype in table_schema.fields.items():
            try:
                col_type: DataType = self._ibis_to_signalpilot_dtype(ibis_dtype)
            except IbisToSignalPilotConversionError:
                col_type = "string"
                LOGGER.warning(
                    f"Failed to convert column `{col_name}` with ibis dtype `{ibis_dtype}` to sp dtype."
    ...
    @staticmethod
    def _ibis_to_signalpilot_dtype(ibis_dtype: dt.DataType) -> DataType:
        """Map the Ibis typing system to sp SQL types.
        ...
        # numeric is a superset of integer, so we evaluate after is_integer
            ...
            raise IbisToSignalPilotConversionError
