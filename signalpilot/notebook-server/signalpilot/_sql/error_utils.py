...
if TYPE_CHECKING:
    ...
    from signalpilot._messaging.errors import SpSQLError
...
class SpSQLException(Exception):
    """Exception raised for SQL-related errors in sp."""
...
def is_sql_parse_error(exception: BaseException) -> bool:
    ...
    return isinstance(exception, SpSQLException)
...
def metadata_to_sql_error(metadata: SQLErrorMetadata) -> SpSQLError:
    """Convert SQLErrorMetadata to SpSQLError for frontend messaging."""
    from signalpilot._messaging.errors import SpSQLError
...
def create_sql_error_from_exception(
    ...
    """Create a SpSQLError from a SQL parsing exception."""
    # Get SQL statement from cell
    sql_statement = ""
    if hasattr(cell, "sqls") and cell.sqls:
        sql_statement = str(cell.sqls[0])

    # Check if this is a SpSQLException with structured hint data
    if isinstance(exception, SpSQLException) and exception.hint:
