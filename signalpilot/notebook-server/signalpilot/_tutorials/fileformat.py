...
def _(sp):
    ...
    sp apps are stored as pure Python files.
...
def _(sp):
    ...
    Consider a sp notebook with the following four cells.
    ...
    Third cell:
    ```python3
    text = sp.ui.text(value=say_hello())
    text
    ```
...
def _(sp):
    sp.md("""
    For the above example, sp would generate the following file
    contents:
    ...
    sp's generated files are **git-friendly**: small changes made using
    the sp editor result in small changes to the file that sp
    generates.
...
def _(sp):
    ...
    sp's file format was designed to be easy to read and easy
    to work with, while also serving the needs of the sp library. You can
    ...
    We explain some properties of sp's file format below.
    """)
    return
...
def _(sp):
    sp.accordion(
        ...
        Cells are stored in the order that they are arranged in the sp
        ...
        sp guarantees that however your source code was
        formatted in the sp editor is exactly how it will be stored in
        ...
        """,
            "No magical tokens": """
        sp's generated code is pure Python; no magical syntax.
        """,
            "Cell signatures automatically maintained": """
        If when editing a cell, you forget to include all a cell's refs in its
        argument list, or all its defs in its returns, sp will fix them he next
        time you try to open it in the sp editor. So don't worry that you'll
        ...
        You can run sp apps as scripts at the command line,
...
def _(sp):
    ...
    The details of sp's file format are important if you want to import
...
def _(sp):
    ...
    sp can serialize functions and classes into the top-level of a file, so you can import them with regular Python syntax:
...
def _(sp):
    ...
    See the docs on [using your own editor](https://docs.signalpilot.ai/docs/).

    ### I want to import functions from a sp notebook, what do I need to know?

    See the docs on [reusable functions and
    classes](https://docs.signalpilot.ai/docs/).

    ### I want to run pytest on sp notebooks, what do I need to know?

    See the docs on [testing](https://docs.signalpilot.ai/docs/).
    """)
    return
