...
def _(sp):
    sp.md(r"""
    With sp, you can mix-and-match both **Python and SQL**. To create a
...
def _(sp):
    ...
    sp is still just Python, even when using SQL. Here is an example of
    how sp embeds SQL in Python in its file format:

    ```python
    output_df = sp.sql(f"SELECT * FROM my_table LIMIT {max_rows.value}")
    ```
...
@app.cell(hide_code=True)
def _(sp):
    charting_library = sp.ui.radio(["matplotlib", "altair", "plotly"])
...
@app.cell(hide_code=True)
def _(charting_library, sp, render_chart, token_prefix):
    _header = sp.md(
        f"""
...
def _(sp, result, token_prefix):
    ...
    def render_altair():
        ...
        return sp.ui.altair_chart(chart, chart_selection=False)
...
def _(sp):
    cars = sp.sql(
        ...
        FROM 'https://datasets.signalpilot.ai/cars.csv';
...
@app.cell(hide_code=True)
def _(cars, sp):
    cylinders_dropdown = sp.ui.range_slider.from_series(
        cars["Cylinders"], debounce=True, show_value=True
    )
    origin_dropdown = sp.ui.dropdown.from_series(cars["Origin"], value="Asia")
    sp.hstack([cylinders_dropdown, origin_dropdown]).left()
    return cylinders_dropdown, origin_dropdown


@app.cell
def _(cars, cylinders_dropdown, sp, origin_dropdown):
    filtered_cars = sp.sql(
        f"""
        SELECT * FROM cars
        WHERE
...
@app.cell(hide_code=True)
def _(filtered_cars, sp):
    sp.hstack(
        [
            sp.stat(label="Total cars", value=str(len(filtered_cars))),
            sp.stat(
