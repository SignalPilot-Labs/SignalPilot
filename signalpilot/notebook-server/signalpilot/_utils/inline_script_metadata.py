...
from signalpilot._cli.files.file_path import FileContentReader
...
def is_sp_dependency(dependency: str) -> bool:
    # Split on any version specifier
    without_version = re.split(r"[=<>~]+", dependency)[0]
    # Match sp and sp[extras], but not sp-<something-else>
    return without_version == "sp" or without_version.startswith("sp[")
...
def has_signalpilot_in_script_metadata(filepath: str) -> bool | None:
    """Check if sp is in the file's PEP 723 script metadata dependencies.

    Returns:
        True if sp is in dependencies
        False if script metadata exists but sp is not in dependencies
    ...
    return any(is_sp_dependency(dep) for dep in dependencies)
