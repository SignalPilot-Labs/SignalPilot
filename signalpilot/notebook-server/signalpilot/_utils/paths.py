...
def sp_package_path() -> Path:
    return Path(str(import_files("signalpilot")))
...
def notebook_output_dir(notebook_path: Path | str | None) -> Path:
    """Compute the __sp__ output directory for a given notebook.
    ...
    ``__pycache__`` directories).  Otherwise the ``__sp__`` directory is
    placed next to the notebook file.

    Resolution order:
        1. ``sys.pycache_prefix`` (if set and path is absolute): mirror tree
        2. Default: ``<notebook_parent>/__sp__``
        3. Fallback (``None`` path): ``__sp__`` relative to CWD
    """
    if notebook_path is None:
        return Path(SP_DIR_NAME)
    ...
    if path.is_dir():
        ...
        # sp notebooks are expected to have a file extension.
        parent = path
    ...
    if prefix is not None and originally_absolute:
        ...
        return Path(prefix) / relative / SP_DIR_NAME

    return parent / SP_DIR_NAME
