...
class SpPath:
    """
    Wrapper around pathlib.Path to provide additional functionality for SignalPilot.
    ...
    def is_valid(self) -> bool:
        return self.is_python() or self.is_markdown() or self.is_raw_editable()
    ...
    def is_raw_editable(self) -> bool:
        allowed = {".sql", ".yml", ".yaml", ".toml", ".txt", ".csv", ".json"}
        return self.path.suffix in allowed

    def rename(self, new_path: Path) -> None:
        if self.strict:
            if not SpPath(new_path).is_relative_to(self.cwd):
                raise ValueError(
