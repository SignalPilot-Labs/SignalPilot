...
export default function AuditPage() {
  ...
  const activitySlots = (() => {
    ...
    entries.forEach((e) => {
      const idx = Math.min(
        35,
        Math.floor(((e.timestamp - minTs) / range) * 35),
      );
      slots[idx]++;
    });
    return slots;
    ...
    <PullToRefreshWrapper onRefresh={refresh}>
      <div className="p-4 sm:p-8 animate-fade-in">
        ...
        <TerminalBar
          ...
          status={
            <StatusDot
              status={entries.length > 0 ? "healthy" : "unknown"}
              size={4}
              pulse={loading}
            />
          }
          ...
          <div className="flex items-center gap-3 sm:gap-6 text-xs">
            ...
            {statsData.blocked > 0 && (
              <span className="text-[var(--color-error)]">
                blocked:{" "}
                <code className="text-[12px]">{statsData.blocked}</code>
              </span>
            )}
          </div>
        </TerminalBar>
        ...
        {entries.length > 0 && (
          <div className="flex flex-wrap items-center gap-3 sm:gap-6 mb-6 px-4 py-2.5 border border-[var(--color-border)] bg-[var(--color-bg-card)]">
            {[
              ...
              {
                label: "queries",
                value: statsData.queries,
                color: "text-[var(--color-success)]",
              },
              {
                label: "executions",
                value: statsData.executions,
                color: "text-blue-400",
              },
              {
                label: "blocked",
                value: statsData.blocked,
                color: statsData.blocked > 0 ? "text-[var(--color-error)]" : "",
              },
            ].map((s) => (
              <div key={s.label} className="flex items-center gap-2">
                ...
                <span className={`text-xs tabular-nums ${s.color}`}>
                  {s.value}
                </span>
              </div>
            ))}
            ...
            {activitySlots.length > 0 && (
              <div
                className={`hidden sm:flex items-center gap-2 ${filtered.filter((e) => e.duration_ms != null).length < 3 ? "ml-auto" : ""}`}
              >
                <span className="text-[11px] text-[var(--color-text-dim)] uppercase tracking-[0.15em]">
                  activity
                </span>
                <ActivityDots
                  values={activitySlots}
                  rows={3}
                  cols={12}
                  dotSize={5}
                  gap={2}
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-1.5">
                    <span
                      className={`flex items-center gap-1.5 text-[10px] tracking-wider ${color}`}
                    >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {isExpanded ? (
                        ...
                        <ChevronRight className="w-2.5 h-2.5 text-[var(--color-text-dim)]" />
                      )}
                    </div>
                  </div>
                  {/* SQL preview */}
                  {entry.sql ? (
                    ...
                    <p className="text-[10px] text-[var(--color-error)] truncate">
                      {entry.block_reason}
                    </p>
                  ) : null}
                  {/* Meta row */}
                  <div className="flex items-center gap-3 mt-1.5 text-[9px] text-[var(--color-text-dim)] tracking-wider">
                    ...
                    {entry.rows_returned != null && (
                      <span className="tabular-nums">
                        {entry.rows_returned.toLocaleString()} rows
                      </span>
                    )}
                  ...
                  {isExpanded && entry.sql && (
                    <div className="mt-3 animate-fade-in">
                      <p className="text-[9px] uppercase tracking-[0.15em] text-[var(--color-text-dim)] mb-1">
                        full sql
                      </p>
            ...
            <table className="w-full text-xs min-w-[640px]">
              <thead>
                <tr className="border-b border-[var(--color-border)]">
                  {["time", "type", "connection", "detail", "duration"].map(
                    (h, i) => (
                      <th
                        key={h}
                        className={`${i === 4 ? "text-right" : "text-left"} px-4 py-2.5 text-[11px] text-[var(--color-text-dim)] uppercase tracking-[0.15em]`}
                      >
                        {h}
                      </th>
                    ),
              ...
              <tbody className="divide-y divide-[var(--color-border)]">
                {filtered.map((entry) => {
                  const Icon = typeIcons[entry.event_type] || ScrollText;
                  const color =
                    typeColors[entry.event_type] || typeColors.connect;
                  const isExpanded = expandedRow === entry.id;
                  return (
                    <tr
                      key={entry.id}
                    ...
                    >
                      <td className="px-4 py-2.5 text-[12px] text-[var(--color-text-dim)] tabular-nums whitespace-nowrap tracking-wider align-top">
                        ...
                        {isExpanded && entry.sql && (
                          <div className="mt-3 ml-4 animate-fade-in">
                            <p className="text-[11px] uppercase tracking-[0.15em] text-[var(--color-text-dim)] mb-1">
                              full sql
                            </p>
                      ...
                      <td className="px-4 py-2.5 align-top">
                        <span
                          className={`flex items-center gap-1.5 text-[12px] tracking-wider ${color}`}
                        >
                          <Icon className="w-3 h-3" strokeWidth={1.5} />
                          {entry.event_type}
                          {entry.blocked && (
                        ...
                        {entry.tables.length > 0 && (
                          <div className="flex gap-1 mt-1 flex-wrap">
                            {entry.tables.map((t) => (
                              <span
                                key={t}
                                className="text-[11px] px-1 py-0.5 border border-[var(--color-border)] text-[var(--color-text-dim)] tracking-wider"
                              >
                                {t}
                              </span>
                            ))}
                      ...
                      <td className="px-4 py-2.5 text-[12px] text-right tabular-nums whitespace-nowrap align-top tracking-wider">
                        ...
                        {entry.rows_returned != null && (
                          <div className="text-[11px] text-[var(--color-text-dim)] mt-0.5">
                            {entry.rows_returned.toLocaleString()}r
                          </div>
                        )}
                      </td>
                    </tr>
