...
const CONNECTOR_TIERS: Record<
  DBType,
  { tier: number; label: string; color: string }
> = {
  postgres: {
    tier: 1,
    label: "T1",
...
const CONNECTION_PRESETS: ConnectionPreset[] = [
  ...
  },
  {
    ...
    defaults: {
      ...
      ssl_enabled: true,
      ssl_mode: "require",
      description: "AWS RDS MySQL instance",
    },
  },
  {
    ...
    defaults: {
      ...
      ssl_enabled: true,
      azure_ad_auth: true,
      description: "Azure SQL with Entra ID auth",
    },
  },
  {
  ...
  },
  {
    ...
    defaults: {
      ...
      description: "Google BigQuery with service account",
    },
  },
  {
    ...
    icon: "🧱",
    defaults: {
      databricks_auth_method: "oauth_m2m",
      description: "Databricks with OAuth M2M service principal",
    },
  },
  {
    ...
    defaults: {
      trino_https: true,
      port: "443",
      trino_auth_method: "password",
      description: "Starburst Galaxy / Trino HTTPS",
    },
  },
  {
  ...
  },
  {
    ...
    defaults: {
      ...
      ssl_enabled: true,
      ssl_mode: "verify-ca",
      description: "GCP Cloud SQL for PostgreSQL",
    },
  },
  {
    label: "MotherDuck",
  ...
  },
  {
    ...
    defaults: {
      ssh_enabled: true,
      ssh_port: "22",
      description: "Database behind SSH bastion",
    },
  },
];
    ...
    case "postgres":
      return (
        ...
        >
          ...
          />
          <path
            d="M1.5 3V9C1.5 10.1 3.5 11 6 11C8.5 11 10.5 10.1 10.5 9V3"
            stroke="currentColor"
            strokeWidth="0.75"
          />
          <path
            d="M1.5 6C1.5 7.1 3.5 8 6 8C8.5 8 10.5 7.1 10.5 6"
            stroke="currentColor"
            strokeWidth="0.5"
            opacity="0.5"
          />
    ...
    case "duckdb":
      return (
        ...
        >
          ...
          <path
            d="M4 7.5C4.5 8.5 7.5 8.5 8 7.5"
            stroke="currentColor"
            strokeWidth="0.75"
            strokeLinecap="round"
            fill="none"
      ...
      <input
        ...
        className={`w-full px-3 py-2 bg-[var(--color-bg-input)] border text-xs focus:outline-none tracking-wide ${
          error
            ...
            : "border-[var(--color-border)] focus:border-[var(--color-text-dim)]"
        }`}
      />
      {error && (
        <p className="text-[11px] text-[var(--color-error)] mt-1 tracking-wider">
          {error}
        </p>
      )}
      {hint && !error && (
...
const defaultForm: FormState = {
  ...
  azure_client_secret: "",
  trino_https: false,
  trino_auth_method: "none",
  trino_jwt_token: "",
  trino_client_cert: "",
  trino_client_key: "",
  trino_krb_service_name: "trino",
  motherduck_token: "",
  tags: [],
  tagInput: "",
...
function buildConnectionPreview(form: FormState): string {
  ...
  if (form.connectionMode === "url" && form.connection_string)
    return form.connection_string.replace(/:[^:@]*@/, ":****@");
...
function detectDbTypeFromUrl(url: string): DBType | null {
  ...
  if (
    ...
    lower.startsWith("mysql+pymysql://") ||
    lower.startsWith("mariadb://")
  )
    return "mysql";
  if (lower.startsWith("redshift://")) return "redshift";
  if (
    lower.startsWith("clickhouse://") ||
    lower.startsWith("clickhouses://") ||
    lower.startsWith("clickhouse+http://") ||
    lower.startsWith("clickhouse+https://")
  )
    return "clickhouse";
  if (lower.startsWith("snowflake://")) return "snowflake";
  if (
    lower.startsWith("mssql://") ||
    lower.startsWith("mssql+pymssql://") ||
    lower.startsWith("sqlserver://")
  )
    return "mssql";
  if (lower.startsWith("trino://") || lower.startsWith("trino+https://"))
    return "trino";
  ...
  try {
    if (
      ...
      const parsed = new URL(
        url.replace(
          /^(postgresql|redshift|clickhouse|mysql\+pymysql|mssql|mssql\+pymssql|sqlserver):/,
          "http:",
        ),
      );
      return {
...
function validateForm(form: FormState): Record<string, string> {
  ...
  if (form.connectionMode === "url") {
    if (!form.connection_string.trim())
      errors.connection_string = "connection URL is required";
    return errors;
  }

  // DB-specific validation
  if (config.fields.includes("host") && !form.host.trim())
  ...
  if (form.db_type === "bigquery") {
    ...
    if (
      ...
      try {
        ...
        errors.credentials_json = "invalid JSON format";
      }
    }
  }

  if (form.db_type === "databricks") {
    if (!form.http_path.trim())
      errors.http_path =
        "HTTP path is required (e.g., /sql/1.0/warehouses/abc123)";
    if (form.databricks_auth_method === "pat" && !form.access_token.trim())
      errors.access_token = "personal access token is required";
    if (form.databricks_auth_method === "oauth_m2m") {
      if (!form.dbx_oauth_client_id?.trim())
        errors.dbx_oauth_client_id = "OAuth client ID is required for M2M auth";
      if (!form.dbx_oauth_client_secret?.trim())
        errors.dbx_oauth_client_secret =
          "OAuth client secret is required for M2M auth";
    }
  }
  ...
  if (form.ssh_enabled) {
    ...
    if (form.ssh_auth_method === "password" && !form.ssh_password.trim())
    ...
    if (form.ssh_auth_method === "key") {
      if (!form.ssh_private_key.trim()) {
        ...
        errors.ssh_private_key =
          "must be a PEM-format private key (-----BEGIN ... PRIVATE KEY-----)";
      }
    }
  ...
  if (form.connection_timeout) {
    ...
    if (isNaN(ct) || ct < 1 || ct > 300)
      errors.connection_timeout = "connection timeout must be 1-300 seconds";
  }
  if (form.query_timeout) {
    const qt = parseInt(form.query_timeout);
    if (isNaN(qt) || qt < 1 || qt > 3600)
      errors.query_timeout = "query timeout must be 1-3600 seconds";
  }
...
function buildCreatePayload(form: FormState): Record<string, unknown> {
  ...
  if (config.fields.includes("port"))
    payload.port = parseInt(form.port) || config.defaultPort;
  ...
  if (config.fields.includes("schema_name"))
    payload.schema_name = form.schema_name;
  ...
  if (config.fields.includes("credentials_json"))
  ...
  if (config.fields.includes("access_token"))
  ...
  if (form.db_type === "trino" && form.connectionMode !== "url") {
    if (form.trino_https) {
      ...
      const userPart = form.password
        ...
        : `${form.username || "trino"}@`;
      const pathPart = form.catalog
        ? `/${form.catalog}${form.schema_name ? `/${form.schema_name}` : ""}`
        : "";
    ...
    if (form.trino_auth_method !== "none") {
      ...
      if (form.trino_auth_method === "jwt") {
        ...
        payload.kerberos_config = {
          service_name: form.trino_krb_service_name || "trino",
        };
      }
    }
  }
  ...
  if (form.db_type === "snowflake") {
    ...
    if (form.snowflake_auth_method === "key_pair") {
      ...
      if (form.sf_private_key_passphrase)
        payload.private_key_passphrase = form.sf_private_key_passphrase;
  ...
  if (
    ...
    if (form.db_type === "redshift") {
      if (form.redshift_cluster_id)
        payload.cluster_id = form.redshift_cluster_id;
  ...
  if (form.azure_ad_auth && form.db_type === "mssql") {
    ...
    if (form.azure_client_secret)
      payload.azure_client_secret = form.azure_client_secret;
  }
  ...
  if (form.ssh_enabled && config.supportsSSH) {
    const sshPayload: Record<string, unknown> = {
      ...
      private_key_passphrase:
        form.ssh_auth_method === "key" ? form.ssh_key_passphrase : null,
    };
  ...
  if (form.schema_filter_include.trim()) {
    payload.schema_filter_include = form.schema_filter_include
      .split(",")
      .map((s: string) => s.trim())
      .filter(Boolean);
  }
  if (form.schema_filter_exclude.trim()) {
    payload.schema_filter_exclude = form.schema_filter_exclude
      .split(",")
      .map((s: string) => s.trim())
      .filter(Boolean);
  }
  ...
  if (connTimeout && connTimeout !== 15)
    payload.connection_timeout = connTimeout;
...
function ConnectionFieldsForm({
  ...
  if (form.connectionMode === "url") {
    const urlHints: Record<string, string> = {
      ...
      redshift:
        "redshift://user:pass@cluster.region.redshift.amazonaws.com:5439/dev",
      clickhouse:
        "clickhouse://user:pass@host:9000/default  (or clickhouse+http:// for HTTP)",
      snowflake:
        "snowflake://user:pass@account/db/schema?warehouse=WH&role=ROLE",
      databricks:
        "databricks://token@host.databricks.com/sql/1.0/warehouses/abc?catalog=main",
      mssql: "mssql://sa:password@host:1433/mydb",
      trino: "trino://user@host:8080/catalog/schema",
    };
    const parsed = form.connection_string
      ? parseConnectionUrl(form.connection_string, form.db_type)
      : null;
    ...
    return (
      <>
        <FormInput
          ...
          onChange={(v) => {
            ...
            if (detected && detected !== form.db_type) {
              ...
              setForm({
                ...
                connection_string: v,
                db_type: detected,
                port: String(DB_CONFIGS[detected].defaultPort),
              });
            } else {
          ...
          type="password"
          placeholder={
            urlHints[form.db_type] ||
            "paste any connection string — db type auto-detected"
          }
          hint={
            form.db_type === "clickhouse"
              ? "native: clickhouse://... | HTTP: clickhouse+http://..."
              : "paste a URL — database type is auto-detected from the scheme"
          }
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-1">
              {parsed.host && (
                <span className="text-[11px] tracking-wider">
                  <span className="text-[var(--color-text-dim)]">host:</span>{" "}
              ...
              {parsed.database && (
                <span className="text-[11px] tracking-wider">
                  <span className="text-[var(--color-text-dim)]">db:</span>{" "}
                  <span className="text-[var(--color-text)]">
                    {parsed.database}
                  </span>
                </span>
              ...
              {parsed.username && (
                <span className="text-[11px] tracking-wider">
                  <span className="text-[var(--color-text-dim)]">user:</span>{" "}
                  <span className="text-[var(--color-text)]">
                    {parsed.username}
                  </span>
                </span>
              ...
              {parsed.catalog && (
                <span className="text-[11px] tracking-wider">
                  <span className="text-[var(--color-text-dim)]">catalog:</span>{" "}
                  <span className="text-[var(--color-text)]">
                    {parsed.catalog}
                  </span>
                </span>
              ...
              {parsed.password && (
                <span className="text-[11px] tracking-wider text-[var(--color-success)]">
                  password: ****
                </span>
              )}
            </div>
    ...
    return (
      <>
          ...
          <div className="flex gap-2">
            {(["password", "key_pair", "oauth"] as const).map((method) => (
              <button
                ...
                type="button"
                onClick={() =>
                  setForm({ ...form, snowflake_auth_method: method })
                }
        ...
        {form.snowflake_auth_method === "password" ? (
          <>
            ...
            <div className="col-span-2 px-3 py-2 border border-[var(--color-warning)]/30 bg-[var(--color-warning)]/5 text-[11px] text-[var(--color-warning)] tracking-wider">
              <AlertTriangle
                ...
                strokeWidth={1.5}
              />
              snowflake is enforcing mandatory MFA for all accounts.
              password-only connections will stop working. switch to{" "}
              <button
                type="button"
                onClick={() =>
                  setForm({ ...form, snowflake_auth_method: "key_pair" })
                }
              ...
              <button
                type="button"
                onClick={() =>
                  setForm({ ...form, snowflake_auth_method: "oauth" })
                }
                className="underline hover:text-[var(--color-text)]"
              >
                OAuth
          ...
          <>
            ...
            <FormInput
              ...
              value={form.sf_private_key_passphrase}
              onChange={(v) =>
                setForm({ ...form, sf_private_key_passphrase: v })
              }
              type="password"
              hint="leave empty if key is unencrypted"
              className="col-span-2"
            />
          </>
        ) : (
          <>
            <FormInput
              ...
              value={form.sf_oauth_token}
              onChange={(v) => setForm({ ...form, sf_oauth_token: v })}
              type="password"
              required
              className="col-span-2"
              hint="from your identity provider (Okta, Azure AD, etc.)"
            />
            <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
              <div>
                ...
                Create a Snowflake security integration (CREATE SECURITY
                INTEGRATION ... TYPE = EXTERNAL_OAUTH) and configure your IdP to
                issue tokens.
              </div>
              <div>
                ...
                Use Snowflake&apos;s built-in SNOWFLAKE$LOCAL_APPLICATION
                integration for quick setup without admin involvement.
              </div>
            </div>
          </>
        )}
        <FormInput
          ...
          hint="optional — default warehouse"
        />
        <FormInput
          label="database"
          value={form.database}
          ...
          placeholder="PROD_DB"
          hint="optional — default database"
        />
        <FormInput
          label="schema"
          value={form.schema_name}
          onChange={(v) => setForm({ ...form, schema_name: v })}
          placeholder="PUBLIC"
          hint="optional — default schema"
        />
        <FormInput
          label="role"
          value={form.role}
        ...
        <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
          <div>
            ...
            Add this server&apos;s IP to ALLOWED_IP_LIST. Snowflake Admin →
          ...
          <div>
            ...
            For AWS PrivateLink or Azure Private Link, use the private account
            URL (e.g., org-account.privatelink.snowflakecomputing.com).
          </div>
          <div>
            <span className="text-[var(--color-text-muted)]">vpn:</span> If your
            Snowflake is behind a VPN, ensure SignalPilot has network access to
  ...
  if (form.db_type === "bigquery") {
    ...
    const bqAuthLabels: Record<string, string> = {
    ...
    return (
      <>
        ...
        <FormInput
          ...
          hint="optional — default dataset for queries"
        />
        ...
        {form.bq_auth_method === "oauth" && (
          <>
            <FormInput
              label="OAuth access token"
              value={form.bq_oauth_token}
              onChange={(v) => setForm({ ...form, bq_oauth_token: v })}
              type="password"
              required
              className="col-span-2"
              hint="from Google Cloud OAuth flow or gcloud auth print-access-token"
            />
            <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
              <div>
                <span className="text-[var(--color-text-muted)]">setup:</span>{" "}
                Create an OAuth client in GCP Console → APIs & Services →
                Credentials → OAuth 2.0 Client ID.
              </div>
              <div>
                <span className="text-[var(--color-text-muted)]">scopes:</span>{" "}
                Token must include https://www.googleapis.com/auth/bigquery
                scope.
              </div>
            </div>
          </>
        ...
        {form.bq_auth_method === "adc" && (
          <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
            <div>
              <span className="text-[var(--color-text-muted)]">setup:</span> Run{" "}
              <code className="bg-[var(--color-bg-hover)] px-1">
                gcloud auth application-default login
              </code>{" "}
              on the server, or set GOOGLE_APPLICATION_CREDENTIALS env var.
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">gke:</span> On
              GKE, workload identity is used automatically. Ensure the KSA is
              bound to a GCP SA with BigQuery roles.
            </div>
          </div>
        )}
        ...
        <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
          <div>
            ...
            Set max bytes billed to prevent runaway costs. 2026 pricing:
            $6.25/TB on-demand (first 1TB free).
          </div>
          <div>
            <span className="text-[var(--color-text-muted)]">vpc:</span> For VPC
            Service Controls, ensure the service account has access from
            SignalPilot&apos;s network perimeter.
          </div>
        </div>
      </>
    );
    ...
    return (
      <>
        ...
        <FormInput
          ...
          onChange={(v) => setForm({ ...form, http_path: v })}
          placeholder="/sql/1.0/warehouses/abc123"
          hint="SQL warehouse or cluster HTTP path"
          required
        />
        {/* Auth method selector */}
        <div className="col-span-2 mb-1">
          ...
          <div className="flex flex-wrap gap-2">
            {(["pat", "oauth_m2m", "oauth_u2m"] as const).map((method) => (
              <button
                ...
                type="button"
                onClick={() =>
                  setForm({ ...form, databricks_auth_method: method })
                }
                className={`px-2.5 py-1 text-[12px] tracking-wider border transition-all ${
        ...
        {form.databricks_auth_method === "pat" ? (
          ...
          <div className="col-span-2 grid grid-cols-2 gap-3 p-3 border border-amber-500/20 bg-amber-500/5">
            <FormInput
              label="client ID"
              value={form.dbx_oauth_client_id}
              onChange={(v) => setForm({ ...form, dbx_oauth_client_id: v })}
              placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
              hint="service principal application (client) ID"
              required
            />
            <FormInput
              label="client secret"
              value={form.dbx_oauth_client_secret}
              onChange={(v) => setForm({ ...form, dbx_oauth_client_secret: v })}
              type="password"
            ...
            <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
              <div>
                ...
                Grant CAN USE on the SQL Warehouse and data access on Unity
                Catalog.
              </div>
              <div>
                <span className="text-[var(--color-text-muted)]">
                ...
                OAuth M2M is the production-grade auth method. PATs are
                workspace-scoped and expire.
              </div>
            </div>
          </div>
        ) : (
          <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
            <div>
              ...
              OAuth U2M opens a browser window for authentication. Best for
              interactive development — the token is automatically refreshed.
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">setup:</span>{" "}
              Ensure your Databricks workspace has OAuth configured (Admin
              Console → App Connections) and your user has access to the SQL
              Warehouse.
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">note:</span>{" "}
              OAuth U2M requires the server to have browser access. For
              headless/server environments, use OAuth M2M instead.
            </div>
          </div>
        )}
        <FormInput
          ...
          placeholder="main"
          hint="optional — Unity Catalog name"
        />
        <FormInput
          label="schema"
          value={form.schema_name}
        ...
        <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
          <div>
            ...
            For AWS PrivateLink or Azure Private Link, use the private workspace
            URL (e.g., adb-xxx.x.azuredatabricks.net).
          </div>
          <div>
            <span className="text-[var(--color-text-muted)]">
              unity catalog:
            </span>{" "}
            If enabled, PKs, FKs, and constraints will be automatically
            extracted for join discovery.
          </div>
          <div>
            <span className="text-[var(--color-text-muted)]">
              ip access list:
            </span>{" "}
            Add this server&apos;s IP to the workspace IP Access List (Workspace
            Settings → Security → IP Access Lists).
          </div>
        </div>
      </>
    );
  ...
  if (form.db_type === "trino") {
    ...
    const trinoAuthLabels: Record<string, string> = {
      none: "no auth",
      password: "password",
      jwt: "JWT token",
    ...
    return (
      <>
        ...
        <FormInput
          ...
          placeholder={form.trino_https ? "443" : "8080"}
        />
        <FormInput
          label="username"
          value={form.username}
        ...
        <FormInput
          ...
          onChange={(v) => setForm({ ...form, catalog: v })}
          placeholder="hive"
          hint="default catalog for queries"
        />
        <FormInput
          label="schema"
          value={form.schema_name}
        ...
        {form.trino_auth_method === "jwt" && (
          <FormInput
            label="JWT token"
            value={form.trino_jwt_token}
            onChange={(v) => setForm({ ...form, trino_jwt_token: v })}
            type="password"
            required
            className="col-span-2"
            hint="Bearer token from your identity provider (Okta, Auth0, etc.)"
          />
        )}
        ...
        {form.trino_auth_method === "kerberos" && (
          <>
            <FormInput
              label="service name"
              value={form.trino_krb_service_name}
              onChange={(v) => setForm({ ...form, trino_krb_service_name: v })}
              placeholder="trino"
              hint="Kerberos service principal name"
              className="col-span-2"
            />
            <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
              <div>
                <span className="text-[var(--color-text-muted)]">setup:</span>{" "}
                Configure krb5.conf and kinit before connecting. The server must
                have a valid Kerberos ticket.
              </div>
              <div>
                <span className="text-[var(--color-text-muted)]">keytab:</span>{" "}
                For unattended access, configure a keytab file in
                /etc/krb5.keytab or via KRB5_KTNAME.
              </div>
            </div>
          </>
        )}
        ...
        <div className="col-span-2">
          <button
            ...
            onClick={() =>
              setForm({
                ...form,
                trino_https: !form.trino_https,
                port: !form.trino_https ? "443" : "8080",
              })
            }
            className="flex items-center gap-2 text-[12px] tracking-wider transition-colors"
          >
            <div
              className={`w-3 h-3 border flex items-center justify-center transition-colors ${form.trino_https ? "border-emerald-500 bg-emerald-500/20" : "border-[var(--color-border)]"}`}
            >
              {form.trino_https && (
          ...
          <p className="text-[11px] text-[var(--color-text-dim)] mt-1 tracking-wider opacity-60 ml-5">
            {form.trino_https
              ? "encrypted connection — required for Starburst Galaxy and password auth"
              : "plain HTTP — for local/development clusters only"}
          </p>
        </div>
        <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider">
          <span className="text-[var(--color-text-muted)]">note:</span> Trino
          supports federated queries across multiple catalogs (Hive, Iceberg,
          MySQL, PostgreSQL, etc.). Each catalog maps to a data source
          configured in Trino.
        </div>
      </>
    );
  ...
  if (form.db_type === "duckdb" || form.db_type === "sqlite") {
    const isMotherDuck =
      form.db_type === "duckdb" && form.database.startsWith("md:");
    return (
      <>
        <FormInput
          label="database path"
          ...
          placeholder={
            form.db_type === "duckdb"
              ? ":memory: or /path/to/db.duckdb or md:my_db"
              : ":memory: or /path/to/db.sqlite"
          }
          hint={
            form.db_type === "duckdb"
              ? "file path, :memory:, or md:<db_name> for MotherDuck cloud"
              : "file path or :memory:"
          }
          className="col-span-2"
        />
    ...
    return (
      <>
              ...
              <button
                ...
                onClick={() =>
                  setForm({
                    ...form,
                    ch_protocol: proto,
                    port: proto === "http" ? httpPort : nativePort,
                  })
                }
                className={`px-2.5 py-1 text-[12px] tracking-wider border transition-all ${
        ...
        <FormInput
          ...
          value={form.port}
          onChange={(v) => setForm({ ...form, port: v })}
          placeholder={form.ch_protocol === "http" ? httpPort : nativePort}
        />
        <FormInput
          label="database"
          value={form.database}
    ...
    return (
      <>
        <FormInput
          ...
          hint="hostname or IP — for named instances: host\\INSTANCE"
          required
        />
        <FormInput
          label="port"
          ...
          onChange={(v) => setForm({ ...form, port: v })}
          placeholder="1433"
          hint="default 1433 — Azure SQL uses 1433"
        />
        <FormInput
          label="database"
          value={form.database}
        ...
        <div className="col-span-2 mb-1">
          <button
            type="button"
            onClick={() =>
              setForm({ ...form, azure_ad_auth: !form.azure_ad_auth })
            }
          ...
          >
            ...
            {form.azure_ad_auth && (
              <span className="text-[var(--color-success)] text-[11px]">
                enabled
              </span>
            )}
          </button>
        ...
        {form.azure_ad_auth && (
          <div className="col-span-2 grid grid-cols-2 gap-3 p-3 border border-blue-500/20 bg-blue-500/5">
            <FormInput
              ...
              onChange={(v) => setForm({ ...form, azure_tenant_id: v })}
              placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
              hint="Azure AD directory (tenant) ID"
              required
            />
            <FormInput
              label="client ID"
              value={form.azure_client_id}
              onChange={(v) => setForm({ ...form, azure_client_id: v })}
              placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
              hint="App registration (client) ID"
              required
            />
            <FormInput
              label="client secret"
              value={form.azure_client_secret}
              onChange={(v) => setForm({ ...form, azure_client_secret: v })}
              type="password"
              hint="Service principal client secret"
              required
              className="col-span-2"
            ...
            <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
              <div>
                <span className="text-[var(--color-text-muted)]">setup:</span>{" "}
                Azure Portal → App Registrations → New → Add API Permission for
                Azure SQL Database. Create a contained DB user: CREATE USER
                [app-name] FROM EXTERNAL PROVIDER.
              </div>
              <div>
                <span className="text-[var(--color-text-muted)]">
                  managed identity:
                </span>{" "}
                For Azure VMs/containers, leave client secret empty to use
                system-assigned managed identity (coming soon).
              </div>
            </div>
          </div>
        )}
        <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
          <div>
            <span className="text-[var(--color-text-muted)]">azure sql:</span>{" "}
            Use &lt;server&gt;.database.windows.net as host. Ensure firewall
            rule allows this server&apos;s IP.
          </div>
          <div>
            <span className="text-[var(--color-text-muted)]">
              named instances:
            </span>{" "}
            Include instance in host: host\SQLEXPRESS. Or use port directly (SQL
            Browser resolves instances to ports).
          </div>
          <div>
            <span className="text-[var(--color-text-muted)]">on-prem:</span> For
            SQL Server behind a firewall, use the SSH tunnel option in Advanced
            settings.
          </div>
        </div>
      </>
    ...
    return (
      <>
        <FormInput
          ...
          hint="Redshift console → Clusters → Properties → Endpoint"
          required
        />
        <FormInput
          label="port"
        ...
        {form.iam_auth && (
          <div className="col-span-2 grid grid-cols-2 gap-3 p-3 border border-amber-500/20 bg-amber-500/5">
            <FormInput
              ...
              onChange={(v) => setForm({ ...form, username: v })}
              placeholder="awsuser"
              hint="Redshift DB user to get temporary credentials for"
              required
            />
            <FormInput
              label="AWS region"
              value={form.aws_region}
              onChange={(v) => setForm({ ...form, aws_region: v })}
              placeholder="us-east-1"
              hint="Redshift cluster region"
            />
            <FormInput
              label="cluster ID"
              value={form.redshift_cluster_id}
              onChange={(v) => setForm({ ...form, redshift_cluster_id: v })}
              placeholder="my-redshift-cluster"
              hint="provisioned cluster ID (auto-detected from endpoint if blank)"
            />
            <FormInput
              label="workgroup"
              value={form.redshift_workgroup}
              onChange={(v) => setForm({ ...form, redshift_workgroup: v })}
              placeholder="default"
              hint="for Redshift Serverless only"
            />
            <FormInput
              label="AWS access key ID"
              value={form.aws_access_key_id}
              onChange={(v) => setForm({ ...form, aws_access_key_id: v })}
              placeholder="AKIA..."
              hint="leave empty to use instance profile / env credentials"
            />
            <FormInput
              label="AWS secret access key"
              value={form.aws_secret_access_key}
              onChange={(v) => setForm({ ...form, aws_secret_access_key: v })}
              type="password"
              hint="leave empty to use instance profile / env credentials"
            />
            <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
              <div>
                <span className="text-[var(--color-text-muted)]">setup:</span>{" "}
                IAM user/role needs redshift:GetClusterCredentials (provisioned)
                or redshift-serverless:GetCredentials (serverless).
              </div>
              <div>
                <span className="text-[var(--color-text-muted)]">
                  credentials:
                </span>{" "}
                Leave access key fields empty to use EC2 instance profile, ECS
                task role, or AWS_* env vars.
              </div>
            </div>
          </div>
        )}
        <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
          <div>
            ...
            Ensure this server&apos;s IP is allowed in the Redshift security
            group. For VPC clusters, use SSH tunnel or VPC peering.
          </div>
          <div>
            <span className="text-[var(--color-text-muted)]">serverless:</span>{" "}
            Use workgroup endpoint:
  ...
  const ph = placeholders[form.db_type] || {
    host: "localhost",
    db: "mydb",
    user: "user",
  };
    ...
    <>
      ...
      {form.iam_auth && (
        <>
          ...
          <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
            <div>
              <span className="text-[var(--color-text-muted)]">setup:</span> DB
              user must have rds_iam role (PostgreSQL) or be created with
              AWSAuthenticationPlugin (MySQL). SSL is auto-enabled.
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">
                credentials:
      ...
      <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider space-y-1">
        {form.db_type === "postgres" ? (
          <>
            <div>
              <span className="text-[var(--color-text-muted)]">rds:</span> Use
              endpoint from RDS Console → Connectivity. Ensure security group
              allows this server&apos;s IP on port 5432.
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">supabase:</span>{" "}
              Project Settings → Database → Connection string. Use pooler for
              serverless (port 6543).
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">neon:</span> Use
              the connection string from Neon console. SSL is required
              (auto-enabled).
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">on-prem:</span>{" "}
              For databases behind a firewall, enable the SSH tunnel in Advanced
              settings.
            </div>
          </>
        ) : (
          <>
            <div>
              <span className="text-[var(--color-text-muted)]">rds:</span> Use
              endpoint from RDS Console → Connectivity. Security group must
              allow port 3306 from this server.
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">
                planetscale:
              </span>{" "}
              Use the connection string from PlanetScale dashboard. SSL is
              required (auto-enabled).
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">cloud sql:</span>{" "}
              For Google Cloud SQL, use the Cloud SQL Auth Proxy or add this
              server&apos;s IP to authorized networks.
            </div>
            <div>
              <span className="text-[var(--color-text-muted)]">on-prem:</span>{" "}
              For MySQL behind a firewall, enable the SSH tunnel in Advanced
              settings.
            </div>
          </>
        )}
          ...
          <div>
            ...
            >
              <option value="require">
                require — encrypt, skip cert verification
              </option>
              ...
              <option value="prefer">
                prefer — encrypt if server supports
              </option>
            ...
            <p className="text-[10px] text-[var(--color-text-dim)] tracking-wider mt-1 opacity-60">
              {form.ssl_mode === "require" &&
                "encrypts traffic but does not verify the server certificate. good for cloud databases with trusted networks."}
              {form.ssl_mode === "verify-ca" &&
                "verifies the server cert is signed by a trusted CA. requires CA certificate below."}
              {form.ssl_mode === "verify-full" &&
                "strongest security: verifies CA + server hostname matches the cert. recommended for production."}
              {form.ssl_mode === "prefer" &&
                "uses encryption if the server supports it, falls back to plaintext otherwise."}
              {form.ssl_mode === "allow" &&
                "connects without preference — server decides. not recommended for production."}
              {form.ssl_mode === "disable" &&
                "no encryption. only use for local development or trusted private networks."}
            </p>
          </div>
          <div />
          {form.ssl_mode !== "disable" && (
            <>
              <FormTextArea
                ...
                onChange={(v) => setForm({ ...form, ssl_ca_cert: v })}
                placeholder="-----BEGIN CERTIFICATE-----"
                hint={
                  form.ssl_mode.startsWith("verify")
                    ? "required for certificate verification"
                    : "optional — root CA for server verification"
                }
                rows={3}
              />
              <FormTextArea
                label="client certificate (pem)"
                value={form.ssl_client_cert}
                onChange={(v) => setForm({ ...form, ssl_client_cert: v })}
                placeholder="-----BEGIN CERTIFICATE-----"
                hint="optional — for mutual TLS (mTLS) authentication"
                rows={3}
              />
              <FormTextArea
                label="client key (pem)"
                value={form.ssl_client_key}
                onChange={(v) => setForm({ ...form, ssl_client_key: v })}
                placeholder="-----BEGIN PRIVATE KEY-----"
                hint="required when using client certificate"
                rows={3}
                className="col-span-2"
              />
            </>
      ...
      {form.ssh_enabled && (
        <div className="grid grid-cols-2 gap-4 mt-3 animate-fade-in">
          ...
          <FormInput
            ...
            onChange={(v) => setForm({ ...form, ssh_username: v })}
            placeholder="ubuntu"
            required
          />
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setForm({ ...form, ssh_auth_method: e.target.value })
              }
              className="w-full px-3 py-2 bg-[var(--color-bg-input)] border border-[var(--color-border)] text-xs focus:outline-none focus:border-[var(--color-text-dim)]"
            >
          ...
          {form.ssh_auth_method === "password" && (
            <FormInput
              ...
              onChange={(v) => setForm({ ...form, ssh_password: v })}
              type="password"
              className="col-span-2"
            />
          )}
          {form.ssh_auth_method === "key" && (
            <>
              <FormTextArea
                ...
                onChange={(v) => setForm({ ...form, ssh_private_key: v })}
              ...
              <FormInput
                ...
                onChange={(v) => setForm({ ...form, ssh_key_passphrase: v })}
                type="password"
                hint="leave empty if key is not encrypted"
                className="col-span-2"
              />
            </>
          )}
          {form.ssh_auth_method === "agent" && (
            <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed">
              <p className="text-[11px] text-[var(--color-text-dim)] tracking-wider">
                uses the ssh-agent running on the signalpilot server. ensure{" "}
                <code className="text-[var(--color-text-muted)]">
                  SSH_AUTH_SOCK
                </code>{" "}
                is set and your key is loaded with{" "}
                <code className="text-[var(--color-text-muted)]">ssh-add</code>.
              </p>
            </div>
          )}
          ...
          <div className="col-span-2 border-t border-[var(--color-border)]/50 pt-3 mt-1">
            <button
              type="button"
              onClick={() =>
                setForm({ ...form, ssh_proxy_enabled: !form.ssh_proxy_enabled })
              }
            ...
            {form.ssh_proxy_enabled && (
              <div className="grid grid-cols-2 gap-3 animate-fade-in">
                <FormInput
                  label="proxy host"
                  value={form.ssh_proxy_host}
                  onChange={(v) => setForm({ ...form, ssh_proxy_host: v })}
                  placeholder="proxy.corp.example.com"
                  hint="HTTP CONNECT proxy (e.g. Squid)"
                  required
                />
                <FormInput
                  label="proxy port"
                  value={form.ssh_proxy_port}
                  onChange={(v) => setForm({ ...form, ssh_proxy_port: v })}
                  placeholder="3128"
                  hint="default: 3128 (Squid)"
                />
                <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed text-[11px] text-[var(--color-text-dim)] tracking-wider">
                  routes ssh through an http connect proxy. use this when your
                  vpc or corporate network blocks direct ssh connections to the
                  bastion host. requires{" "}
                  <code className="text-[var(--color-text-muted)]">socat</code>{" "}
                  on the signalpilot server.
                </div>
              </div>
            )}
          </div>
          ...
          <div className="col-span-2 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed">
            <p className="text-[11px] text-[var(--color-text-dim)] tracking-wider">
              signalpilot creates an on-demand ssh tunnel to your database
              through this bastion host.
              {serverIp ? (
                <>
                  {" "}
                  whitelist{" "}
...
export default function ConnectionsPage() {
  ...
  const [healthData, setHealthData] = useState<
    Record<string, ConnectionHealthStats>
  >({});
  ...
  const [schemaSearchResults, setSchemaSearchResults] = useState<
    ...
    >
  >({});
  const [schemaSearchLoading, setSchemaSearchLoading] = useState<string | null>(
    null,
  );
  const [endorsements, setEndorsements] = useState<
    Record<
      string,
      { endorsed: string[]; hidden: string[]; mode: "all" | "endorsed_only" }
    >
  >({});
  ...
  const [diagResults, setDiagResults] = useState<
    ...
    >
  >({});
  const [schemaRefreshStatus, setSchemaRefreshStatus] = useState<
    Record<
      string,
      {
    ...
    getConnections()
      .then((conns) => {
        ...
        for (const conn of conns) {
          getConnectionHealthHistory(conn.name, 3600, 120)
            .then((res) => {
    ...
    try {
      ...
      toast(
        result.status === "healthy"
          ? `${connName}: connection healthy`
          : `${connName}: ${result.message}`,
  ...
  async function handlePreTest() {
    ...
    try {
      ...
      if (result.status === "healthy") {
        ...
        const failedPhase = result.phases?.find(
  ...
  function handleEditConnection(conn: ConnectionInfo) {
    const connConfig =
      DB_CONFIGS[conn.db_type as DBType] || DB_CONFIGS.postgres;
    setForm({
      ...defaultForm,
      name: conn.name,
      ...
      bq_max_bytes_billed: (conn as any).maximum_bytes_billed
        ? String((conn as any).maximum_bytes_billed)
        : "",
    ...
    const hasCustomTimeouts =
      (conn.connection_timeout && conn.connection_timeout !== 15) ||
      (conn.query_timeout && conn.query_timeout !== 120) ||
      (conn.keepalive_interval && conn.keepalive_interval > 0);
    setShowAdvanced(
      !!(
        conn.ssl ||
        conn.ssh_tunnel?.enabled ||
        conn.schema_refresh_interval ||
        conn.schema_filter_include?.length ||
        conn.schema_filter_exclude?.length ||
        hasCustomTimeouts
      ),
    );
  }
  ...
  async function handleGenerateSemantic(name: string) {
    try {
      ...
      toast(
        `${name}: semantic model generated — ${result.joins} joins, ${result.glossary_terms} glossary terms`,
        "success",
      );
    } catch (e) {
      toast(
        `${name}: semantic model generation failed — ${String(e)}`,
        "error",
      );
    }
  }
  ...
  async function handleDiagnose(name: string) {
    ...
    try {
      ...
      const allOk = result.diagnostics.every(
        (d: { status: string }) => d.status === "ok",
      );
      toast(
        allOk
          ? `${name}: all checks passed`
          : `${name}: see diagnostic results`,
        allOk ? "success" : "info",
      );
    } catch (e) {
  ...
  async function handleClone(name: string) {
    ...
    try {
      ...
      toast(
        `Clone failed: ${err.message?.slice(0, 80) || "unknown error"}`,
        "error",
      );
    }
  }
  ...
  async function handleToggleSchema(name: string) {
    ...
    getSchemaRefreshStatus(name)
      .then((status) =>
        setSchemaRefreshStatus((prev) => ({ ...prev, [name]: status })),
      )
      .catch(() => {});
    ...
    getConnectionSchemaDiff(name)
      .then((diff) => {
        if (diff.diff)
          setSchemaDiff((prev) => ({ ...prev, [name]: diff.diff as any }));
      })
      .catch(() => {});
  }
  ...
  async function handleScanPII(name: string) {
    ...
    try {
      ...
      setPiiData((prev) => ({
        ...prev,
        [name]: { tables_scanned: 0, tables_with_pii: 0, detections: {} },
      }));
    } finally {
      setPiiLoading(null);
    }
  ...
  function handleSchemaSearch(name: string, query: string) {
    ...
    searchTimerRef.current[name] = setTimeout(async () => {
      ...
      try {
        ...
        setSchemaSearchResults((prev) => ({
          ...prev,
          [name]: { result_count: 0, total_tables: 0, tables: {} },
        }));
      } finally {
  ...
  async function handleImportFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (result.skipped.length)
        msg.push(`Skipped (existing): ${result.skipped.join(", ")}`);
    ...
    <PullToRefreshWrapper onRefresh={refresh}>
      <div className="p-4 sm:p-8 animate-fade-in">
        ...
        <TerminalBar
          ...
          status={
            <StatusDot
              status={connections.length > 0 ? "healthy" : "unknown"}
              size={4}
            />
          }
        >
        ...
        {showForm && (
          <div className="mb-6 border border-[var(--color-border)] bg-[var(--color-bg-card)] animate-scale-in overflow-hidden">
            ...
            <div className="p-6">
                  ...
                  {DB_TYPE_ORDER.map((dbType) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {cfg.label}
                        <span
                          className={`text-[10px] opacity-60 ${CONNECTOR_TIERS[dbType]?.tier === 1 ? "text-emerald-400" : CONNECTOR_TIERS[dbType]?.tier === 2 ? "text-sky-400" : "text-zinc-400"}`}
                        >
              ...
              {!editingConnection && (
                <div className="mb-4">
                  <label className="block text-[12px] text-[var(--color-text-dim)] mb-1.5 tracking-wider opacity-60">
                    quick start
                  </label>
                  <div className="flex flex-wrap gap-1.5">
                    {[...CONNECTION_PRESETS]
                        ...
                        <button
                          ...
                          onClick={() => {
                            const updates = {
                              ...
                              port:
                                preset.defaults.port ||
                                String(DB_CONFIGS[preset.db_type].defaultPort),
                            };
                            setForm({ ...form, ...updates } as FormState);
                          }}
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                {editingConnection ? (
                  <div>
                    ...
                    <div className="px-3 py-2 bg-[var(--color-bg-hover)] border border-[var(--color-border)] text-xs text-[var(--color-text-dim)] tracking-wide">
                      {editingConnection}
                    </div>
                  </div>
                ) : (
                  <FormInput
                    ...
                    required
                    error={form.name.length > 0 ? formErrors.name : undefined}
                  />
                )}
                <FormInput
                  label="description"
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {form.tags.map((tag) => (
                    <span
                      key={tag}
                      className="flex items-center gap-1 px-2 py-0.5 text-[11px] bg-[var(--color-bg-hover)] border border-[var(--color-border)] text-[var(--color-text-dim)] tracking-wider"
                    >
                      {tag}
                      <button
                        type="button"
                        ...
                        className="text-[var(--color-text-dim)] hover:text-[var(--color-error)] ml-0.5"
                      >
                        &times;
                      </button>
                    </span>
                ...
                <p className="text-[11px] text-[var(--color-text-dim)] mt-1 tracking-wider opacity-60">
                  press enter or comma to add — organize connections by
                  environment, team, or purpose
                </p>
              </div>
                      ...
                      onClick={() => {
                        ...
                        if (mode === "url") {
                          ...
                          setForm({
                            ...form,
                            connectionMode: "url",
                            connection_string: preview.replace(
                              ":****@",
                              `:${form.password || ""}@`,
                            ),
                          });
                ...
                <div className="mb-4 px-3 py-2 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed">
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const fullUrl = buildConnectionPreview({
                          ...form,
                          connectionMode: "fields",
                        }).replace(":****@", `:${form.password || ""}@`);
                        navigator.clipboard
                          .writeText(fullUrl)
                          .then(() => toast("Connection URL copied", "info"));
                      }}
                  ...
                  <code className="text-[12px] text-[var(--color-text-muted)] tracking-wide break-all">
                    {buildConnectionPreview(form)}
                  </code>
                </div>
              )}
              ...
              <div>
                ...
                >
                  ...
                  {(form.ssl_enabled ||
                    ...
                    form.connection_timeout !== "15" ||
                    form.query_timeout !== "120") && (
                    <span className="text-[var(--color-success)] text-[11px] ml-1">
                      {[
                        form.ssl_enabled && "ssl",
                        form.ssh_enabled && "ssh",
                        !form.read_only && "read-write",
                        form.schema_refresh_enabled && "auto-refresh",
                        (form.connection_timeout !== "15" ||
                          form.query_timeout !== "120") &&
                          "custom timeouts",
                      ]
                ...
                {showAdvanced && (
                  <div className="animate-fade-in">
                    ...
                    <div className="flex gap-0 mb-4 border-b border-[var(--color-border)]">
                      {(["security", "performance", "schema"] as const).map(
                        (tab) => {
                          const tabIcons = {
                            security: (
                              <Lock className="w-3 h-3" strokeWidth={1.5} />
                          ...
                          const tabBadges = {
                            ...
                            performance:
                              form.connection_timeout !== "15" ||
                              form.query_timeout !== "120" ||
                              form.keepalive_interval !== "0",
                            schema:
                              form.schema_refresh_enabled ||
                              form.schema_filter_include.trim() !== "" ||
                              form.schema_filter_exclude.trim() !== "",
                          };
                          return (
                            <button
                              key={tab}
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-[10px] text-[var(--color-text-dim)] mt-1 tracking-wider opacity-60">
                                workspace connections are shared across all
                                projects
                              </p>
                            </div>
                            <div>
                        ...
                        <div className="border-t border-[var(--color-border)] pt-4 mt-4">
                          ...
                          <div className="px-3 py-2.5 bg-[var(--color-bg)]/50 border border-[var(--color-border)] border-dashed">
                            <p className="text-[11px] text-[var(--color-text-dim)] tracking-wider mb-1.5">
                              ...
                              this signalpilot server ip to your firewall rules:
                            </p>
                            <div className="flex flex-wrap gap-2">
                            ...
                            <p className="text-[10px] text-[var(--color-text-dim)] tracking-wider mt-1.5 opacity-60">
                              {serverIp
                                ? "add this ip to your database firewall, security group, or network policy."
                                : "fetching server ip..."}
                            </p>
                          </div>
                        </div>
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-1.5">
                                <select
                                  ...
                                  className="bg-[var(--color-bg)] border border-[var(--color-border)] text-[var(--color-text)] text-[12px] px-2 py-2 tracking-wider"
                                >
                              ...
                              <p className="text-[10px] text-[var(--color-text-dim)] mt-1 tracking-wider opacity-60">
                                ping to prevent idle disconnect
                              </p>
                            </div>
                          </div>
                      ...
                      <div className="animate-fade-in">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-[var(--color-text-dim)] tracking-wider mb-3 opacity-60">
                            filter which schemas are visible to the ai agent.
                            excludes staging, dev, and raw schemas to improve
                            accuracy.
                          </div>
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setForm({
                                    ...form,
                                    schema_filter_include: e.target.value,
                                  })
                                }
                                className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] text-[var(--color-text)] text-[12px] px-3 py-2 tracking-wider placeholder:text-[var(--color-text-dim)]"
                              />
                            </div>
                            <div>
                              <label className="block text-[11px] text-[var(--color-text-muted)] tracking-wider mb-1">
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setForm({
                                    ...form,
                                    schema_filter_exclude: e.target.value,
                                  })
                                }
                                className="w-full bg-[var(--color-bg)] border border-[var(--color-border)] text-[var(--color-text)] text-[12px] px-3 py-2 tracking-wider placeholder:text-[var(--color-text-dim)]"
                              />
                            ...
                            <label className="flex items-center gap-2 cursor-pointer">
                              <input
                                ...
                                onChange={(e) =>
                                  setForm({
                                    ...form,
                                    schema_refresh_enabled: e.target.checked,
                                  })
                                }
                                className="accent-[var(--color-text)]"
                              />
                            ...
                            <div className="flex items-center gap-2 animate-fade-in">
                              ...
                              <select
                                ...
                                onChange={(e) =>
                                  setForm({
                                    ...form,
                                    schema_refresh_interval: e.target.value,
                                  })
                                }
                                className="bg-[var(--color-bg)] border border-[var(--color-border)] text-[var(--color-text)] text-[12px] px-2 py-1 tracking-wider"
                              >
                                <option value="60">1 min</option>
                                <option value="300">5 min</option>
                              ...
                              <span className="text-[10px] text-[var(--color-text-dim)] tracking-wider opacity-60">
              ...
              {preTestResult && (
                <div
                  className={`mt-4 p-3 border ${preTestResult.status === "healthy" ? "border-emerald-500/30 bg-emerald-500/5" : "border-red-500/30 bg-red-500/5"}`}
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span
                      className={`text-[12px] tracking-wider font-medium ${preTestResult.status === "healthy" ? "text-emerald-400" : "text-red-400"}`}
                    >
                    ...
                    <div className="space-y-1 ml-5">
                      {preTestResult.phases.map((phase, i) => (
                        ...
                        >
                          ...
                          {phase.duration_ms !== undefined && (
                            <span className="text-[var(--color-text-dim)] opacity-50">
                      ...
                      {preTestResult.phases.some((p) => p.hint) && (
                        <div className="mt-1.5 pl-2 border-l border-amber-500/30">
                          {preTestResult.phases
                            .filter((p) => p.hint)
                            .map((p, i) => (
                              <div
                                key={i}
                                className="text-[11px] text-amber-400/80 tracking-wider"
                              >
                                hint: {p.hint}
                              </div>
                            ))}
              ...
              {(() => {
                ...
                if (
                  cfg.supportsSSL &&
                  !form.ssl_enabled &&
                  !["duckdb", "sqlite"].includes(form.db_type)
                ) {
                  warnings.push(
                    "SSL/TLS is not enabled. Recommended for production databases to encrypt traffic in transit.",
                  );
                }
                ...
                if (
                  ...
                  warnings.push(
                    "Snowflake recommends key-pair authentication over password. Password auth may be blocked by Snowflake's mandatory MFA policy.",
                  );
                }
                // Warn about read-write mode
                if (!form.read_only) {
                  warnings.push(
                    "Read-write mode enabled. This allows INSERT, UPDATE, DELETE, and DDL queries. Use read-only for analytics workloads.",
                  );
                }
                // Warn about missing schema filtering for large warehouses
                if (
                  ["snowflake", "bigquery", "redshift", "databricks"].includes(
                    form.db_type,
                  ) &&
                  !form.schema_filter_include.trim() &&
                  !form.schema_filter_exclude.trim()
                  ...
                  warnings.push(
                    "No schema filtering configured. For large warehouses, filtering schemas improves AI agent accuracy and reduces metadata overhead.",
                  );
                }
                ...
                return (
                  <div className="mt-4 space-y-1.5">
                    {warnings.map((w, i) => (
                      <div
                        key={i}
                        className="flex items-start gap-2 px-3 py-2 border border-amber-500/20 bg-amber-500/5"
                      >
                        <AlertTriangle
                          className="w-3 h-3 text-amber-400 flex-shrink-0 mt-0.5"
                          strokeWidth={1.5}
                        />
              ...
              <div className="flex items-center gap-3 mt-5 pt-4 border-t border-[var(--color-border)]">
                <button
                  ...
                  disabled={
                    saving || preTesting || (!editingConnection && !form.name)
                  }
                ...
                <button
                  ...
                  disabled={saving || preTesting}
                  className="flex items-center gap-2 px-4 py-2 border border-emerald-500/30 text-xs text-emerald-400 hover:bg-emerald-500/10 hover:border-emerald-500/50 transition-all tracking-wider"
                >
                  {preTesting ? (
                ...
                <button
                  ...
                  disabled={
                    saving ||
                    preTesting ||
                    (!editingConnection && !form.name) ||
                    hasFormErrors
                  }
                  className="flex items-center gap-2 px-4 py-2 border border-[var(--color-border)] text-xs text-[var(--color-text-dim)] hover:text-[var(--color-text)] hover:border-[var(--color-border-hover)] transition-all tracking-wider disabled:opacity-40 disabled:cursor-not-allowed"
                >
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {filterTag && (
                    <button
                      ...
                      className="text-[11px] text-[var(--color-text-dim)] hover:text-[var(--color-text)] tracking-wider ml-1"
                    >
                      clear
                    </button>
                  )}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 p-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Tooltip
                            content={`Tier ${CONNECTOR_TIERS[conn.db_type as DBType]?.tier || 3}: ${CONNECTOR_TIERS[conn.db_type as DBType]?.tier === 1 ? "Full support" : CONNECTOR_TIERS[conn.db_type as DBType]?.tier === 2 ? "Stable" : "Basic"}`}
                            position="top"
                          >
                            <span
                              className={`text-[11px] px-1 py-0.5 border tracking-wider cursor-default ${CONNECTOR_TIERS[conn.db_type as DBType]?.color || "text-zinc-400 border-zinc-500/30"}`}
                            >
                              {CONNECTOR_TIERS[conn.db_type as DBType]?.label ||
                                "T3"}
                            </span>
                          </Tooltip>
                          {conn.ssl && (
                            <span className="text-[11px] px-1 py-0.5 border border-[var(--color-success)]/30 text-[var(--color-success)] tracking-wider">
                              ssl
                            </span>
                          )}
                          {conn.ssh_tunnel?.enabled && (
                            <span className="text-[11px] px-1 py-0.5 border border-purple-500/30 text-purple-400 tracking-wider">
                              ssh
                            </span>
                          )}
                          {conn.tags?.map((tag) => (
                            <span
                              key={tag}
                              className="text-[11px] px-1 py-0.5 border border-blue-500/30 text-blue-400 tracking-wider"
                            >
                              {tag}
                            </span>
                          ))}
                        ...
                        <div className="text-[12px] text-[var(--color-text-dim)] mt-0.5 tracking-wider">
                          ...
                          {conn.last_used && (
                            <span
                              className="ml-2 text-[var(--color-text-dim)] opacity-60"
                              title={new Date(
                                conn.last_used * 1000,
                              ).toLocaleString()}
                            >
                        ...
                        {health && health.sample_count > 0 && (
                          <div className="flex flex-wrap items-center gap-2 sm:gap-4 mt-1.5 text-[11px] text-[var(--color-text-dim)] tracking-wider">
                            ...
                            {health.latency_p50_ms != null && (
                              <Tooltip
                                content={`p50: ${health.latency_p50_ms.toFixed(1)}ms${health.latency_p95_ms ? ` · p95: ${health.latency_p95_ms.toFixed(1)}ms` : ""}`}
                                position="top"
                              >
                                <span className="flex items-center gap-1.5 tabular-nums cursor-default">
                                  ...
                                  <MiniBar
                                    ...
                                    height={3}
                                    color={
                                      health.latency_p50_ms < 50
                                        ? "var(--color-success)"
                                        : health.latency_p50_ms < 150
                                          ? "var(--color-warning)"
                                          : "var(--color-error)"
                                    }
                                  />
                            ...
                            {healthHistory[conn.name] &&
                              healthHistory[conn.name].length >= 2 && (
                                <Tooltip
                                  content="latency trend (1h)"
                                  position="top"
                                >
                      ...
                      {testResult[conn.name] && (
                        <div className="flex flex-col gap-0.5">
                          <span
                            className={`flex items-center gap-1.5 text-[12px] tracking-wider ${
                              testResult[conn.name].status === "healthy"
                                ? "text-[var(--color-success)]"
                                : testResult[conn.name].status === "warning"
                                  ? "text-[var(--color-warning)]"
                                  : "text-[var(--color-error)]"
                            }`}
                          >
                              ...
                              <span className="flex items-center gap-2">
                                {testResult[conn.name].phases!.map((p, i) => {
                                  ...
                                  return (
                                    <Tooltip
                                      key={i}
                                      content={p.message || phaseLabel}
                                      position="top"
                                    >
                                      <span
                                        className={`${statusColor} cursor-default tabular-nums`}
                                      >
                                        {phaseLabel}
                                        {statusIcon}
                                        {p.duration_ms
                                ...
                                {testResult[conn.name].total_duration_ms !=
                                  null && (
                                  <span className="text-[11px] text-[var(--color-text-dim)] tabular-nums">
                                    total:{" "}
                                    {testResult[conn.name].total_duration_ms}ms
                                  </span>
                                )}
                              </span>
                            ) : (
                              <Tooltip
                                content={testResult[conn.name].message}
                                position="top"
                              >
                                <span className="cursor-default">
                                  {testResult[conn.name].message.slice(0, 50)}
                                </span>
                              </Tooltip>
                            )}
                          </span>
                      ...
                      <div className="flex flex-wrap items-center gap-2 sm:gap-1 w-full sm:w-auto border-t sm:border-t-0 border-[var(--color-border)]/30 pt-2 sm:pt-0">
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handleScanPII(conn.name);
                          }}
                          disabled={piiLoading === conn.name}
                          className="flex items-center gap-1.5 px-2.5 py-2 sm:py-1.5 text-[12px] text-[var(--color-text-dim)] hover:text-[var(--color-text)] hover:bg-[var(--color-bg-hover)] transition-all tracking-wider"
                        >
                          {piiLoading === conn.name ? (
                          ...
                          pii
                          {piiData[conn.name] &&
                            piiData[conn.name].tables_with_pii > 0 && (
                        ...
                        >
                          {testing === conn.name ? (
                            ...
                            <TestTube className="w-3 h-3" strokeWidth={1.5} />
                          )}
                          test
                        </button>
                        <button
                          onClick={() => handleDiagnose(conn.name)}
                          disabled={diagnosing === conn.name}
                        ...
                        {schemaLoading === conn.name ? (
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center gap-3 mb-3">
                              ...
                              {(() => {
                                const displayTables =
                                  schemaSearchResults[conn.name]?.tables ||
                                  tables;
                                const totalFKs = Object.values(
                                  displayTables,
                                ).reduce(
                                  (sum: number, t: any) =>
                                    sum + (t.foreign_keys?.length || 0),
                                  0,
                                );
                                const totalIdxs = Object.values(
                                  displayTables,
                                ).reduce(
                                  (sum: number, t: any) =>
                                    sum + (t.indexes?.length || 0),
                                  0,
                                );
                                return (
                                  <>
                              ...
                              {schemaRefreshStatus[conn.name] && (
                                <>
                                  <Tooltip
                                    content={`Structural fingerprint: ${schemaRefreshStatus[conn.name].fingerprint || "n/a"}`}
                                    position="top"
                                  >
                                  ...
                                  {schemaRefreshStatus[conn.name]
                                    .last_schema_refresh && (
                                    <Tooltip
                                      content={`Last refresh: ${new Date(schemaRefreshStatus[conn.name].last_schema_refresh! * 1000).toLocaleString()}`}
                                      position="top"
                                    >
                              ...
                              {schemaDiff[conn.name] &&
                                schemaDiff[conn.name]?.has_changes && (
                                  <Tooltip
                                    content={`Changes: +${(schemaDiff[conn.name] as any).added_tables?.length || 0} added, -${(schemaDiff[conn.name] as any).removed_tables?.length || 0} removed, ~${(schemaDiff[conn.name] as any).modified_tables?.length || 0} modified`}
                                    position="top"
                                  >
                              ...
                              <button
                                onClick={async () => {
                                  const current = endorsements[conn.name] || {
                                    endorsed: [],
                                    hidden: [],
                                    mode: "all" as const,
                                  };
                                  ...
                                  try {
                                    ...
                                    toast(
                                      `Schema filter: ${nextMode === "endorsed_only" ? "endorsed only" : "all tables"}`,
                                      "success",
                                    );
                                  } catch {
                                    toast(
                                      "Failed to update endorsements",
                                      "error",
                                    );
                                  }
                                }}
                                className={`px-2 py-0.5 text-[11px] border tracking-wider transition-all ${
                                  endorsements[conn.name]?.mode ===
                                  "endorsed_only"
                                ...
                                onClick={async () => {
                                  ...
                                  try {
                                    ...
                                    getConnectionSchemaDiff(conn.name)
                                      ...
                                      .catch(() => {});
                                    toast(
                                      `${conn.name}: schema refreshed`,
                                      "success",
                                    );
                                  } catch {
                                    toast(
                                      `${conn.name}: refresh failed`,
                                      "error",
                                    );
                                  } finally {
                                    setSchemaLoading(null);
                              ...
                              >
                                <RefreshCw
                                  className={`w-2.5 h-2.5 ${schemaLoading === conn.name ? "animate-spin" : ""}`}
                                  strokeWidth={1.5}
                                />
                                refresh
                              </button>
                              ...
                              <div className="relative">
                                <Search
                                  className="absolute left-2 top-1/2 -translate-y-1/2 w-3 h-3 text-[var(--color-text-dim)]"
                                  strokeWidth={1.5}
                                />
                                <input
                                  type="text"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-px max-h-96 overflow-auto bg-[var(--color-border)]">
                              {Object.entries(
                                ...
                                >
                                  <div className="flex items-center gap-2 mb-2">
                                    ...
                                    {t.row_count > 0 && (
                                      <span className="text-[11px] text-[var(--color-text-dim)] tabular-nums tracking-wider">
                                        ~
                                        {t.row_count >= 1000000
                                          ? `${(t.row_count / 1000000).toFixed(1)}M`
                                          : t.row_count >= 1000
                                    ...
                                    <button
                                      onClick={async (e) => {
                                        ...
                                        const isEndorsed =
                                          current.endorsed.includes(key);
                                        const isHidden =
                                          current.hidden.includes(key);
                                        let updated = { ...current };
                                        if (isEndorsed) {
                                          updated.endorsed =
                                            current.endorsed.filter(
                                              (k) => k !== key,
                                            );
                                        } else if (isHidden) {
                                          updated.hidden =
                                      ...
                                      title={
                                        endorsements[
                                          ...
                                          ? "Endorsed — click to remove"
                                          : "Click to endorse for AI agents"
                                      }
                                    >
                                      <Star
                                        className="w-2.5 h-2.5"
                                  ...
                                  {t.description && (
                                    <p className="text-[10px] text-[var(--color-text-dim)] tracking-wider opacity-70 mb-1.5 italic">
                                      {t.description}
                                    </p>
                                  )}
                                      ...
                                      .map((col: any) => {
                                        ...
                                        return (
                                          ...
                                          >
                                            ...
                                            {col.comment && (
                                              <span
                                                className="text-[var(--color-text-dim)] opacity-40 text-[10px] italic truncate max-w-[120px]"
                                                title={col.comment}
                                              >
                                                {col.comment}
                                              </span>
                                    ...
                                    <div className="mt-2 pt-1.5 border-t border-[var(--color-border)]">
                                      {t.foreign_keys.map(
                                        (fk: any, i: number) => (
                                          <div
                                            key={i}
                                            className="text-[10px] text-purple-400/80 tracking-wider"
                                          >
                                  ...
                                  <div className="mt-2 pt-1.5 border-t border-[var(--color-border)]">
                                    <button
                                      onClick={async (e) => {
                                        ...
                                        if (exploredData[exploreKey]) {
                                          setExploredData((prev) => {
                                            const next = { ...prev };
                                            delete next[exploreKey];
                                            return next;
                                          });
                                          return;
                                        }
                                        ...
                                        try {
                                          const data = await exploreColumns(
                                            conn.name,
                                            key,
                                          );
                                          setExploredData((prev) => ({
                                    ...
                                    >
                                      ...
                                      `${conn.name}:${key}` ? (
                                        <>
                                          <Loader2 className="w-2.5 h-2.5 inline animate-spin mr-0.5" />{" "}
                                          exploring...
                                        </>
                                      ) : exploredData[
                                          `${conn.name}:${key}`
                                            ...
                                            >
                                              ...
                                              {ec.value_stats && (
                                                <span className="text-[var(--color-text-dim)] ml-1.5">
                                                  [{ec.value_stats.min} ..{" "}
                                                  {ec.value_stats.max}]
                                                  {ec.value_stats.avg != null &&
                                                    ` avg=${ec.value_stats.avg}`}
                                                </span>
                                              )}
                                              {ec.sample_values &&
                                                ec.sample_values.length > 0 && (
                                                  <span className="text-[var(--color-text-dim)] opacity-60 ml-1.5">
                                                    {ec.sample_values
                                                      .slice(0, 4)
                                                      .join(", ")}
                                                    {ec.sample_values.length >
                                                      4 && "..."}
                                                  </span>
                                                )}
                                            </div>
                      ...
                      piiData[conn.name].tables_with_pii > 0 && (
                        <div className="border-t border-[var(--color-border)] px-4 py-4 animate-fade-in">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            />
                            <span className="text-[12px] text-[var(--color-text-muted)] tracking-wider">
                              pii detected: {piiData[conn.name].tables_with_pii}{" "}
                              table
                              {piiData[conn.name].tables_with_pii > 1
                                ? "s"
                                : ""}
                            </span>
                          </div>
                          <div className="space-y-2">
                            {Object.entries(piiData[conn.name].detections).map(
                              ([table, columns]) => (
                                <div
                                  key={table}
                                  className="p-3 border border-[var(--color-warning)]/20 bg-[var(--color-warning)]/5"
                                >
                                  <p className="text-[12px] text-[var(--color-text-muted)] mb-1.5 tracking-wider">
                                    {table}
                                  </p>
                                  <div className="flex flex-wrap gap-2">
                                    {Object.entries(columns).map(
                                      ([col, rule]) => (
                                        <span
                                          key={col}
                                          className={`text-[11px] px-1.5 py-0.5 border tracking-wider uppercase ${
                                            rule === "drop"
                                              ? "badge-error"
                                              : rule === "hash"
                          ...
                          <p className="text-[11px] text-[var(--color-text-dim)] mt-2 tracking-wider">
                            add these rules to schema.yml annotations for
