  ...
  return (
    <div className="bg-[var(--color-bg-card)] p-3 sm:p-5 hover:bg-[var(--color-bg-hover)] transition-all card-glow card-accent-top group relative overflow-hidden">
      <div className="flex items-center gap-2 mb-3">
        <Icon
          className={`w-4 h-4 ${accentColor || "text-[var(--color-text-dim)]"} transition-transform group-hover:scale-110`}
          strokeWidth={1.5}
        />
      ...
      <p className="text-2xl font-light metric-value text-[var(--color-text)] animate-count-up">
        {value}
      </p>
      {subtext && (
      ...
      {sparkValues && sparkValues.length >= 3 && (
        <div className="absolute bottom-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">
          <Sparkline
            ...
            height={24}
            color={
              accentColor?.includes("success")
                ? "var(--color-success)"
                : accentColor?.includes("error")
                  ? "var(--color-error)"
                  : "var(--color-text-dim)"
            }
            fillOpacity={0.08}
...
function StatusBadge({ ok }: { ok: boolean | null }) {
  if (ok === null)
    return (
      <Loader2 className="w-3 h-3 animate-spin text-[var(--color-text-dim)]" />
    );
  return ok ? (
      ...
      <div className="p-4 sm:p-8 max-w-[1400px] animate-fade-in mx-auto">
        ...
        <TerminalBar
          ...
          status={
            <StatusDot
              status={
                metrics?.sandbox_health === "healthy" && metrics?.kvm_available
                  ? "healthy"
                  : metrics
                    ? "error"
                    : "unknown"
        ...
        >
          <div className="flex flex-wrap items-center gap-x-3 sm:gap-x-6 gap-y-2 text-xs">
            <div className="flex items-center gap-2">
              ...
              <StatusBadge
                ok={metrics ? metrics.sandbox_health === "healthy" : null}
              />
            </div>
            <div className="flex items-center gap-2">
              <Cpu
            ...
            <div
              className={`${latencyValues.length <= 3 ? "sm:ml-auto" : ""} flex items-center gap-2`}
            >
              <Shield
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-px mb-px bg-[var(--color-border)] border border-[var(--color-border)] stagger-fade-in">
          <MetricCard
            ...
            subtext={
              metrics ? `${metrics.running_sandboxes} running` : undefined
            }
            icon={Terminal}
          />
          <MetricCard
          ...
          <MetricCard
            label="total spent"
            value={
              budgetData ? `$${budgetData.total_spent_usd.toFixed(4)}` : "$0.00"
            }
            subtext={
              budgetData ? `${budgetData.sessions.length} sessions` : undefined
            }
            icon={DollarSign}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-px mb-8 bg-[var(--color-border)] stagger-fade-in">
          ...
          <MetricCard
            ...
            icon={Shield}
            accentColor={
              auditStats.blocks > 0 ? "text-[var(--color-error)]" : undefined
            }
          />
          <MetricCard
            label="avg latency"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="sm:col-span-2 border border-[var(--color-border)] bg-[var(--color-bg-card)] card-radial-glow">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)]">
              ...
              <a
                href="/audit"
                className="flex items-center gap-1 text-[12px] text-[var(--color-text-dim)] hover:text-[var(--color-text)] transition-colors tracking-wider py-1 px-2 -mr-2 active:text-[var(--color-text)]"
              >
            ...
            <div className="divide-y divide-[var(--color-border)]">
              {recentAudit.length === 0 ? (
                ...
                recentAudit.slice(0, 12).map((entry) => {
                  const cfg =
                    eventTypeConfig[entry.event_type] || eventTypeConfig.query;
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        setExpandedActivity(isExpanded ? null : entry.id)
                      }
                    >
                      <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-y-1 sm:gap-x-3 px-4 py-3 sm:py-2.5">
                        <div className="flex items-center gap-2 sm:contents min-w-0">
                          <span
                            className={`text-[11px] font-medium uppercase tracking-[0.15em] w-8 flex-shrink-0 ${
                              entry.blocked
                          ...
                          <span className="flex-1 text-xs truncate overflow-hidden min-w-0 sm:basis-[120px]">
                            {entry.sql ? (
                              <SqlHighlight
                                sql={entry.sql.slice(0, 80)}
                                className="text-xs"
                              />
                            ) : (
                              <span className="text-[var(--color-text-muted)]">
                                {entry.metadata?.code_preview
                                  ? String(entry.metadata.code_preview).slice(
                                      0,
                                      80,
                                    )
                      ...
                      <div
                        className={`grid grid-cols-[2rem_1fr] gap-3 px-4 overflow-hidden transition-all duration-200 ease-out ${
                          isExpanded
                        ...
                        <div className="flex items-center gap-4 text-[11px] text-[var(--color-text-dim)] tracking-wider">
                          ...
                          {entry.metadata?.governance_latency_ms != null && (
                            <span>
                              gov:{" "}
                              {Number(
                                entry.metadata.governance_latency_ms,
                              ).toFixed(1)}
                              ms
                            </span>
                          )}
              ...
              <div className="divide-y divide-[var(--color-border)]">
                {connections.length === 0 ? (
                  <div className="px-4 py-10 text-center">
                    <Database
                      className="w-5 h-5 mx-auto mb-2 text-[var(--color-text-dim)] opacity-20"
                      strokeWidth={1}
                    />
                  ...
                  <>
                    <div className="flex items-center gap-3">
                      <RingGauge
                        ...
                        strokeWidth={3}
                        color={
                          cacheStats.hit_rate > 0.7
                            ? "var(--color-success)"
                            : cacheStats.hit_rate > 0.3
                              ? "var(--color-warning)"
                              : "var(--color-error)"
                        }
                      />
