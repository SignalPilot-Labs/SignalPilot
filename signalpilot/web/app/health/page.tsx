...
function LatencyBar({
  ...
  return (
    <Tooltip
      content={`${label}: ${value.toFixed(2)}ms (${pct.toFixed(0)}% of ${maxMs}ms budget)`}
      position="top"
    >
      <div className="flex items-center gap-3 cursor-default">
        <span className="text-[11px] text-[var(--color-text-dim)] w-8 text-right uppercase tracking-[0.15em]">
          {label}
        </span>
        <div className="flex-1 h-1.5 bg-[var(--color-bg)] overflow-hidden relative">
          ...
          {/* Threshold markers */}
          <div
            className="absolute top-0 h-full w-px bg-[var(--color-text-dim)] opacity-20"
            style={{ left: `${(50 / maxMs) * 100}%` }}
          />
          <div
            className="absolute top-0 h-full w-px bg-[var(--color-text-dim)] opacity-20"
            style={{ left: `${(200 / maxMs) * 100}%` }}
          />
        </div>
        <span
          className={`text-[12px] tabular-nums w-16 text-right tracking-wider ${
            value < 50
    ...
    <div className="flex items-end gap-1 h-6">
      {[
        ...
        return (
          <Tooltip
            key={label}
            content={`${label}: ${value.toFixed(1)}ms`}
            position="top"
          >
            <div className="flex flex-col items-center gap-0.5 cursor-default">
              ...
              />
              <span className="text-[7px] text-[var(--color-text-dim)] tracking-wider">
                {label}
              </span>
            </div>
          </Tooltip>
  ...
  async function handleTest(name: string) {
    ...
    try {
      ...
      try {
        ...
        setHealthData((prev) =>
          prev.map((item) => (item.connection_name === name ? h : item)),
        );
      } catch {}
    } catch (e) {
      setTestResults((prev) => ({
      ...
      <div className="p-4 sm:p-8 animate-fade-in">
        <PageHeader
          ...
          actions={
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 text-[12px] text-[var(--color-text-dim)] cursor-pointer tracking-wider px-3 py-2.5 sm:py-1.5 border border-[var(--color-border)] hover:border-[var(--color-border-hover)] transition-colors">
                <input
                  type="checkbox"
                  checked={autoRefresh}
        ...
        <TerminalBar
          ...
          status={
            <StatusDot
              status={
                overallHealthy === overallTotal && overallTotal > 0
                  ? "healthy"
                  : overallTotal > 0
                    ? "warning"
                    : "unknown"
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-px mb-8 bg-[var(--color-border)] stagger-fade-in">
          ...
          <div className="bg-[var(--color-bg-card)] p-3 sm:p-5 hover:bg-[var(--color-bg-hover)] transition-colors card-accent-top">
            <div className="flex items-center gap-2 mb-3">
              <Wifi
                className={`w-3.5 h-3.5 ${overallHealthy === overallTotal && overallTotal > 0 ? "text-[var(--color-success)]" : "text-[var(--color-text-dim)]"}`}
                strokeWidth={1.5}
              />
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p
                  className={`text-lg font-light tabular-nums ${overallHealthy === overallTotal && overallTotal > 0 ? "text-[var(--color-success)]" : ""}`}
                >
                  {overallHealthy}/{overallTotal}
                </p>
          ...
          <div className="bg-[var(--color-bg-card)] p-3 sm:p-5 hover:bg-[var(--color-bg-hover)] transition-colors card-accent-top">
            ...
            <p className="text-[12px] text-[var(--color-text-dim)] mt-1 tracking-wider">
              all connections
            </p>
          </div>
          {/* Query Cache */}
          <div className="bg-[var(--color-bg-card)] p-3 sm:p-5 hover:bg-[var(--color-bg-hover)] transition-colors card-accent-top">
            <div className="flex items-center gap-2 mb-3">
              <Zap
                className={`w-3.5 h-3.5 ${cacheStats && cacheStats.hit_rate > 0.7 ? "text-[var(--color-success)]" : "text-[var(--color-text-dim)]"}`}
                strokeWidth={1.5}
              />
              <span className="text-[12px] text-[var(--color-text-dim)] uppercase tracking-[0.15em]">
                query cache
            ...
            <div className="flex items-center gap-3">
              <RingGauge
                ...
                color={
                  cacheStats && cacheStats.hit_rate > 0.7
                    ? "var(--color-success)"
                    : cacheStats && cacheStats.hit_rate > 0.3
                      ? "var(--color-warning)"
                      : "var(--color-error)"
                }
              />
              <div>
                <p
                  className={`text-lg font-light tabular-nums ${cacheStats && cacheStats.hit_rate > 0.7 ? "text-[var(--color-success)]" : ""}`}
                >
                  {cacheStats
          ...
          <div className="bg-[var(--color-bg-card)] p-3 sm:p-5 hover:bg-[var(--color-bg-hover)] transition-colors card-accent-top">
            ...
            <p className="text-xl font-light tabular-nums">
              {schemaCache ? String(schemaCache.cached_connections) : "--"}
            </p>
            <p className="text-[12px] text-[var(--color-text-dim)] mt-1 tracking-wider">
              {schemaCache
              ...
              {healthData.map((health) => {
                ...
                return (
                  <div
                    ...
                    className="bg-[var(--color-bg-card)] overflow-hidden"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-5 py-3 border-b border-[var(--color-border)]">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h3 className="text-xs text-[var(--color-text)] truncate">
                            {health.connection_name}
                          </h3>
                          <span className="text-[12px] text-[var(--color-text-dim)] tracking-wider truncate block">
                            {health.db_type}
                      ...
                      <div className="flex items-center gap-2">
                        <span
                          className={`flex items-center gap-1 text-[12px] tracking-wider ${cfg.color}`}
                        >
                        ...
                        <button
                          ...
                          className="flex items-center gap-1 px-3 py-2.5 sm:py-1 text-[12px] text-[var(--color-text-dim)] hover:text-[var(--color-text)] hover:bg-[var(--color-bg-hover)] transition-all disabled:opacity-50 tracking-wider active:bg-[var(--color-bg-hover)]"
                        >
                    ...
                    <div className="px-4 sm:px-5 py-4 space-y-4">
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center justify-between">
                          <div className="text-[11px] text-[var(--color-text-dim)] uppercase tracking-[0.15em]">
                            latency
                          </div>
                          <LatencyDistribution
                            p50={health.latency_p50_ms}
                            p95={health.latency_p95_ms}
                            p99={health.latency_p99_ms}
                          />
                        </div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        {[
                          {
                            label: "samples",
                            value: health.sample_count,
                            color: "",
                          },
                          {
                            label: "success",
                            value: health.successes ?? "--",
                            color: "text-[var(--color-success)]",
                          },
                          {
                            label: "failures",
                          ...
                          <div key={stat.label} className="flex-1">
                            <p className="text-[11px] text-[var(--color-text-dim)] uppercase tracking-[0.15em]">
                              {stat.label}
                            </p>
                            <p
                              className={`text-xs font-light tabular-nums ${stat.color}`}
                            >
                              {stat.value}
                            </p>
                          </div>
                        ...
                        {health.consecutive_failures != null &&
                          health.consecutive_failures > 0 && (
                            <div>
                              ...
                              <p className="text-xs font-light tabular-nums text-[var(--color-error)]">
                      ...
                      {health.last_error && (
                        <div className="p-2.5 border border-[var(--color-error)]/20 bg-[var(--color-error)]/5">
                          <p className="text-[11px] text-[var(--color-error)] tracking-wider uppercase mb-0.5">
                            last error
                          </p>
                      ...
                      {testResult && (
                        <div
                          className={`p-2.5 border ${testResult.status === "ok" ? "border-[var(--color-success)]/20 bg-[var(--color-success)]/5" : "border-[var(--color-error)]/20 bg-[var(--color-error)]/5"} animate-fade-in`}
                        >
                          <p
                            className={`text-[12px] tracking-wider ${testResult.status === "ok" ? "text-[var(--color-success)]" : "text-[var(--color-error)]"}`}
                          >
                            {testResult.message}
                          </p>
                        </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8">
                <div>
                  <div className="text-[12px] text-[var(--color-text-dim)] mb-3 flex items-center gap-2 tracking-wider">
                    <BarChart3 className="w-3 h-3" strokeWidth={1.5} /> query
                    cache
                  </div>
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between text-[12px] mb-1">
                        <span className="text-[var(--color-text-dim)] tracking-wider">
                          hit rate
                        </span>
                    ...
                    <div>
                      ...
                      <div className="w-full h-1.5 bg-[var(--color-bg)] overflow-hidden">
                        <div
                          className="h-full bg-[var(--color-text-dim)] transition-all"
                          style={{
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-1">
                      <div>
                        ...
                        <p className="text-xs font-light tabular-nums text-[var(--color-success)]">
                      ...
                      <div>
                        ...
                        <p className="text-xs font-light tabular-nums text-[var(--color-text-muted)]">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {[
                        {
                          label: "connections",
                          value: schemaCache.cached_connections,
                        },
