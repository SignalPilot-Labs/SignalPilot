...
// ---------------------------------------------------------------------------
// MCP URL helper (mirrors mcp-connect page)
// ---------------------------------------------------------------------------
...
const STEP_LABELS = ["welcome", "api key", "mcp connect", "done"];
      ...
      {createdKey ? (
        ...
        <div className="border border-[var(--color-border)] bg-[var(--color-bg-card)] p-5 mb-6">
          <div className="space-y-4">
            ...
            {/* Scopes */}
            <div>
              <span className="block text-[12px] text-[var(--color-text-dim)] mb-2 tracking-wider">
                scopes
              </span>
  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-[12px] text-[var(--color-text-dim)] tracking-wider leading-relaxed">
          add the signalpilot mcp server to your ai client. below is the
          configuration for claude desktop — other clients use the same url and
          auth pattern.
        </p>
      </div>
      ...
      {!createdKey && (
        <p className="text-[12px] text-[var(--color-text-dim)] tracking-wider mb-4">
          replace{" "}
          <code className="text-[var(--color-text-muted)]">
            sp_your_api_key_here
          </code>{" "}
          with your actual api key once created.
        </p>
      )}
        ...
        <div className="border border-[var(--color-border)] bg-[var(--color-bg-card)] divide-y divide-[var(--color-border)]">
          <div className="flex items-center gap-3 px-5 py-3">
            <Key
              ...
              strokeWidth={1.5}
            />
            <span className="flex-1 text-[12px] text-[var(--color-text-dim)] tracking-wider">
              api key
            </span>
            {createdKey ? (
...
// ---------------------------------------------------------------------------
// OnboardingContent — inner component, calls useOnboardingStatus + useBackendClient
// ---------------------------------------------------------------------------
...
function OnboardingContent() {
  ...
  return (
    <div className="p-8 max-w-2xl animate-fade-in">
      ...
      {step === 2 && (
        <StepConnectMcp onNext={() => setStep(3)} createdKey={createdKey} />
      )}
      {step === 3 && (
        <StepDone createdKey={createdKey} onFinish={handleFinish} />
      )}
    </div>
  );
}
...
// ---------------------------------------------------------------------------
// OnboardingPage — gate: redirects local-mode users, renders OnboardingContent
// ---------------------------------------------------------------------------
