...
//   "loading"     → initial check for existing session / auto-launch
//   "no-session"  → show landing page (Open IDE button)
//   "ready"       → session exists, render NotebookProvider + NotebookBoot
...
export default function ProjectsPage() {
  ...
  // Extract session token from a notebook_url like "...?token=abc123"
  function extractToken(notebookUrl: string): string | null {
    const match = notebookUrl.match(/[?&]token=([^&]+)/);
    return match ? match[1] : null;
  }

  // Build NotebookConfig from session data + local state
  ...
  // Single init: fetch API key, check session, auto-launch if needed.
  useEffect(() => {
    let cancelled = false;
    ...
    async function init() {
      ...
      // 2. Check for existing running session
      try {
      ...
      // 3. No session found — auto-launch on deep-link, otherwise show landing
  ...
  if (state === "loading") {
    return (
      <div className="flex flex-col h-screen">
        <div className="flex items-center px-4 py-2 border-b border-[var(--color-border)] bg-[var(--color-bg)] gap-3">
  ...
  // ─── Render: No session (landing page) ───────────────────────
  return (
