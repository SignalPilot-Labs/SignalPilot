...
const statusConfig: Record<string, { indicator: string; label: string }> = {
  ...
  starting: {
    indicator: "bg-[var(--color-warning)] pulse-dot",
    label: "starting",
  },
      ...
      <div className="p-4 sm:p-8 animate-fade-in">
        ...
        <TerminalBar
          ...
          status={
            <StatusDot
              status={
                sandboxes.some((s) => s.status === "running")
                  ? "healthy"
                  : sandboxes.length > 0
                    ? "warning"
                    : "unknown"
              ...
              size={4}
              pulse={sandboxes.some((s) => s.status === "running")}
            />
          }
        >
          <div className="flex items-center gap-3 sm:gap-6 text-xs">
            ...
            <span className="text-[var(--color-text-dim)]">
              running:{" "}
              <code className="text-[12px] text-[var(--color-success)]">
        ...
        {sandboxes.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-[var(--color-border)] stagger-fade-in">
            {sandboxes.map((sb) => {
              ...
              const budgetPct =
                sb.budget_usd > 0 ? (sb.budget_used / sb.budget_usd) * 100 : 0;
                ...
                >
                          ...
                          backgroundColor:
                            sb.status === "running"
                              ...
                              : sb.status === "error"
                                ? "var(--color-error)"
                                : "var(--color-text-dim)",
                          opacity: sb.status === "running" ? 1 : 0.4,
                        }}
                      />
                  ...
                  <div className="p-4 space-y-2.5">
                    ...
                    <div>
                      ...
                      {sb.vm_id && (
                        <code className="text-[11px] text-[var(--color-text-dim)] tracking-wider mt-0.5 block pl-5">
                          {sb.vm_id}
                        </code>
                      )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[12px] text-[var(--color-text-dim)] tracking-wider">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {sb.uptime_sec != null && sb.uptime_sec > 0 && (
                          <span className="text-[var(--color-text-dim)]">
                            (
                            {sb.uptime_sec < 60
                              ? `${sb.uptime_sec.toFixed(0)}s`
                              : `${(sb.uptime_sec / 60).toFixed(0)}m`}
                            )
                          </span>
                        )}
                      </div>
                      <div className="flex items-center gap-1.5 tabular-nums">
                        ...
                        {sb.budget_used.toFixed(4)} / $
                        {sb.budget_usd.toFixed(2)}
                      </div>
                      <div className="flex items-center gap-1.5">
                        <Shield
