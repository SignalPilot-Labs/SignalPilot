...
const dashboardFilterRuleSchema = z
  .object({
    ...
    tileTargets: z.record(z.string(), z.union([fieldTargetSchema, z.literal(false)])).optional(),
...
type FilterGroupInput =
  | { id: string; and: Array<FilterGroupInput | z.infer<typeof filterRuleSchema>> }
  | { id: string; or: Array<FilterGroupInput | z.infer<typeof filterRuleSchema>> };

const filterGroupSchema: z.ZodType<FilterGroupInput> = z.lazy(() =>
  z.union([
    z.object({ id: z.string().min(1), and: z.array(z.union([filterGroupSchema, filterRuleSchema])) }).strict(),
    z.object({ id: z.string().min(1), or: z.array(z.union([filterGroupSchema, filterRuleSchema])) }).strict(),
  ]),
);
...
const logicalTypeSchema = z.enum(["string", "number", "boolean", "date", "timestamp"]);
...
const sqlQuerySchema = z
  .object({
    ...
    parameterDefinitions: z.array(
      z.object({ name: z.string().min(1), logicalType: logicalTypeSchema, nullable: z.boolean() }).strict(),
    ),
...
const visualizationSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    ...
    config: z.object({ columns: z.array(z.string().min(1)).min(1), groups: z.array(z.string().min(1)).optional() }).strict(),
  }).strict(),
  z.object({
...
export const dashboardDefinitionSchema = z
  .object({
    ...
    name: z.string().min(1),
    description: z.string().optional(),
    filters: z.object({ dimensions: z.array(dashboardFilterRuleSchema), metrics: z.array(dashboardFilterRuleSchema) }).strict(),
    ...
    signalPilot: z.object({
      ...
      evalBindings: z.array(z.object({ chartId: z.string().min(1), evalId: z.string().min(1) }).strict()).optional(),
  ...
  .superRefine((definition, context) => {
    ...
    const chartById = new Map(definition.charts.map((chart) => [chart.id, chart]));
    for (const chart of definition.charts) {
      if (chartIds.has(chart.id)) context.addIssue({ code: "custom", path: ["charts"], message: `Duplicate chart id '${chart.id}'` });
      chartIds.add(chart.id);
      const outputs = new Set(chart.query.kind === "semantic" ? [...chart.query.dimensions, ...chart.query.metrics] : chart.query.outputBindings.map((item) => item.outputColumn));
      const encoded = chart.visualization.type === "big_number"
        ? [chart.visualization.config.field]
        : chart.visualization.type === "table"
          ? chart.visualization.config.columns
          : [chart.visualization.config.layout.xField, ...chart.visualization.config.layout.yField];
      for (const field of encoded) {
        if (!outputs.has(field)) context.addIssue({ code: "custom", path: ["charts", chart.id, "visualization"], message: `Encoding references unknown query field '${field}'` });
      }
    }
    for (const tile of definition.tiles) {
      if (tileIds.has(tile.uuid)) context.addIssue({ code: "custom", path: ["tiles"], message: `Duplicate tile id '${tile.uuid}'` });
      tileIds.add(tile.uuid);
      if (!chartById.has(tile.chartId)) context.addIssue({ code: "custom", path: ["tiles", tile.uuid, "chartId"], message: `Tile references unknown chart '${tile.chartId}'` });
      if (tile.x + tile.w > 36) context.addIssue({ code: "custom", path: ["tiles", tile.uuid], message: "Tile exceeds the 36-column grid" });
    }
  });
