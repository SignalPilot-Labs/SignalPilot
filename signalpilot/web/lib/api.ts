...
export const getTunnels = () =>
  request<import("./types").TunnelInfo[]>("/api/tunnels");
export const createTunnel = (t: { local_port: number; label?: string }) =>
  request<import("./types").TunnelInfo>("/api/tunnels", {
    method: "POST",
    body: JSON.stringify(t),
  });
export const getTunnel = (id: string) =>
  request<import("./types").TunnelInfo>(`/api/tunnels/${id}`);
export const deleteTunnel = (id: string) =>
  request<void>(`/api/tunnels/${id}`, { method: "DELETE" });
export const stopTunnel = (id: string) =>
  request<import("./types").TunnelInfo>(`/api/tunnels/${id}/stop`, {
    method: "POST",
  });
export const startTunnel = (id: string) =>
  request<import("./types").TunnelInfo>(`/api/tunnels/${id}/start`, {
    method: "POST",
  });
