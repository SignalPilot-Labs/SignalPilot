...
const INTERACTIVE_SELECTOR =
  'input, button, select, textarea, a, label, [role="checkbox"], [role="button"], [contenteditable="true"], sp-ui-element';

// `<sp-ui-element>` wraps every stateful UIElement, but content-wrapper
// UIElements like `sp.lazy` and `sp.routes` are themselves inert. Clicks on
// their inner content should still allow cell selection.
// See https://github.com/SignalPilot-Labs/SignalPilot/issues/9189.
const CONTENT_WRAPPER_SP_TAGS: ReadonlySet<string> = new Set([
  "sp-lazy",
  "sp-routes",
]);
 ...
 * sp plugins. Without this, `event.target` is retargeted to the outermost
 * shadow host (e.g. `<sp-lazy>`), hiding any genuinely interactive
...
export function isInteractiveTarget(e: React.MouseEvent): boolean {
  ...
  for (const node of path) {
    ...
    // A `<sp-ui-element>` directly wrapping a passive content-wrapper is
    // inert; keep walking to find a real interactive ancestor (if any).
    if (node.localName === "sp-ui-element") {
      const inner = node.firstElementChild;
      if (inner && CONTENT_WRAPPER_SP_TAGS.has(inner.localName)) {
        continue;
      }
    }
    return true;
