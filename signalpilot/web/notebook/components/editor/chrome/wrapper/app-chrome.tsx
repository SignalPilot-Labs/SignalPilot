...
const LazyAgentChatPanel = React.lazy(
  () => import("@/components/chat/agent-chat-panel"),
);
...
const LazyDbtPanel = React.lazy(
  () => import("@/components/editor/dbt/dbt-panel"),
);
...
export const AppChrome: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  return (
    <PanelsWrapper>
      <PanelGroup autoSaveId="sp:chrome:v1:l2" direction={"horizontal"}>
        <TooltipProvider>
          <Sidebar />
        </TooltipProvider>
        {helperPanel}
        ...
        >
          <PanelGroup autoSaveId="sp:chrome:v1:l1" direction="vertical">
            {appBodyPanel}
            {bottomPanel}
          </PanelGroup>
        </Panel>
