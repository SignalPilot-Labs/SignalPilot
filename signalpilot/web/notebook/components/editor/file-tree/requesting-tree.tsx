...
export class RequestingTree {
  ...
  constructor(
    ...
    if (initialRootPath) {
      this.rootPath = initialRootPath as FilePath;
    }
  }
  ...
  initialize = async (onChange: (data: FileInfo[]) => void): Promise<void> => {
    ...
    try {
      ...
      // leave delegate empty so the error state is visible, not stale
    }
  ...
  /**
   * Re-fetch the root listing AND each currently-expanded folder.
   * Folders that no longer exist in the new root are skipped (find returns undefined).
   * Errors leave the existing children intact — never blanks them out.
   */
