...
const SP_CELL_MIMETYPE = "web application/x-sp-cell";
...
async function writeCellsToClipboard(
  ...
  const clipboardItem = new ClipboardItemBuilder()
    .add(SP_CELL_MIMETYPE, clipboardData)
  ...
  const pasteAtCell = useEvent(async (cellId: CellId, opts?: PasteOptions) => {
    ...
    try {
      ...
      for (const item of clipboardItems) {
        if (item.types.includes(SP_CELL_MIMETYPE)) {
          const blob = await item.getType(SP_CELL_MIMETYPE);
