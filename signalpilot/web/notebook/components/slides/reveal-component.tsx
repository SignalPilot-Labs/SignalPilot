...
function triggerResize(deck: RevealApi | null) {
  if (deck?.getCurrentSlide()?.querySelector(".vega-embed, sp-vega")) {
...
const RevealSlidesComponent = ({
  ...
  const codeToggleEnabled = codeAvailable;
          ...
          <div className="absolute top-2 right-2 z-20 opacity-0 group-hover:opacity-70 text-muted-foreground transition-opacity">
            {codeToggleEnabled && (
              <Tooltip content={codeShown ? "Hide code (C)" : "Show code (C)"}>
                <Button
                  data-testid="sp-plugin-slides-toggle-code"
                  variant="ghost"
                  size="icon"
                  className={cn(
            ...
            <Tooltip content="Fullscreen (F)">
              <Button
                data-testid="sp-plugin-slides-fullscreen"
                variant="ghost"
                size="icon"
