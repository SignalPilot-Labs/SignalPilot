...
const DialogContent = React.forwardRef<
  ...
  { usePortal?: boolean } & React.ComponentPropsWithoutRef<
...
>(({ className, children, usePortal = true, ...props }, ref) => {
  ...
  const content = (
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed z-50 grid w-full gap-4 rounded-lg border bg-background p-6 shadow-sm animate-in data-[state=open]:fade-in-90 data-[state=open]:slide-in-from-bottom-10 sm:max-w-2xl sm:mx-4 sm:rounded-lg sm:zoom-in-90 sm:data-[state=open]:slide-in-from-bottom-0",
        className,
      )}
      {...restoreFocus}
      {...props}
