...
export const ReorderableList = <T extends object>({
  ...
  // React Aria's drag sets aria-hidden="true" on the dragged element while it
  // still has focus. Browsers kill the focus and the component tree crashes.
  // This observer detects the conflict and blurs the element before the browser
  // throws the accessibility violation.
    ...
    const observer = new MutationObserver((mutations) => {
      for (const m of mutations) {
        if (m.type === "attributes" && m.attributeName === "aria-hidden") {
          ...
          if (target.getAttribute("aria-hidden") === "true" && target.contains(document.activeElement)) {
            (document.activeElement as HTMLElement)?.blur();
          }
        }
      }
    });
    observer.observe(el, { attributes: true, subtree: true, attributeFilter: ["aria-hidden"] });
