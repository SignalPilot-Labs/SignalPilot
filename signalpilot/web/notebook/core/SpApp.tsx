...
import { Provider as SlotzProvider } from "@/packages/react-slotz";
...
/**
 * The root component of the SignalPilot notebook.
 */
export const SpApp: React.FC = memo(() => {
  const [userConfig] = useResolvedSpConfig();
  const [appConfig] = useAppConfig();
  const editorFontSize = toRem(userConfig.display.code_editor_font_size);
  const mode = useAtomValue(currentModeAtom);

  // When on the home page, preload the edit page chunk so the first
  // home → edit transition is instant (no visible Suspense flash).
  useEffect(() => {
    if (mode === "home") {
      LazyEditPage.preload();
    }
