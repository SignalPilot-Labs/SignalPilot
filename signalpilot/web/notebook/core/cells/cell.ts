...
export function transitionCell(
  ...
  // Handle errors: the kernel includes an error output when a cell is interrupted
  // or errored
  if (
    message.output != null &&
    message.output.mimetype === "application/vnd.sp+error"
  ) {
...
export function outputIsStale(
  ...
  // Output is received while the cell is running (e.g. sp.output.append())
 ...
 * "application/vnd.sp+traceback" and parses its data to retrieve
...
export function outputToTracebackInfo(
  ...
  const firstTraceback = outputs.find(
    (output) => output.mimetype === "application/vnd.sp+traceback",
  );
  if (!firstTraceback) {
    return undefined;
  }
