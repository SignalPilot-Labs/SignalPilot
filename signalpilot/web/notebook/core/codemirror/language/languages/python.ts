...
} from "@/packages/codemirror-languageserver";
...
const pylspClient = once((lspConfig: LSPConfig) => {
  ...
  const ignoredFlakeRules = [
    ...
    // Blank line rules are not useful in sp-notebook because cells are joined
  ...
  const settings = {
    pylsp: {
      plugins: {
        // pylsp plugin name; must match the registered entry-point name
        sp_plugin: {
          enabled: true,
        },
        jedi: {
...
function getPlaceholderExtension(placeholderType: PlaceholderType): Extension {
  if (placeholderType === "sp-import") {
    return Prec.highest(smartPlaceholderExtension("import signalpilot as sp"));
  }
