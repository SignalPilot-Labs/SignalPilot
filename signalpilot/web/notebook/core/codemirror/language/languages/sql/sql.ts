...
import {
  ...
  type SqlParseError,
  sqlExtension,
} from "@/packages/codemirror-sql";
import { DuckDBDialect } from "@/packages/codemirror-sql/dialects";
import { type SQLMetadata, SQLParser } from "@/packages/smart-cells";
...
export class SQLLanguageAdapter implements LanguageAdapter<SQLLanguageAdapterMetadata> {
  ...
  get defaultCode(): string {
    ...
    if (engine && engine !== DUCKDB_ENGINE) {
      return `_df = sp.sql(f"""SELECT * FROM """, engine="${engine}")`;
    }
