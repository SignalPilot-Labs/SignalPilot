...
import { topologicalCodesAtom } from "./topological-codes";
import {
 ...
 * Private variables in sp-notebook are cell-local and should not be renamed across cells.
 */
...
export class NotebookLanguageServerClient implements ILanguageServerClient {
  ...
  public async resyncAllDocuments(): Promise<void> {
    ...
    await this.client.textDocumentDidOpen({
      textDocument: {
        languageId: "python", // Default to Python for SignalPilot notebooks
