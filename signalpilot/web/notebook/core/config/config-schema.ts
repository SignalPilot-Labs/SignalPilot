...
import type { SpConfig } from "../network/types";
...
export const UserConfigSchema = z
  .looseObject({
    ...
    display: z
      .looseObject({
        theme: z.enum(["light", "dark", "system"]).prefault("dark"),
...
/** AI model configuration (client-only; not in the OpenAPI SpConfig schema). */
...
/**
 * UserConfig extends SpConfig with client-side-only fields (e.g. `ai` models)
 * that the Python backend stores under the same config namespace but are not
 * yet reflected in the generated OpenAPI schema.
 */
export type UserConfig = SpConfig & {
  ai?: AiConfig;
};
...
export function parseUserConfig(config: unknown): UserConfig {
  try {
    ...
    if (error instanceof z.ZodError) {
      Logger.error(
        `Unexpected config value: ${z.prettifyError(error)}`,
      );
    } else {
      Logger.error(
        `Unexpected config value: ${error}`,
...
export function parseConfigOverrides(config: unknown): {} {
  try {
    ...
    invariant(
      ...
      "internal-error: sp-config-overrides is not an object",
    );
    ...
    Logger.error(`Unexpected configuration overrides: ${error}`);
    return {};
  }
}
