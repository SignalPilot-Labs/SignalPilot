...
import type { ConnectionName } from "./engines";
import { datasetTablesAtom } from "./state";

function createInitialConnections(): ConnectionsMap {
  return new Map();
}
...
function initialState(): DataSourceState {
  return {
    latestEngineSelected: "" as ConnectionName,
    connectionsMap: createInitialConnections(),
  };
}
...
const {
  ...
  addDataSourceConnection: (
    ...
    return {
      latestEngineSelected: latestEngineSelected || opts.connections[0]?.name as ConnectionName || ("" as ConnectionName),
      connectionsMap: newMap,
    };
  },

  // Gateway connections are not Python variables — keep all connections
  filterDataSourcesFromVariables: (
    state: DataSourceState,
    _variableNames: VariableName[],
  ) => {
    return state;
  },
  ...
  removeDataSourceConnection: (
    ...
    return {
      latestEngineSelected: newMap.has(latestEngineSelected)
        ...
        : ([...newMap.keys()][0] ?? ("" as ConnectionName)),
      connectionsMap: newMap,
    };
  },
