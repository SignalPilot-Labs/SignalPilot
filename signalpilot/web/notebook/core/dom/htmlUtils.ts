 ...
 * It will first check if the parent element is a <sp-ui-element/> with an object-id that
...
export function parseInitialValue<T>(
  ...
  // If parent is a <sp-ui-element/> and has object-id, use that as the initialize the value
...
export function getFilenameFromDOM() {
  ...
  const filenameTag = document.querySelector("sp-filename");
  if (BUILD_ENV.MODE === "test" && !filenameTag) {
    return null;
  }
  assertExists(filenameTag, "sp-filename tag not found");
