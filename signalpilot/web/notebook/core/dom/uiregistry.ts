...
function isUIValueUpdateMessage(msg: unknown): msg is UIValueUpdateMessage {
  ...
  return "type" in msg && msg.type === "sp-ui-value-update";
}
...
export class UIElementRegistry {
  ...
  static get INSTANCE(): UIElementRegistry {
    return getCurrentRegistries().uiElementRegistry;
  }
  ...
  broadcastMessage(
    ...
    // dispatching SpValueReadyEvent to avoid a round-trip.
    if (isUIValueUpdateMessage(message)) {
      entry.value = message.value;
      entry.elements.forEach((element) => {
        element.dispatchEvent(
          SpValueUpdateEvent.create({
            bubbles: false,
            composed: true,
...
// UI_ELEMENT_REGISTRY is a Proxy so that import-time capture does not freeze
// the module-singleton registry. Every method call routes through
// UIElementRegistry.INSTANCE, which delegates to the active client's registry
// via getCurrentRegistries(). Callers that cache this const across client
// lifetimes are safe because the Proxy always reads the live current value.
export const UI_ELEMENT_REGISTRY: UIElementRegistry = new Proxy(
  {} as UIElementRegistry,
  {
    get(_t, p) {
      const target = UIElementRegistry.INSTANCE;
