...
export function createNetworkRequests(): EditRequests & RunRequests {
  ...
  return {
    ...
    listStorageEntries: (request) => {
      // /api/storage/list_entries is not yet in the generated OpenAPI schema;
      // cast the entire call to bypass strict PathsWithMethod check.
      type OkResponse = { error?: Record<string, unknown>; response: Response };
      return (getClient().POST as (...args: unknown[]) => Promise<OkResponse>)(
        "/api/storage/list_entries",
        { body: request, params: getParams() },
      ).then(handleResponseReturnNull);
    },
    downloadStorage: (request) => {
      // /api/storage/download is not yet in the generated OpenAPI schema;
      // cast the entire call to bypass strict PathsWithMethod check.
      type OkResponse = { error?: Record<string, unknown>; response: Response };
      return (getClient().POST as (...args: unknown[]) => Promise<OkResponse>)(
