...
export function useSaveNotebook() {
  ...
  const saveNotebook = useEvent(
    async (filename: string, userInitiated: boolean) => {
      ...
      // DEFENSE: Verify filename matches the current active file
      // Prevents autosave from writing one notebook's cells to another's file
      const currentFilename = store.get(filenameAtom);
      if (currentFilename && filename && currentFilename !== filename) {
        Logger.warn(
          `save blocked: filename mismatch (saving "${filename}" but current file is "${currentFilename}")`,
        );
        return;
      }
...
export function useAutoSaveNotebook(opts: {
  ...
  const currentFilename = useAtomValue(filenameAtom);
