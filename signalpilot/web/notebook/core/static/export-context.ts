...
declare global {
  interface Window {
    __SP_EXPORT_CONTEXT__?: Readonly<SpExportContext>;
  }
}

function isSpExportContext(
  value: unknown,
): value is Readonly<SpExportContext> {
  if (typeof value !== "object" || value === null) {
    return false;
  }

  const candidate = value as SpExportContext;
...
export function getSpExportContext():
  | Readonly<SpExportContext>
  | undefined {
  const context = window?.__SP_EXPORT_CONTEXT__;
  return isSpExportContext(context) ? context : undefined;
}

export function hasTrustedExportContext(): boolean {
  return getSpExportContext()?.trusted === true;
}
 ...
 *   configuration), OR
 * - the page was loaded in `read` / app mode (served as an app).
 *
...
export function hasTrustedNotebookContext(): boolean {
  ...
  try {
    if (getCurrentAppMode() === "read") {
      return true;
    }
  } catch {
    // getCurrentAppMode may throw in unusual environments; treat as untrusted.
  }
  return false;
}
