...
import { QUOTE_PREFIX_KINDS } from "../types";
import { splitQuotePrefix, unescapeQuotes } from "../utils/index";
...
const regexes = pairs.map(
  ([start, end]) =>
    [
      start,
      new RegExp(
        `^sp\\.md\\(\\s*${escapeRegex(start)}(.*)${escapeRegex(end)}\\s*\\)$`,
        "s",
      ),
    ] as const,
);
...
/**
 * Parser for sp Markdown cells (sp.md()).
 *
 * Converts between Python code like `sp.md(r"""# Hello""")` and
 * plain Markdown like `# Hello`.
 */
export class MarkdownParser implements LanguageParser<MarkdownMetadata> {
  readonly type = "markdown";
  readonly defaultCode = 'sp.md(r"""\n""")';
  ...
  transformOut(code: string, metadata: MarkdownMetadata): FormatResult {
    // NB. Must be kept consistent with the backend markdown converter
  ...
  isSupported(pythonCode: string): boolean {
    ...
    // Must start with sp.md(
    if (!pythonCode.startsWith("sp.md(")) {
      return false;
    }
    ...
    // Parse the code using Lezer and check for the exact match of sp.md() signature
    const tree = pythonLanguage.parser.parse(pythonCode);

    // This is the exact match of sp.md() signature
