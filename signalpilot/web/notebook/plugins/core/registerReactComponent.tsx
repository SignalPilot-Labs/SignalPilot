...
import { PluginLoadingFallback } from "./plugin-loading-fallback";
...
export interface ISpHTMLElement extends HTMLElement {
  /**
   * Reset the plugin initial value and data.
   */
  reset: () => void;
  ...
  return (
    <StyleNamespace>
      <div className={`contents ${theme}`}>
        <Suspense fallback={<PluginLoadingFallback host={hostElement} />}>
          <TooltipProvider>
            {plugin.render({
...
const customElementLocator = "__custom_sp_element__";
...
export function registerReactComponent<T>(plugin: IPlugin<T, unknown>): void {
  const WebComponent = class extends HTMLElement implements ISpHTMLElement {
    ...
    /**
     * Check if this element is in the light DOM of another sp
     * custom element. When an sp element (e.g. sp-tabs) has
     * children that are also sp elements (e.g. sp-lazy), the
     ...
     * (shadow root or document), so shadow DOM copies never find an
     * sp ancestor and correctly return false.
     */
    private isLightDOMChildOfSpElement(): boolean {
      let parent = this.parentElement;
      while (parent) {
        if (isCustomSpElement(parent)) {
          return true;
        }
...
function shouldCopyStyleSheet(sheet: CSSStyleSheet): boolean {
  // Copy local stylesheets owned by sp
  if (sheet.title?.startsWith("sp")) {
    return true;
  }
