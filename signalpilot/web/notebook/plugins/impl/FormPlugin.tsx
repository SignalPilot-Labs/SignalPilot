...
export const FormPlugin = createPlugin("sp-form")
  .withData<Data>(
    z.object({
      ...
      >
        ...
        <div className="flex justify-end gap-2 font-code">
          {showClearButton &&
            withTooltip(
              <Button
                data-testid="sp-plugin-form-clear-button"
                variant="text"
                onClick={(e) => {
                  e.preventDefault();
                  clear();
          ...
          {withTooltip(
            <Button
              data-testid="sp-plugin-form-submit-button"
              variant={variant}
...
const Form = ({
  ...
  useEffect(() => {
    // Spy on when the plugin generates an event (SpValueInputEvent)
    const handleUpdate = (e: SpValueInputEventType) => {
    ...
    document.addEventListener(SpValueInputEvent.TYPE, handleUpdate);
    return () => {
      document.removeEventListener(SpValueInputEvent.TYPE, handleUpdate);
    };
  }, [elementId, setValue]);
