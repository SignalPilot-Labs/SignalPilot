...
const spSymbol = Symbol("sp");

interface SpInternalApi<T extends ModelState> {
  /**
...
/**
 * Get the internal sp API for a Model instance.
 * These are not part of the public AnyModel interface.
 */
export function getSpInternal<T extends ModelState>(
  model: Model<T>,
): SpInternalApi<T> {
  return model[spSymbol];
}

// MODEL_MANAGER is a Proxy so that import-time capture does not freeze the
// module-singleton manager. Every method call routes through
// getCurrentRegistries().modelManager, which delegates to the active client's
// manager via the client-binding stack. Methods are bound to the real instance
// so that private field access (#entries, #timeout) works correctly.
export const MODEL_MANAGER: ModelManager = new Proxy({} as ModelManager, {
  get(_t, p) {
    const target = getCurrentRegistries().modelManager;
...
export class Model<T extends ModelState> implements AnyModel<T> {
  ...
  // Points to MODEL_MANAGER (a Proxy that delegates to the active client's
  // model manager). Tests may override this field directly for isolation;
  // in production code it always routes through the active client's manager.
  static _modelManager: ModelManager = MODEL_MANAGER;

  constructor(data: T, comm: SpComm<T>, signal?: AbortSignal) {
    this.#data = data;
    this.#comm = comm;
  ...
  /**
   * Internal sp API - not part of AnyWidget AFM.
   * Access via getSpInternal().
   */
  [spSymbol]: SpInternalApi<T> = {
...
export async function handleWidgetMessage(
  ...
  switch (msg.method) {
    case "open": {
      ...
      if (existingModel) {
        getSpInternal(existingModel).updateAndEmitDiffs(stateWithBuffers);
        return;
      }

      modelManager.create(modelId, (signal) => {
        // In static exports there is no kernel, so comm calls are no-ops.
        const comm: SpComm<ModelState> = isStaticNotebook()
          ? {
    ...
    case "custom": {
      ...
      getSpInternal(model).emitCustomMessage(
    ...
    case "update": {
      ...
      const model = await modelManager.get(modelId);
      getSpInternal(model).updateAndEmitDiffs(stateWithBuffers);
      return;
    }
