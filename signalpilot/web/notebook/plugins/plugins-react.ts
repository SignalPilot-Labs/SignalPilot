...
import { initializeCustomElements } from "./plugins";
...
/**
 * Initialize all React plugin classes (UI + layout). Does NOT register the
 * custom DOM elements — call initializeCustomElements() separately if you need
 * those without the full plugin set.
 */
export function initializeReactPlugins(): void {
  UI_PLUGINS.forEach(registerReactComponent);
  LAYOUT_PLUGINS.forEach(registerReactComponent);
}

/**
 * Full initializer: registers custom DOM elements AND all React plugins.
 * Use this from islands/bootstrap and any path that needs the complete set.
 */
export function initializePlugins(): void {
  initializeCustomElements();
  initializeReactPlugins();
}
