...
export class TreeNode<T> {
  ...
  private _descendantCount: number | undefined;
  private _descendants: T[] | undefined;
  private _inOrderIds: T[] | undefined;
  ...
  geDescendantCount(): number {
    if (this._descendantCount !== undefined) return this._descendantCount;
    let count = 0;
  ...
  getDescendants(): T[] {
    if (this._descendants !== undefined) return this._descendants;
    const result: T[] = [];
    const stack = [...this.children];
  ...
  get inOrderIds(): T[] {
    if (this._inOrderIds !== undefined) return this._inOrderIds;
    const result: T[] = [];
...
export class CollapsibleTree<T> {
  ...
  private _topLevelIds: T[] | undefined;
  private _inOrderIds: T[] | undefined;
  private _idSet: Set<T> | undefined;
  private _nodeMapCache: Map<T, TreeNode<T>> | undefined;
  ...
  get topLevelIds(): T[] {
    if (this._topLevelIds !== undefined) return this._topLevelIds;
    this._topLevelIds = this.nodes.map((n) => n.value);
    return this._topLevelIds;
  }

  get inOrderIds(): T[] {
    if (this._inOrderIds !== undefined) return this._inOrderIds;
    this._inOrderIds = this.nodes.flatMap((n) => [n.value, ...n.inOrderIds]);
    return this._inOrderIds;
  }

  get idSet(): Set<T> {
    if (this._idSet !== undefined) return this._idSet;
    this._idSet = new Set(this.inOrderIds);
    return this._idSet;
  }
  ...
  get _nodeMap(): Map<T, TreeNode<T>> {
    if (this._nodeMapCache !== undefined) return this._nodeMapCache;
    const result = new Map<T, TreeNode<T>>();
    for (const node of this.nodes) {
      result.set(node.value, node);
    }
    this._nodeMapCache = result;
    return result;
  }
  ...
  deleteById(id: T): CollapsibleTree<T> {
    const index = this.indexOfOrThrow(id);
...
export class MultiColumn<T> {
  ...
  private _topLevelIds: T[][] | undefined;
  private _inOrderIds: T[] | undefined;
  private _idLength: number | undefined;
  private _columnMapCache: Map<CellColumnId, CollapsibleTree<T>> | undefined;
  private _columnIds: CellColumnId[] | undefined;
  ...
  isEmpty(): boolean {
    ...
    this._isEmpty = this.columns.every((c) => c.nodes.length === 0);
  ...
  get topLevelIds(): T[][] {
    if (this._topLevelIds !== undefined) return this._topLevelIds;
    this._topLevelIds = this.columns.map((c) => c.topLevelIds);
    return this._topLevelIds;
  }
  ...
  get inOrderIds(): T[] {
    if (this._inOrderIds !== undefined) return this._inOrderIds;
    this._inOrderIds = this.columns.flatMap((c) => c.inOrderIds);
    return this._inOrderIds;
  }
  ...
  get idLength(): number {
    if (this._idLength !== undefined) return this._idLength;
    this._idLength = this.columns.reduce((acc, c) => acc + c.nodes.length, 0);
    return this._idLength;
  }

  get _columnMap(): Map<CellColumnId, CollapsibleTree<T>> {
    if (this._columnMapCache !== undefined) return this._columnMapCache;
    this._columnMapCache = new Map(this.columns.map((c) => [c.id, c]));
  ...
  getColumnIds(): CellColumnId[] {
    if (this._columnIds !== undefined) return this._columnIds;
    this._columnIds = this.columns.map((c) => c.id);
  ...
  deleteById(cellId: T): MultiColumn<T> {
    return this.transformWithCellId(cellId, (c) => c.deleteById(cellId));
  }
