...
/**
 * Check if a DOM node contains a cell file.
 */
export const elementContainsCellFile = (domNode: Element) => {
  return (
    domNode &&
    matchesSelector(domNode, "span.nb") &&
    domNode.firstChild instanceof Text &&
    domNode.firstChild.nodeValue?.includes("__sp__")
  );
};
 ...
 *
 *   File <span class="nb">"/tmp/sp_<number>/__sp__cell_<CellId>.py"</span>
...
export function getTracebackInfo(domNode: DOMNode): TracebackInfo | null {
  ...
  //
  // File <span class="nb">"/tmp/sp_<number>/__sp__cell_<cell-id>.py</span>
  // , line <span class="n">1</span>...
  //
  // into
  //
  // File sp://notebook#cell=<CellID>, line 1, in <module>
  if (
    ...
    if (nextSibling && nextSibling instanceof Text) {
      ...
      if (
        ...
        if (domNode.firstChild.nodeValue?.includes("__sp__")) {
          const maybeCellId = /__sp__cell_(\w+)_/.exec(
