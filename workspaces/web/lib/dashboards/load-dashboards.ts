...
import { DASHBOARD_CHART_TYPES, type DashboardDefinition } from "@/lib/dashboards/types";
...
export async function loadDashboards(): Promise<DashboardDefinition[]> {
  ...
  if (env.mode !== "local") {
    throw new Error("loadDashboards is only available in local mode");
  }
...
export async function loadDashboard(id: string): Promise<DashboardDefinition | null> {
  const env = getServerEnv();
  if (env.mode !== "local") {
    throw new Error("loadDashboard is only available in local mode");
  }
  ...
  const filepath = join(env.localDashboardsDir, `${id}.json`);
